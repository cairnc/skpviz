...
namespace {
// Returns true if the layer is root of a display and can be mirrored by
// mirroringLayer
bool canMirrorRootLayer(RequestedLayerState &mirroringLayer,
                        RequestedLayerState &rootLayer) {
  return rootLayer.isRoot() &&
         rootLayer.layerStack == mirroringLayer.layerStackToMirror &&
         rootLayer.id != mirroringLayer.id;
}
} // namespace

void LayerLifecycleManager::addLayers(
  ...
  for (auto &newLayer : newLayers) {
    ...
    auto [it, inserted] =
        mIdToLayer.try_emplace(layer.id, References{.owner = layer});
    ...
    if (layer.layerStackToMirror != ui::INVALID_LAYER_STACK) {
      // Set mirror layer's default layer stack to -1 so it doesn't end up
      // rendered on a display accidentally.
      layer.layerStack = ui::INVALID_LAYER_STACK;

      // if this layer is mirroring a display, then walk though all the existing
      // root layers for the layer stack and add them as children to be
      // mirrored.
      ...
      // Check if we are mirroring a single layer, and if so add it to the list
      // of children to be mirrored.
...
void LayerLifecycleManager::onHandlesDestroyed(
    const std::vector<std::pair<uint32_t, std::string /* debugName */>>
        &destroyedHandles,
  ...
  for (const auto &[layerId, name] : destroyedHandles) {
    ...
    if (it == mIdToLayer.end()) {
      LLOG_ALWAYS_FATAL_WITH_TRACE_IF(!ignoreUnknownHandles,
                                      "%s Layerid not found %s[%d]", __func__,
                                      name.c_str(), layerId);
      continue;
    }
  ...
  for (size_t i = 0; i < layersToBeDestroyed.size(); i++) {
    ...
    auto it = mIdToLayer.find(layerId);
    LLOG_ALWAYS_FATAL_WITH_TRACE_IF(it == mIdToLayer.end(),
                                    "%s Layer with id %d not found", __func__,
                                    layerId);
    ...
    for (uint32_t linkedLayerId : references) {
      ...
      LLOG_ALWAYS_FATAL_WITH_TRACE_IF(
          !linkedLayer, "%s Layerid reference %d not found for %d", __func__,
...
void LayerLifecycleManager::applyTransactions(
    const std::vector<TransactionState> &transactions,
    ...
    for (const auto &resolvedComposerState : transaction.states) {
      ...
      if (layer == nullptr) {
        LLOG_ALWAYS_FATAL_WITH_TRACE_IF(!ignoreUnknownLayers,
                                        "%s Layer with layerid=%d not found",
                                        __func__, layerId);
        continue;
      }
      ...
      if (layer->what & layer_state_t::eBackgroundColorChanged) {
        if (layer->bgColorLayerId == UNASSIGNED_LAYER_ID &&
            layer->bgColor.a != 0) {
          LayerCreationArgs backgroundLayerArgs(
              LayerCreationArgs::getInternalLayerId(
          ...
          addLayers({std::move(newLayers)});
                   ...
                   layer->bgColor.a == 0) {
          RequestedLayerState *bgColorLayer =
              getLayerFromId(layer->bgColorLayerId);
          layer->bgColorLayerId = UNASSIGNED_LAYER_ID;
          bgColorLayer->parentId =
              unlinkLayer(bgColorLayer->parentId, bgColorLayer->id);
          ...
          bgColorLayer->what |= layer_state_t::eColorChanged |
                                layer_state_t::eDataspaceChanged |
...
void LayerLifecycleManager::addLifecycleListener(
    std::shared_ptr<ILifecycleListener> listener) {
  mListeners.emplace_back(std::move(listener));
}

void LayerLifecycleManager::removeLifecycleListener(
    std::shared_ptr<ILifecycleListener> listener) {
  swapErase(mListeners, listener);
}

const std::vector<std::unique_ptr<RequestedLayerState>> &
LayerLifecycleManager::getLayers() const {
  return mLayers;
}

const std::vector<std::unique_ptr<RequestedLayerState>> &
LayerLifecycleManager::getDestroyedLayers() const {
  return mDestroyedLayers;
}

const std::vector<RequestedLayerState *> &
LayerLifecycleManager::getChangedLayers() const {
  return mChangedLayers;
}

const ftl::Flags<RequestedLayerState::Changes>
LayerLifecycleManager::getGlobalChanges() const {
  return mGlobalChanges;
}

const RequestedLayerState *
LayerLifecycleManager::getLayerFromId(uint32_t id) const {
...
std::vector<uint32_t> *
LayerLifecycleManager::getLinkedLayersFromId(uint32_t id) {
  if (id == UNASSIGNED_LAYER_ID) {
    return nullptr;
  }
  auto it = mIdToLayer.find(id);
...
uint32_t LayerLifecycleManager::linkLayer(uint32_t layerId,
                                          uint32_t layerToLink) {
  if (layerId == UNASSIGNED_LAYER_ID) {
  ...
  if (!linkedLayers) {
    ALOGV("Could not find layer id %d to link %d. Parent is probably destroyed",
          layerId, layerToLink);
    return UNASSIGNED_LAYER_ID;
  }
...
uint32_t LayerLifecycleManager::unlinkLayer(uint32_t layerId,
...
std::vector<uint32_t>
LayerLifecycleManager::unlinkLayers(const std::vector<uint32_t> &layerIds,
                                    uint32_t linkedLayer) {
...
std::string LayerLifecycleManager::References::getDebugString() const {
  std::string debugInfo =
      owner.name + "[" + std::to_string(owner.id) + "] refs:";
...
void LayerLifecycleManager::fixRelativeZLoop(uint32_t relativeRootId) {
  ...
  RequestedLayerState &layer = it->second.owner;
  layer.relativeParentId = unlinkLayer(layer.relativeParentId, layer.id);
  layer.changes |= RequestedLayerState::Changes::Hierarchy |
                   RequestedLayerState::Changes::RelativeParent;
  mGlobalChanges |= RequestedLayerState::Changes::Hierarchy;
}

// Some layers mirror the entire display stack. Since we don't have a single
// root layer per display we have to track all these layers and update what they
// mirror when the list of root layers on a display changes. This function walks
// through the list of display mirroring layers and updates its list of layers
// that its mirroring. This function should be called when a new root layer is
// added, removed or moved to another display.
void LayerLifecycleManager::updateDisplayMirrorLayers(
    RequestedLayerState &rootLayer) {

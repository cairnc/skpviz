...
namespace {
auto layerZCompare =
    [](const std::pair<LayerHierarchy *, LayerHierarchy::Variant> &lhs,
...
void insertSorted(
    std::vector<std::pair<LayerHierarchy *, LayerHierarchy::Variant>> &vec,
...
LayerHierarchy::LayerHierarchy(const LayerHierarchy &hierarchy,
                               bool childrenOnly) {
...
void LayerHierarchy::traverse(const Visitor &visitor,
  ...
  LLOG_ALWAYS_FATAL_WITH_TRACE_IF(
      depth > 50, "Cycle detected in LayerHierarchy::traverse. See "
  ...
  LLOG_ALWAYS_FATAL_WITH_TRACE_IF(traversalPath.hasRelZLoop(),
                                  "Found relative z loop layerId:%d",
                                  traversalPath.invalidRelativeRootId);
  for (auto &[child, childVariant] : mChildren) {
    ScopedAddToTraversalPath addChildToTraversalPath(
        traversalPath, child->mLayer->id, childVariant);
...
void LayerHierarchy::addChild(LayerHierarchy *child,
...
void LayerHierarchy::removeChild(LayerHierarchy *child) {
  ...
  LLOG_ALWAYS_FATAL_WITH_TRACE_IF(it == mChildren.end(),
...
void LayerHierarchy::updateChild(LayerHierarchy *hierarchy,
                                 LayerHierarchy::Variant variant) {
  auto it =
      std::find_if(mChildren.begin(), mChildren.end(),
...
const RequestedLayerState *LayerHierarchy::getLayer() const { return mLayer; }

const LayerHierarchy *LayerHierarchy::getRelativeParent() const {
  return mRelativeParent;
}

const LayerHierarchy *LayerHierarchy::getParent() const { return mParent; }
...
void LayerHierarchy::dump(std::ostream &out, const std::string &prefix,
  ...
  if (!mLayer) {
    ...
    out << *mLayer << " pid=" << mLayer->ownerPid.val()
        << " uid=" << mLayer->ownerUid.val();
  }
...
bool LayerHierarchy::hasRelZLoop(uint32_t &outInvalidRelativeRoot) const {
  ...
  traverse([&outInvalidRelativeRoot](
...
void LayerHierarchyBuilder::init(
...
void LayerHierarchyBuilder::attachToRelativeParent(LayerHierarchy *hierarchy) {
  ...
  hierarchy->mRelativeParent->addChild(hierarchy,
                                       LayerHierarchy::Variant::Relative);
  hierarchy->mParent->updateChild(hierarchy, LayerHierarchy::Variant::Detached);
}

void LayerHierarchyBuilder::detachFromRelativeParent(
...
std::vector<LayerHierarchy *>
LayerHierarchyBuilder::getDescendants(LayerHierarchy *root) {
...
void LayerHierarchyBuilder::attachHierarchyToRelativeParent(
    LayerHierarchy *root) {
...
void LayerHierarchyBuilder::detachHierarchyFromRelativeParent(
    LayerHierarchy *root) {
...
void LayerHierarchyBuilder::onLayerDestroyed(RequestedLayerState *layer) {
  LLOGV(layer->id, "");
  LayerHierarchy *hierarchy =
      getHierarchyFromId(layer->id, /*crashOnFailure=*/false);
  if (!hierarchy) {
    // Layer was never part of the hierarchy if it was created and destroyed in
  ...
  swapErase(mHierarchies,
            [hierarchy](std::unique_ptr<LayerHierarchy> &layerHierarchy) {
...
void LayerHierarchyBuilder::updateMirrorLayer(RequestedLayerState *layer) {
  ...
  for (uint32_t mirrorId : layer->mirrorIds) {
    hierarchy->addChild(getHierarchyFromId(mirrorId),
                        LayerHierarchy::Variant::Mirror);
  }
  if (FlagManager::getInstance().detached_mirror()) {
    if (layer->layerIdToMirror != UNASSIGNED_LAYER_ID) {
...
void LayerHierarchyBuilder::doUpdate(
  ...
  // When moving from onscreen to offscreen and vice versa, we need to attach
  // and detach from our relative parents. This walks down both trees to do so.
  // We can optimize this further by tracking onscreen, offscreen state in
  // LayerHierarchy.
  detachHierarchyFromRelativeParent(&mOffscreenRoot);
...
void LayerHierarchyBuilder::update(
    LayerLifecycleManager &layerLifecycleManager) {
  if (!mInitialized) {
...
std::string LayerHierarchyBuilder::getDebugString(uint32_t layerId,
                                                  uint32_t depth) const {
  if (depth > 10)
    return "too deep, loop?";
  ...
  std::string debug = "[" + std::to_string(hierarchy->mLayer->id) + "] " +
                      hierarchy->mLayer->name;
  if (hierarchy->mRelativeParent) {
...
LayerHierarchy *LayerHierarchyBuilder::getHierarchyFromId(uint32_t layerId,
  ...
  if (it == mLayerIdToHierarchy.end()) {
    LLOG_ALWAYS_FATAL_WITH_TRACE_IF(
        crashOnFailure, "Could not find hierarchy for layer id %d", layerId);
    return nullptr;
  };
...
void LayerHierarchyBuilder::logSampledChildren(
    const LayerHierarchy &hierarchy) const {
...
// Helper class to update a passed in TraversalPath when visiting a child. When
// the object goes out of scope the TraversalPath is reset to its original
// state.
LayerHierarchy::ScopedAddToTraversalPath::ScopedAddToTraversalPath(
    TraversalPath &traversalPath, uint32_t layerId,
    LayerHierarchy::Variant variant)
    : mTraversalPath(traversalPath), mParentPath(traversalPath) {
  // Update the traversal id with the child layer id and variant. Parent id and
  // variant are stored to reset the id upon destruction.
  traversalPath.id = layerId;
  traversalPath.variant = variant;
  if (LayerHierarchy::isMirror(variant)) {
    traversalPath.mirrorRootIds.emplace_back(mParentPath.id);
  } else if (variant == LayerHierarchy::Variant::Relative) {
    if (std::find(traversalPath.relativeRootIds.begin(),
                  traversalPath.relativeRootIds.end(),
...
LayerHierarchy::ScopedAddToTraversalPath::~ScopedAddToTraversalPath() {
  // Reset the traversal id to its original parent state using the state that

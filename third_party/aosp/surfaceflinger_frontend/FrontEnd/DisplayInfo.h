...
struct DisplayInfo {
  ...
  bool receivesInput;
  bool isSecure;
  // TODO(b/259407931): can eliminate once
  // SurfaceFlinger::sActiveDisplayRotationFlags is removed.
  bool isPrimary;
  bool isVirtual;
  ...
  std::string getDebugString() const {
    ...
    debug << "DisplayInfo {displayId=" << info.displayId
          << " lw=" << info.logicalWidth << " lh=" << info.logicalHeight
          << " transform={" << transform.dsdx() << " ," << transform.dsdy()
          << " ," << transform.dtdx() << " ," << transform.dtdy()

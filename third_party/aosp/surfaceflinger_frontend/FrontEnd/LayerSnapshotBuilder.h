...
public:
  ...
  // Update an existing set of snapshot using change flags in
  // RequestedLayerState and LayerLifecycleManager. This needs to be called
  // before LayerLifecycleManager.commitChanges is called as that function will
  ...
  // Visit each visible snapshot in z-order
  void forEachVisibleSnapshot(const ConstVisitor &visitor,
  ...
  // Visit each snapshot that satisfies the predicate and move the snapshot if
  // needed with visible snapshots in z-order
...
private:
  ...
  const LayerSnapshot &
  updateSnapshotsInHierarchy(const Args &, const LayerHierarchy &hierarchy,
                             LayerHierarchy::TraversalPath &traversalPath,
                             const LayerSnapshot &parentSnapshot, int depth);
  void updateSnapshot(LayerSnapshot &, const Args &,
                      const RequestedLayerState &,
                      const LayerSnapshot &parentSnapshot,
                      const LayerHierarchy::TraversalPath &);
  static void updateRelativeState(LayerSnapshot &snapshot,
                                  const LayerSnapshot &parentSnapshot,
                                  bool parentIsRelative, const Args &args);
  static void resetRelativeState(LayerSnapshot &snapshot);
  static void updateRoundedCorner(LayerSnapshot &snapshot,
                                  const RequestedLayerState &layerState,
                                  const LayerSnapshot &parentSnapshot,
                                  const Args &args);
  static bool
  extensionEdgeSharedWithParent(LayerSnapshot &snapshot,
                                const RequestedLayerState &requested,
                                const LayerSnapshot &parentSnapshot);
  static void updateBoundsForEdgeExtension(LayerSnapshot &snapshot);
  void updateLayerBounds(LayerSnapshot &snapshot,
                         const RequestedLayerState &layerState,
                         const LayerSnapshot &parentSnapshot,
                         uint32_t displayRotationFlags);
  static void updateShadows(LayerSnapshot &snapshot,
                            const RequestedLayerState &requested,
                            const ShadowSettings &globalShadowSettings);
  void updateInput(LayerSnapshot &snapshot,
                   const RequestedLayerState &requested,
                   const LayerSnapshot &parentSnapshot,
  ...
  LayerSnapshot *createSnapshot(const LayerHierarchy::TraversalPath &id,
                                ...
                                const LayerSnapshot &parentSnapshot);
  void updateFrameRateFromChildSnapshot(
  ...
  std::unordered_set<LayerHierarchy::TraversalPath,
                     LayerHierarchy::TraversalPathHash>

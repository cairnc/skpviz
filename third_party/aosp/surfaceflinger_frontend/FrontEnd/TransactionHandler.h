...
public:
  struct TransactionFlushState {
    ...
    // Layer handles that have transactions with buffers that are ready to be
    // applied.
    ftl::SmallMap<IBinder * /* binder address */, uint64_t /* framenumber */,
                  15>
        bufferLayersReadyToPresent = {};
    // Tracks the queue with an unsignaled buffer. This is used to handle
    // LatchUnsignaledConfig::AutoSingleLayer to ensure we only apply an
    // unsignaled buffer if it's the only transaction that is ready to be
    // applied.
  ...
  enum class TransactionReadiness {
    ...
    Ready,
    // Transaction has unmet conditions (fence, present time, etc) and cannot be
    // applied.
    NotReady,
    // Transaction is waiting on a barrier (another buffer to be latched first)
    NotReadyBarrier,
    // Transaction has an unsignaled fence but can be applied if it's the only
    // transaction
    NotReadyUnsignaled,
  };
  using TransactionFilter =
      std::function<TransactionReadiness(const TransactionFlushState &)>;
  ...
  void onTransactionQueueStalled(uint64_t transactionId,
                                 StalledTransactionInfo);
...
private:
  ...
  int flushPendingTransactionQueues(std::vector<TransactionState> &,
                                    TransactionFlushState &);
  void applyUnsignaledBufferTransaction(std::vector<TransactionState> &,
                                        TransactionFlushState &);
  void popTransactionFromPending(std::vector<TransactionState> &,
                                 TransactionFlushState &,
  ...
  std::unordered_map<uint64_t /* transactionId */, StalledTransactionInfo>
      mStalledTransactions GUARDED_BY(mStalledMutex);
};
} // namespace surfaceflinger::frontend
} // namespace android

...
// LayerHierarchy allows us to navigate the layer hierarchy in z-order, or depth
// first traversal. The hierarchy is created from a set of RequestedLayerStates.
// The hierarchy itself does not contain additional states. Instead, it is a
// representation of RequestedLayerStates as a graph.
//
// Each node in the hierarchy can be visited by multiple parents (making this a
// graph). While traversing the hierarchy, a new concept called Variant can be
// used to understand the relationship of the layer to its parent. The following
// variants are possible: Attached - child of the parent Detached - child of the
// parent but currently relative parented to another layer Relative - relative
// child of the parent Mirror - mirrored from another layer Detached_Mirror -
// mirrored from another layer, ignoring local transform
//
// By representing the hierarchy as a graph, we can represent mirrored layer
// hierarchies without cloning the layer requested state. The mirrored hierarchy
// and its corresponding RequestedLayerStates are kept in sync because the
// mirrored hierarchy does not clone any states.
class LayerHierarchy {
public:
  enum Variant : uint32_t {
    Attached, // child of the parent
    Detached, // child of the parent but currently relative parented to another
              // layer
  ...
  // The layer hierarchy is represented as a graph. Each node can be visited by
  // multiple parents. This allows us to represent mirroring in an efficient
  // way. See the example below: root ├─ A {Traversal path id = 1} ├─ B
  // {Traversal path id = 2} │  ├─ C {Traversal path id = 3} │  ├─ D {Traversal
  // path id = 4} │  └─ E (Mirrors C) {Traversal path id = 5} └─ F (Mirrors B)
  // {Traversal path id = 6}
  //
  // C can be traversed via B or E or F and or via F then E.
  // Depending on how the node is reached, its properties such as geometry or
  // visibility might be different. And we can uniquely identify the node by
  // keeping track of the nodes leading up to it. But to be more efficient we
  // only need to track the nodes id and the top mirror root path. So C for
  // example, would have the following unique traversal paths:
  //  - {Traversal path id = 3}
  ...
  struct TraversalPath {
    ...
    // Mirrored layers can have a different geometry than their parents so we
    // need to track the mirror roots in the traversal.
    ftl::SmallVector<uint32_t, 5> mirrorRootIds;
    // Relative layers can be visited twice, once by their parent and then once
    // again by their relative parent. We keep track of the roots here to detect
    // any loops in the hierarchy. If a relative root already exists in the list
    // while building the TraversalPath, it means that somewhere in the
    // hierarchy two layers are relatively parented to each other.
    ftl::SmallVector<uint32_t, 5> relativeRootIds;
    // First duplicate relative root id found. If this is a valid layer id that
    // means we are in a loop.
    uint32_t invalidRelativeRootId = UNASSIGNED_LAYER_ID;
    // See isAttached()
    bool detached = false;
    bool hasRelZLoop() const {
      return invalidRelativeRootId != UNASSIGNED_LAYER_ID;
    }
  ...
  // Visitor function that provides the hierarchy node and a traversal id which
  // uniquely identifies how was visited. The hierarchy contains a pointer to
  // the RequestedLayerState. Return false to stop traversing down the
  // hierarchy.
  ...
  // Traverse the hierarchy in z-order, skipping children that have relative
  // parents.
  ...
  friend std::ostream &operator<<(std::ostream &os, const LayerHierarchy &obj) {
    std::string prefix = " ";
    obj.dump(os, prefix, LayerHierarchy::Variant::Attached,
             /*isLastChild=*/false,
  ...
  // Traverse the hierarchy and return true if loops are found. The
  // outInvalidRelativeRoot will contain the first relative root that was
  // visited twice in a traversal.
...
private:
  ...
  void traverseInZOrder(const Visitor &visitor,
                        LayerHierarchy::TraversalPath &parent) const;
...
// Given a list of RequestedLayerState, this class will build a root hierarchy
// and an offscreen hierarchy. The builder also has an update method which can
// update an existing hierarchy from a list of RequestedLayerState and
// associated change flags.
class LayerHierarchyBuilder {
public:
...
private:
  ...
  LayerHierarchy *getHierarchyFromId(uint32_t layerId,

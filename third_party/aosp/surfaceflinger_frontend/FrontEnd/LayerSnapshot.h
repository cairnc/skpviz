...
// LayerSnapshot stores Layer state used by CompositionEngine and RenderEngine.
// Composition Engine uses a pointer to LayerSnapshot (as
// LayerFECompositionState*) and the LayerSettings passed to Render Engine are
// created using properties stored on this struct.
struct LayerSnapshot : public compositionengine::LayerFECompositionState {
  LayerSnapshot() = default;
  LayerSnapshot(const RequestedLayerState &,
                const LayerHierarchy::TraversalPath &);
  ...
  // Some consumers of this snapshot (input, layer traces) rely on each snapshot
  // to be unique. For mirrored layers, snapshots will have the same sequence so
  // this unique id provides an alternative identifier when needed.
  uint32_t uniqueSequence;
  // Layer id used to create this snapshot. Multiple snapshots will have the
  // same sequence if they generated from the same layer, for example when
  // mirroring.
  int32_t sequence;
  std::string name;
  ...
  bool hasReadyFrame; // used in post composition to check if there is another
                      // frame ready
  bool autoRefresh;
  ui::Transform localTransformInverse;
  gui::WindowInfo inputInfo;
  ui::Transform localTransform;
  // set to true if this snapshot will ignore local transforms. Used when the
  // snapshot is a mirror root
  ...
  enum class Reachablilty : uint32_t {
    ...
    Unreachable,
    // Can only reach this node from a relative parent. This means the nodes
    // parents are
    // not reachable.
    ...
    // │       └ * 12 (mirroring)
    // └── 2
    // 111 will create two snapshots, first when visited from 1 -> 12 or 1 -> 11
    // and the
    // second when visited from 1 -> 14 -> 12. Because its parent 11 doesn't
    // exist in the
    // mirrored hierarchy, the second snapshot will be marked as
    // ReachableByRelativeParent.
    // This snapshot doesn't have any valid properties because it cannot inherit
    // from its
    // parent. Therefore, snapshots that are not reachable will be ignored for
    // composition
    // and input.
    ReachableByRelativeParent
  };
  ...
  getBlendMode(const RequestedLayerState &requested) const;
  friend std::ostream &operator<<(std::ostream &os, const LayerSnapshot &obj);
  void merge(const RequestedLayerState &requested, bool forceUpdate,

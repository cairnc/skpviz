...
void updateSurfaceDamage(const RequestedLayerState &requested,
                         bool hasReadyFrame, bool forceFullDamage,
                         Region &outSurfaceDamageRegion) {
  if (!hasReadyFrame) {
...
std::ostream &operator<<(std::ostream &os, const ui::Transform &transform) {
  ...
  if (type & ui::Transform::SCALE) {
    std::string out;
    android::base::StringAppendF(&out, " scale x=%.4f y=%.4f ",
  ...
  if (type & ui::Transform::TRANSLATE) {
    std::string out;
    android::base::StringAppendF(&out, " tx=%.4f ty=%.4f ", transform.tx(),
                                 transform.ty());
    os << out;
  }
...
LayerSnapshot::LayerSnapshot(const RequestedLayerState &state,
  ...
  if (path.isClone()) {
    uniqueSequence = LayerCreationArgs::getInternalLayerId(
  ...
  clientChanges = 0;
  mirrorRootPath = LayerHierarchy::isMirror(path.variant)
                       ? path
...
bool LayerSnapshot::drawShadows() const { return shadowSettings.length > 0.f; }

bool LayerSnapshot::fillsColor() const {
  return !hasBufferOrSidebandStream() && color.r >= 0.0_hf &&
         color.g >= 0.0_hf && color.b >= 0.0_hf;
}
...
bool LayerSnapshot::isContentOpaque() const {
  // if we don't have a buffer or sidebandStream yet, we're translucent
  // regardless of the layer's opaque flag.
  ...
  if (hasBufferOrSidebandStream() &&
      isOpaqueFormat(externalTexture ? externalTexture->getPixelFormat()
                                     : PIXEL_FORMAT_NONE)) {
    return true;
  }
...
bool LayerSnapshot::isHiddenByPolicy() const {
  return invalidTransform || isHiddenByPolicyFromParent ||
...
std::string LayerSnapshot::getIsVisibleReason() const {
  ...
  if (isHiddenByPolicyFromParent)
    return "hidden by parent or layer flag";
  if (isHiddenByPolicyFromRelativeParent)
    return "hidden by relative parent";
  if (handleSkipScreenshotFlag & outputFilter.toInternalDisplay)
    return "eLayerSkipScreenshot";
  if (invalidTransform)
    return "invalidTransform";
  if (color.a == 0.0f && !hasBlur())
    return "alpha = 0 and no blur";
  if (!hasSomethingToDraw())
  ...
  if (sidebandStream != nullptr)
    reason << " sidebandStream";
  if (externalTexture != nullptr)
    reason << " buffer=" << externalTexture->getId()
           << " frame=" << frameNumber;
  if (fillsColor() || color.a > 0.0f)
    reason << " color{" << color << "}";
  if (drawShadows())
    reason << " shadowSettings.length=" << shadowSettings.length;
  if (backgroundBlurRadius > 0)
    reason << " backgroundBlurRadius=" << backgroundBlurRadius;
  if (blurRegions.size() > 0)
    reason << " blurRegions.size()=" << blurRegions.size();
  if (contentDirty)
    reason << " contentDirty";
  return reason.str();
}
...
bool LayerSnapshot::canReceiveInput() const {
  return !isHiddenByPolicy() &&
         (!hasBufferOrSidebandStream() || color.a > 0.0f);
}
...
std::string LayerSnapshot::getDebugString() const {
  ...
  debug << "Snapshot{" << path.toString() << name << " isVisible=" << isVisible
        ...
        << geomLayerBounds.left << "," << geomLayerBounds.top << ","
        << geomLayerBounds.bottom << "," << geomLayerBounds.right << "}"
  ...
  if (hasInputInfo()) {
    ...
    if (touchCropId != UNASSIGNED_LAYER_ID)
      debug << " touchCropId=" << touchCropId;
    if (inputInfo.replaceTouchableRegionWithCrop)
      debug << " replaceTouchableRegionWithCrop";
    auto touchableRegion = inputInfo.touchableRegion.getBounds();
    debug << " touchableRegion={" << touchableRegion.left << ","
          << touchableRegion.top << "," << touchableRegion.bottom << ","
          << touchableRegion.right << "}"
          << "}";
  }
  return debug.str();
}
...
std::ostream &operator<<(std::ostream &out, const LayerSnapshot &obj) {
  ...
  if (!obj.geomLayerBounds.isEmpty()) {
    out << "\n    bounds={" << obj.transformedBounds.left << ","
        << obj.transformedBounds.top << "," << obj.transformedBounds.bottom
        << "," << obj.transformedBounds.right << "}";
  }
  ...
  if (obj.hasInputInfo()) {
    ...
    if (obj.inputInfo.canOccludePresentation)
      out << " canOccludePresentation";
    if (obj.touchCropId != UNASSIGNED_LAYER_ID)
      out << " touchCropId=" << obj.touchCropId;
    if (obj.inputInfo.replaceTouchableRegionWithCrop)
      out << " replaceTouchableRegionWithCrop";
    auto touchableRegion = obj.inputInfo.touchableRegion.getBounds();
    out << " touchableRegion={" << touchableRegion.left << ","
        << touchableRegion.top << "," << touchableRegion.bottom << ","
        << touchableRegion.right << "}"
        << "}";
  }
...
LayerSnapshot::getBlendMode(const RequestedLayerState &requested) const {
...
void LayerSnapshot::merge(const RequestedLayerState &requested,
                          bool forceUpdate, bool displayChanges,
  ...
  updateSurfaceDamage(requested, requested.hasReadyFrame(), forceFullDamage,
                      surfaceDamage);

  if (forceUpdate ||
      requested.what & layer_state_t::eTransparentRegionChanged) {
    transparentRegionHint = requested.transparentRegion;
  }
  if (forceUpdate || requested.what & layer_state_t::eFlagsChanged) {
    layerOpaqueFlagSet = (requested.flags & layer_state_t::eLayerOpaque) ==
  ...
  if (forceUpdate ||
      requested.what & layer_state_t::eTransformToDisplayInverseChanged) {
  ...
  if (forceUpdate ||
      requested.what & layer_state_t::eExtendedRangeBrightnessChanged) {
    currentHdrSdrRatio = requested.currentHdrSdrRatio;
    desiredHdrSdrRatio = requested.desiredHdrSdrRatio;
  }
  if (forceUpdate ||
      requested.what & layer_state_t::eDesiredHdrHeadroomChanged) {
    desiredHdrSdrRatio = requested.desiredHdrSdrRatio;
  }
  ...
  if (forceUpdate ||
      requested.what & layer_state_t::eFrameRateSelectionPriority) {
    frameRateSelectionPriority = requested.frameRateSelectionPriority;
  }
  if (forceUpdate ||
      requested.what & layer_state_t::eColorSpaceAgnosticChanged) {
  ...
  if (forceUpdate ||
      requested.what & layer_state_t::ePictureProfileHandleChanged) {
    pictureProfileHandle = requested.pictureProfileHandle;
  }
  if (forceUpdate ||
      requested.what & layer_state_t::eAppContentPriorityChanged) {
    // TODO(b/337330263): Also consider the system-determined priority of the
    // app
    pictureProfilePriority = requested.appContentPriority;
  }

  if (forceUpdate ||
      requested.what & layer_state_t::eDefaultFrameRateCompatibilityChanged) {
    const auto compatibility = Layer::FrameRate::convertCompatibility(
        requested.defaultFrameRateCompatibility);
  ...
  if (forceUpdate || requested.what & layer_state_t::eInputInfoChanged) {
    if (requested.windowInfoHandle) {
      ...
      inputInfo = {};
      // b/271132344 revisit this and see if we can always use the layers
      // uid/pid
  ...
  if (forceUpdate || requested.what & layer_state_t::eBufferChanged) {
    acquireFence = (requested.externalTexture &&
                    requested.bufferData->flags.test(
                        BufferData::BufferDataChange::fenceChanged))
                       ? requested.bufferData->acquireFence
                       : Fence::NO_FENCE;
    buffer = requested.externalTexture ? requested.externalTexture->getBuffer()
                                       : nullptr;
    externalTexture = requested.externalTexture;
    frameNumber =
        (requested.bufferData) ? requested.bufferData->frameNumber : 0;
    hasProtectedContent =
  ...
  if (forceUpdate ||
    ...
    forceClientComposition = shadowSettings.length > 0 ||
  ...
  if (forceUpdate ||
    ...
    isOpaque =
        contentOpaque && !roundedCorner.hasRoundedCorners() && color.a == 1.f;

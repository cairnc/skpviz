...
#define LLOGV(LAYER_ID, x, ...)                                                \
  ALOGV_IF(((LAYER_ID) == LOG_LAYER), "[%d] %s " x, LOG_LAYER, __func__,       \
           ##__VA_ARGS__);
#else
#define LLOGV(LAYER_ID, x, ...)                                                \
  ALOGV("[%d] %s " x, (LAYER_ID), __func__, ##__VA_ARGS__);
#endif

#define LLOGD(LAYER_ID, x, ...)                                                \
  ALOGD("[%d] %s " x, (LAYER_ID), __func__, ##__VA_ARGS__);

#define LLOG_ALWAYS_FATAL_WITH_TRACE(...)                                      \
  do {                                                                         \
    TransactionTraceWriter::getInstance().invoke(__func__,                     \
                                                 /* overwrite= */ false);      \

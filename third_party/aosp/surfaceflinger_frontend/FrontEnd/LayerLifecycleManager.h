...
// the RequestedLayerState but a layer id associated with the
// RequestedLayerState. If the handle is destroyed and the RequestedLayerState
// does not have a parent, the LayerLifecycleManager destroys the
// RequestedLayerState.
//
// Threading: This class is not thread safe, it requires external
// synchronization.
//
// Typical usage: Input states (new layers, transactions, destroyed layer
// handles) are collected in the background passed into the
// LayerLifecycleManager to update layer lifecycle and layer state at start of
// composition.
...
public:
  ...
  // Ignore unknown layers when interoping with legacy front end. In legacy we
  // destroy the layers it is unreachable. When using the LayerLifecycleManager
  // for layer trace generation we may encounter layers which are known because
  // we don't have an explicit lifecycle. Ignore these errors while we have to
  // interop with legacy.
  void applyTransactions(const std::vector<TransactionState> &,
                         bool ignoreUnknownLayers = false);
  // Ignore unknown handles when iteroping with legacy front end. In the old
  // world, we would create child layers which are not necessary with the new
  // front end. This means we will get notified for handle changes that don't
  // exist in the new front end.
  ...
  // ILifecycleListener callbacks and clears any change flags from previous
  // state updates. This function should be called outside the hot path since
  // it's not critical to composition.
  void commitChanges();
...
private:
  ...
  std::vector<uint32_t> unlinkLayers(const std::vector<uint32_t> &layerIds,
  ...
  // Layers pending destruction. Layers will be destroyed once changes are
  // committed.
  std::vector<std::unique_ptr<RequestedLayerState>> mDestroyedLayers;
  // Keeps track of all the layers that were added in order. Changes will be

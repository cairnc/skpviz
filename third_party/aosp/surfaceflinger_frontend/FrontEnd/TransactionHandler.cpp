...
void TransactionHandler::collectTransactions() {
  while (!mLocklessTransactionQueue.isEmpty()) {
    ...
    mPendingTransactionQueues[transaction.applyToken].emplace(
...
std::vector<TransactionState> TransactionHandler::flushTransactions() {
  ...
  // Transactions with a buffer pending on a barrier may be on a different
  // applyToken than the transaction which satisfies our barrier. In fact this
  // is the exact use case that the primitive is designed for. This means we may
  // first process the barrier dependent transaction, determine it ineligible to
  // complete and then satisfy in a later inner iteration of
  // flushPendingTransactionQueues. The barrier dependent transaction was
  // eligible to be presented in this frame but we would have prevented it
  // without case. To fix this we continually loop through
  // flushPendingTransactionQueues until we perform an iteration where the
  // number of transactionsPendingBarrier doesn't change. This way we can
  // continue to resolve dependency chains of barriers as far as possible.
  ...
  do {
    ...
    transactionsPendingBarrier =
        flushPendingTransactionQueues(transactions, flushState);
...
void TransactionHandler::applyUnsignaledBufferTransaction(
  ...
  auto it =
      mPendingTransactionQueues.find(flushState.queueWithUnsignaledBuffer);
  LLOG_ALWAYS_FATAL_WITH_TRACE_IF(
      it == mPendingTransactionQueues.end(),
...
void TransactionHandler::popTransactionFromPending(
    std::vector<TransactionState> &transactions,
    TransactionFlushState &flushState, std::queue<TransactionState> &queue) {
      ...
      [&](const layer_state_t &state) {
        ...
        if (frameNumberChanged) {
          flushState.bufferLayersReadyToPresent.emplace_or_replace(
              state.surface.get(), state.bufferData->frameNumber);
        } else {
          // Barrier function only used for BBQ which always includes a frame
          // number. This value only used for barrier logic.
          flushState.bufferLayersReadyToPresent.emplace_or_replace(
              state.surface.get(), std::numeric_limits<uint64_t>::max());
        }
      });
}

TransactionHandler::TransactionReadiness
TransactionHandler::applyFilters(TransactionFlushState &flushState) {
  ...
  for (auto &filter : mTransactionReadyFilters) {
    ...
    case TransactionReadiness::NotReadyUnsignaled:
      // If one of the filters allows latching an unsignaled buffer, latch this
      // ready state.
...
bool TransactionHandler::hasPendingTransactions() {
  return !mPendingTransactionQueues.empty() ||
         !mLocklessTransactionQueue.isEmpty();
}

void TransactionHandler::onTransactionQueueStalled(
    uint64_t transactionId, StalledTransactionInfo stalledTransactionInfo) {
  std::lock_guard lock{mStalledMutex};
  mStalledTransactions.emplace(transactionId,
                               std::move(stalledTransactionInfo));
}
...
void TransactionHandler::onLayerDestroyed(uint32_t layerId) {
  std::lock_guard lock{mStalledMutex};
  for (auto it = mStalledTransactions.begin();
       it != mStalledTransactions.end();) {

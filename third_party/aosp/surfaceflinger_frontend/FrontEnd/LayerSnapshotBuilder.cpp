...
FloatRect getMaxDisplayBounds(const DisplayInfos &displays) {
  const ui::Size maxSize = [&displays] {
    if (displays.empty())
      return ui::Size{5000, 5000};

    return std::accumulate(
        displays.begin(), displays.end(), ui::kEmptySize,
  ...
  // Ignore display bounds for now since they will be computed later. Use a
  // large Rect bound to ensure it's bigger than an actual display will be.
...
// Applies the given transform to the region, while protecting against overflows
// caused by any offsets. If applying the offset in the transform to any of the
// Rects in the region would result in an overflow, they are not added to the
// output Region.
Region transformTouchableRegionSafely(const ui::Transform &t, const Region &r,
                                      const std::string &debugWindowName) {
  // Round the translation using the same rounding strategy used by
  // ui::Transform.
  ...
  // Apply the translation to each of the Rects in the region while discarding
  // any that overflow.
  Region ret;
  for (const auto &rect : transformed) {
    ...
    if (__builtin_add_overflow(rect.left, tx, &newRect.left) ||
      ...
      ALOGE("Applying transform to touchable region of window '%s' resulted in "
            "an overflow.",
 ...
 *
 * Similar to getInputTransform, we need to update the bounds to include the
 * transform. This is because bounds don't include the buffer transform, where
 * the input assumes that's already included.
 */
std::pair<FloatRect, bool> getInputBounds(const LayerSnapshot &snapshot,
                                          bool fillParentBounds) {
  ...
  if (!inputBoundsValid) {
    /**
     * Input bounds are based on the layer crop or buffer size. But if we are
     * using the layer bounds as the input bounds
     * (replaceTouchableRegionWithCrop flag) then we can use the parent bounds
     * as the input bounds if the layer does not have buffer or a crop. We want
     * to unify this logic but because of compat reasons we cannot always use
     * the parent bounds. A layer without a buffer can get input. So when a
     * window is initially added, its touchable region can fill its parent layer
     * bounds and that can have negative consequences.
     */
  ...
  const float xSurfaceInset =
      std::clamp(inset, 0.f, inputBounds.getWidth() / 2.f);
  const float ySurfaceInset =
      std::clamp(inset, 0.f, inputBounds.getHeight() / 2.f);
...
Rect getInputBoundsInDisplaySpace(const LayerSnapshot &snapshot,
                                  const FloatRect &insetBounds,
                                  const ui::Transform &screenToDisplay) {
  // InputDispatcher works in the display device's coordinate space. Here, we
  // calculate the frame and transform used for the layer, which determines the
  // bounds and the coordinate space within which the layer will receive input.

  // Coordinate space definitions:
  //   - display: The display device's coordinate space. Correlates to pixels on
  //   the display.
  //   - screen: The post-rotation coordinate space for the display, a.k.a.
  //   logical display space.
  //   - layer: The coordinate space of this layer.
  //   - input: The coordinate space in which this layer will receive input
  //   events. This could be
  //            different than layer space if a surfaceInset is used, which
  //            changes the origin of the input space.

  // Crop the input bounds to ensure it is within the parent's bounds.
  const FloatRect croppedInsetBoundsInLayer =
      snapshot.geomLayerBounds.intersect(insetBounds);
...
void fillInputFrameInfo(gui::WindowInfo &info,
                        const ui::Transform &screenToDisplay,
                        const LayerSnapshot &snapshot) {
  auto [inputBounds, inputBoundsValid] =
      getInputBounds(snapshot, /*fillParentBounds=*/false);
  if (!inputBoundsValid) {
  ...
  const ui::Transform layerToScreen = getInputTransform(snapshot);
  const ui::Transform inputToDisplay =
      screenToDisplay * layerToScreen * inputToLayer;

  // InputDispatcher expects a display-to-input transform.
  info.transform = inputToDisplay.inverse();

  // The touchable region is specified in the input coordinate space. Change it
  // to display space.
  info.touchableRegion = transformTouchableRegionSafely(
      inputToDisplay, info.touchableRegion, snapshot.name);
}

void handleDropInputMode(LayerSnapshot &snapshot,
                         const LayerSnapshot &parentSnapshot) {
  if (snapshot.inputInfo.inputConfig.test(
  ...
  if (dropInputMode == gui::DropInputMode::ALL) {
    ...
    ALOGV("Dropping input for %s as requested by policy.",
          snapshot.name.c_str());
    return;
  }
  ...
  if (!bufferSize.isValid()) {
    snapshot.inputInfo.inputConfig |=
        gui::WindowInfo::InputConfig::DROP_INPUT_IF_OBSCURED;
    return;
  }

  // Screenbounds are the layer bounds cropped by parents, transformed to
  // screenspace. To check if the layer has been cropped, we take the buffer
  // bounds, apply the local layer crop and apply the same set of transforms to
  // move to screenspace. If the bounds match then the layer has not been
  // cropped by its parents.
  Rect bufferInScreenSpace(snapshot.geomLayerTransform.transform(bufferSize));
  bool croppedByParent =
      bufferInScreenSpace != Rect{snapshot.transformedBounds};

  if (croppedByParent) {
    snapshot.inputInfo.inputConfig |= gui::WindowInfo::InputConfig::DROP_INPUT;
    ALOGV("Dropping input for %s as requested by policy because buffer is "
          "cropped by parent",
          snapshot.name.c_str());
  } else {
    // If the layer is not obscured by its parents (by setting an alpha or
    // crop), then only drop input if the window is obscured. This check should
    // be done in surfaceflinger but the logic currently resides in
    // inputflinger. So pass the if_obscured check to input to only drop input
    // events if the window is obscured.
    snapshot.inputInfo.inputConfig |=
        gui::WindowInfo::InputConfig::DROP_INPUT_IF_OBSCURED;
  }
}

auto getBlendMode(const LayerSnapshot &snapshot,
                  const RequestedLayerState &requested) {
  auto blendMode = Hwc2::IComposerClient::BlendMode::NONE;
  if (snapshot.alpha != 1.0f || !snapshot.isContentOpaque()) {
    blendMode = requested.premultipliedAlpha
...
void updateVisibility(LayerSnapshot &snapshot, bool visible) {
  ...
  if (FlagManager::getInstance().skip_invisible_windows_in_input()) {
    ...
    // anything.
    const bool visibleForInput = snapshot.hasInputInfo()
                                     ? snapshot.canReceiveInput()
                                     : snapshot.isVisible;
    snapshot.inputInfo.setInputConfig(gui::WindowInfo::InputConfig::NOT_VISIBLE,
                                      !visibleForInput);
  }
  LLOGV(snapshot.sequence, "updating visibility %s %s",
        visible ? "true" : "false", snapshot.getDebugString().c_str());
}

void updateMetadata(LayerSnapshot &snapshot,
                    const RequestedLayerState &requested,
  ...
  for (const auto &[key, mandatory] : args.supportedLayerGenericMetadata) {
    ...
    snapshot.metadata.emplace(key, compositionengine::GenericLayerMetadataEntry{
                                       mandatory, it->second});
  }
}

void updateMetadataAndGameMode(LayerSnapshot &snapshot,
                               const RequestedLayerState &requested,
                               const LayerSnapshotBuilder::Args &args,
                               const LayerSnapshot &parentSnapshot) {
...
LayerSnapshot LayerSnapshotBuilder::getRootSnapshot() {
  ...
  snapshot.inputInfo.touchOcclusionMode =
...
bool LayerSnapshotBuilder::tryFastUpdate(const Args &args) {
  ...
  if (args.layerLifecycleManager.getGlobalChanges().get() == 0 &&
      !forceUpdate && !args.displayChanges) {
    return true;
  }

  // There are only content changes which do not require any child layer
  // snapshots to be updated.
  ALOGV("%s", __func__);
  SFTRACE_NAME("FastPath");

  uint32_t primaryDisplayRotationFlags =
      getPrimaryDisplayRotationFlags(args.displays);
  if (forceUpdate || args.displayChanges) {
    for (auto &snapshot : mSnapshots) {
      ...
      if (!requested)
        continue;
      snapshot->merge(*requested, forceUpdate, args.displayChanges,
                      args.forceFullDamage, primaryDisplayRotationFlags);
    }
    return false;
  }

  // Walk through all the updated requested layer states and update the
  // corresponding snapshots.
  for (const RequestedLayerState *requested :
       args.layerLifecycleManager.getChangedLayers()) {
    auto range = mIdToSnapshots.equal_range(requested->id);
    for (auto it = range.first; it != range.second; it++) {
      it->second->merge(*requested, forceUpdate, args.displayChanges,
                        args.forceFullDamage, primaryDisplayRotationFlags);
    }
  }
  ...
  if ((args.layerLifecycleManager.getGlobalChanges().get() &
       ~(RequestedLayerState::Changes::Content |
         RequestedLayerState::Changes::Buffer)
            .get()) != 0) {
    // We have changes that require us to walk the hierarchy and update child
    // layers. No fast path for you.
    return false;
  }
  return true;
}
...
void LayerSnapshotBuilder::updateSnapshots(const Args &args) {
  ...
  if (args.forceUpdate == ForceUpdateFlags::HIERARCHY) {
    rootSnapshot.changes |= RequestedLayerState::Changes::Hierarchy |
  ...
  if (args.root.getLayer()) {
    // The hierarchy can have a root layer when used for screenshots otherwise,
    // it will have multiple children.
    LayerHierarchy::ScopedAddToTraversalPath addChildToPath(
        root, args.root.getLayer()->id, LayerHierarchy::Variant::Attached);
    updateSnapshotsInHierarchy(args, args.root, root, rootSnapshot,
                               /*depth=*/0);
  } else {
    for (auto &[childHierarchy, variant] : args.root.mChildren) {
      LayerHierarchy::ScopedAddToTraversalPath addChildToPath(
          root, childHierarchy->getLayer()->id, variant);
      updateSnapshotsInHierarchy(args, *childHierarchy, root, rootSnapshot,
                                 /*depth=*/0);
    }
  }

  // Update touchable region crops outside the main update pass. This is because
  // a layer could be cropped by any other layer and it requires both snapshots
  // to be updated.
  updateTouchableRegionCrop(args);

  const bool hasUnreachableSnapshots = sortSnapshotsByZ(args);

  // Destroy unreachable snapshots for clone layers. And destroy snapshots for
  // non-clone layers if the layer have been destroyed.
  // TODO(b/238781169) consider making clone layer ids stable as well
  if (!hasUnreachableSnapshots &&
      args.layerLifecycleManager.getDestroyedLayers().empty()) {
    return;
  }
  ...
  while (it < mSnapshots.end()) {
    ...
    auto matchingSnapshot = std::find_if(
        range.first, range.second, [&traversalPath](auto &snapshotWithId) {
...
const LayerSnapshot &LayerSnapshotBuilder::updateSnapshotsInHierarchy(
  ...
  if (newSnapshot) {
    ...
    snapshot->merge(*layer, /*forceUpdate=*/true, /*displayChanges=*/true,
                    args.forceFullDamage, primaryDisplayRotationFlags);
    snapshot->changes |= RequestedLayerState::Changes::Created;
  }

  if (traversalPath.isRelative()) {
    bool parentIsRelative =
        traversalPath.variant == LayerHierarchy::Variant::Relative;
  ...
  for (auto &[childHierarchy, variant] : hierarchy.mChildren) {
    LayerHierarchy::ScopedAddToTraversalPath addChildToPath(
        traversalPath, childHierarchy->getLayer()->id, variant);
    const LayerSnapshot &childSnapshot = updateSnapshotsInHierarchy(
        args, *childHierarchy, traversalPath, *snapshot, depth + 1);
    updateFrameRateFromChildSnapshot(*snapshot, childSnapshot,
                                     *childHierarchy->getLayer(), args,
                                     &childHasValidFrameRate);
  }
...
LayerSnapshot *LayerSnapshotBuilder::getSnapshot(
    const LayerHierarchy::TraversalPath &id) const {
  auto it = mPathToSnapshot.find(id);
  return it == mPathToSnapshot.end() ? nullptr : it->second;
}

LayerSnapshot *
LayerSnapshotBuilder::createSnapshot(const LayerHierarchy::TraversalPath &path,
  ...
  snapshot->ignoreLocalTransform =
      path.isClone() &&
      path.variant == LayerHierarchy::Variant::Detached_Mirror;
...
bool LayerSnapshotBuilder::sortSnapshotsByZ(const Args &args) {
  if (!mResortSnapshots && args.forceUpdate == ForceUpdateFlags::NONE &&
    ...
    // We are not force updating and there are no hierarchy or visibility
    // changes. Avoid sorting the snapshots.
    return false;
  }
  ...
  while (globalZ < mSnapshots.size()) {
    ...
    if (mSnapshots[globalZ]->reachablilty ==
        LayerSnapshot::Reachablilty::Unreachable) {
...
void LayerSnapshotBuilder::updateRelativeState(
    LayerSnapshot &snapshot, const LayerSnapshot &parentSnapshot,
    bool parentIsRelative, const Args &args) {
  if (parentIsRelative) {
    snapshot.isHiddenByPolicyFromRelativeParent =
        parentSnapshot.isHiddenByPolicyFromParent ||
        parentSnapshot.invalidTransform;
    if (args.includeMetadata) {
  ...
  if (snapshot.reachablilty == LayerSnapshot::Reachablilty::Unreachable) {
    snapshot.reachablilty =
        LayerSnapshot::Reachablilty::ReachableByRelativeParent;
  }
}
...
void LayerSnapshotBuilder::updateFrameRateFromChildSnapshot(
  ...
  // We return whether this layer or its children has a vote. We ignore
  // ExactOrMultiple votes for the same reason we are allowing touch boost for
  // those layers. See RefreshRateSelector::rankFrameRates for details.
  const auto layerVotedWithDefaultCompatibility =
      childSnapshot.frameRate.vote.rate.isValid() &&
  ...
  *outChildHasValidFrameRate |=
      layerVotedWithDefaultCompatibility || layerVotedWithNoVote ||
...
void LayerSnapshotBuilder::updateSnapshot(
    LayerSnapshot &snapshot, const Args &args,
    const RequestedLayerState &requested, const LayerSnapshot &parentSnapshot,
    const LayerHierarchy::TraversalPath &path) {
  // Always update flags and visibility
  ftl::Flags<RequestedLayerState::Changes> parentChanges =
      parentSnapshot.changes & (RequestedLayerState::Changes::Hierarchy |
                                RequestedLayerState::Changes::Geometry |
                                RequestedLayerState::Changes::Visibility |
                                RequestedLayerState::Changes::Metadata |
                                RequestedLayerState::Changes::AffectsChildren |
                                RequestedLayerState::Changes::Input |
                                RequestedLayerState::Changes::FrameRate |
                                RequestedLayerState::Changes::GameMode);
  ...
  snapshot.clientChanges |=
      (parentSnapshot.clientChanges & layer_state_t::AFFECTS_CHILDREN);
  // mark the content as dirty if the parent state changes can dirty the child's
  // content (for example alpha)
  snapshot.contentDirty |=
      (snapshot.clientChanges & layer_state_t::CONTENT_DIRTY) != 0;
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eLayerStackChanged) {
    // If root layer, use the layer stack otherwise get the parent's layer
    // stack.
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eTrustedOverlayChanged) {
    ...
    case gui::TrustedOverlay::DISABLED:
      snapshot.trustedOverlay =
          FlagManager::getInstance().override_trusted_overlay()
  ...
  if (forceUpdate ||
      snapshot.changes.any(RequestedLayerState::Changes::Mirror)) {
    // Display mirrors are always placed in a VirtualDisplay so we never want to
    // capture layers marked as skip capture
    snapshot.handleSkipScreenshotFlag =
        parentSnapshot.handleSkipScreenshotFlag ||
  ...
  if (forceUpdate || snapshot.clientChanges & layer_state_t::eFlagsChanged) {
    snapshot.isSecure = parentSnapshot.isSecure ||
                        (requested.flags & layer_state_t::eLayerSecure);
  ...
  if (forceUpdate || (snapshot.clientChanges | parentSnapshot.clientChanges) &
    ...
    if (requested.edgeExtensionParameters.extendLeft ||
      ...
      snapshot.edgeExtensionEffect =
          EdgeExtensionEffect(requested.edgeExtensionParameters.extendLeft,
                              ...
                              requested.edgeExtensionParameters.extendTop,
                              requested.edgeExtensionParameters.extendBottom);
    } else if (parentSnapshot.clientChanges &
               layer_state_t::eEdgeExtensionChanged) {
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eColorTransformChanged) {
    if (!parentSnapshot.colorTransformIsIdentity) {
      snapshot.colorTransform =
          parentSnapshot.colorTransform * requested.colorTransform;
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eFixedTransformHintChanged ||
      args.displayChanges) {
    snapshot.fixedTransformHint =
        requested.fixedTransformHint != ui::Transform::ROT_INVALID
  ...
  if (forceUpdate ||
    ...
    // Set the framerate as the inherited frame rate and allow children to
    // override it if needed.
    snapshot.frameRate = snapshot.inheritedFrameRate;
    snapshot.changes |= RequestedLayerState::Changes::FrameRate;
  }

  if (forceUpdate || snapshot.clientChanges &
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eFrameRateSelectionPriority) {
  ...
  if (forceUpdate ||
      snapshot.clientChanges &
          (layer_state_t::eBackgroundBlurRadiusChanged |
           layer_state_t::eBlurRegionsChanged | layer_state_t::eAlphaChanged)) {
    snapshot.backgroundBlurRadius =
        args.supportsBlur
            ? static_cast<int>(parentSnapshot.color.a *
                               (float)requested.backgroundBlurRadius)
            : 0;
  ...
  if (forceUpdate ||
    ...
    uint32_t primaryDisplayRotationFlags =
        getPrimaryDisplayRotationFlags(args.displays);
    updateLayerBounds(snapshot, requested, parentSnapshot,
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eCornerRadiusChanged ||
      snapshot.changes.any(RequestedLayerState::Changes::Geometry |
  ...
  if (forceUpdate ||
      snapshot.clientChanges & layer_state_t::eShadowRadiusChanged ||
      snapshot.changes.any(RequestedLayerState::Changes::Geometry)) {
  ...
  snapshot.forceClientComposition = snapshot.shadowSettings.length > 0 ||
                                    ...
                                    snapshot.edgeExtensionEffect.hasEffect();
  snapshot.contentOpaque = snapshot.isContentOpaque();
  snapshot.isOpaque = snapshot.contentOpaque &&
                      !snapshot.roundedCorner.hasRoundedCorners() &&
                      snapshot.color.a == 1.f;
  snapshot.blendMode = getBlendMode(snapshot, requested);
  LLOGV(snapshot.sequence,
        "%supdated %s changes:%s parent:%s requested:%s requested:%s from "
        "parent %s",
        args.forceUpdate == ForceUpdateFlags::ALL ? "Force " : "",
        snapshot.getDebugString().c_str(), snapshot.changes.string().c_str(),
        parentSnapshot.changes.string().c_str(),
        requested.changes.string().c_str(),
        std::to_string(requested.what).c_str(),
        parentSnapshot.getDebugString().c_str());
}

void LayerSnapshotBuilder::updateRoundedCorner(
  ...
  const bool layerSettingsValid =
      layerSettings.hasRoundedCorners() && !layerCropRect.isEmpty();
  const bool parentRoundedCornerValid = parentRoundedCorner.hasRoundedCorners();
  if (layerSettingsValid && parentRoundedCornerValid) {
    // If the parent and the layer have rounded corner settings, use the parent
    // settings if the parent crop is entirely inside the layer crop. This has
    // limitations and cause rendering artifacts. See b/200300845 for correct
    // fix.
...
/**
 * According to the edges that we are requested to extend, we increase the
 * bounds to the maximum extension allowed by the crop (parent crop + requested
 * crop). The animation that called Transition#setEdgeExtensionEffect is in
 * charge of setting the requested crop.
 * @param snapshot
 */
void LayerSnapshotBuilder::updateBoundsForEdgeExtension(
    LayerSnapshot &snapshot) {
  ...
  snapshot.transformedBounds =
      snapshot.geomLayerTransform.transform(snapshot.geomLayerBounds);
}

void LayerSnapshotBuilder::updateLayerBounds(
    LayerSnapshot &snapshot, const RequestedLayerState &requested,
    const LayerSnapshot &parentSnapshot, uint32_t primaryDisplayRotationFlags) {
  snapshot.geomLayerTransform =
      parentSnapshot.geomLayerTransform * snapshot.localTransform;
  const bool transformWasInvalid = snapshot.invalidTransform;
  snapshot.invalidTransform =
      !LayerSnapshot::isTransformValid(snapshot.geomLayerTransform);
  if (snapshot.invalidTransform) {
    auto &t = snapshot.geomLayerTransform;
    auto &requestedT = requested.requestedTransform;
    std::string transformDebug = base::StringPrintf(
        " transform={%f,%f,%f,%f}  requestedTransform={%f,%f,%f,%f}", t.dsdx(),
        t.dsdy(), t.dtdx(), t.dtdy(), requestedT.dsdx(), requestedT.dsdy(),
        requestedT.dtdx(), requestedT.dtdy());
    std::string bufferDebug;
    if (requested.externalTexture) {
      auto unRotBuffer =
          requested.getUnrotatedBufferSize(primaryDisplayRotationFlags);
      auto &destFrame = requested.destinationFrame;
      bufferDebug = base::StringPrintf(
          " buffer={%d,%d}  displayRot=%d"
          " destFrame={%d,%d,%d,%d} unRotBuffer={%d,%d}",
          requested.externalTexture->getWidth(),
          requested.externalTexture->getHeight(), primaryDisplayRotationFlags,
    ...
    ALOGW("Resetting transform for %s because it is invalid.%s%s",
          snapshot.getDebugString().c_str(), transformDebug.c_str(),
          bufferDebug.c_str());
    snapshot.geomLayerTransform.reset();
  }
  ...
  snapshot.geomLayerBounds = requested.externalTexture
  ...
  snapshot.geomLayerBounds =
      snapshot.geomLayerBounds.intersect(snapshot.geomLayerCrop);
  snapshot.transformedBounds =
      snapshot.geomLayerTransform.transform(snapshot.geomLayerBounds);
  ...
  snapshot.transformedBoundsWithoutTransparentRegion =
      snapshot.geomLayerTransform.transform(
          geomLayerBoundsWithoutTransparentRegion);
  snapshot.parentTransform = parentSnapshot.geomLayerTransform;

  if (requested.potentialCursor) {
    // Subtract the transparent region and snap to the bounds
    const Rect bounds = RequestedLayerState::reduce(
        Rect(snapshot.croppedBufferSize), requested.transparentRegion);
    snapshot.cursorFrame = snapshot.geomLayerTransform.transform(bounds);
  }
}

void LayerSnapshotBuilder::updateShadows(
    LayerSnapshot &snapshot, const RequestedLayerState &,
  ...
  if (snapshot.shadowSettings.length > 0.f) {
    ...
    // Note: this preserves existing behavior of shadowing the entire layer and
    // not cropping it if transparent regions are present. This may not be
    // necessary since shadows are typically cast by layers without transparent
    // regions.
    snapshot.shadowSettings.boundaries = snapshot.geomLayerBounds;

    // If the casting layer is translucent, we need to fill in the shadow
    // underneath the layer. Otherwise the generated shadow will only be shown
    // around the casting layer.
...
void LayerSnapshotBuilder::updateInput(
    LayerSnapshot &snapshot, const RequestedLayerState &requested,
    const LayerSnapshot &parentSnapshot,
  ...
  snapshot.inputInfo.displayId = ui::LogicalDisplayId{
      static_cast<int32_t>(snapshot.outputFilter.layerStack.id)};
  snapshot.inputInfo.touchOcclusionMode =
      requested.hasInputInfo()
          ? requested.windowInfoHandle->getInfo()->touchOcclusionMode
          : parentSnapshot.inputInfo.touchOcclusionMode;
  snapshot.inputInfo.canOccludePresentation =
      parentSnapshot.inputInfo.canOccludePresentation ||
  ...
  if (snapshot.isSecure || parentSnapshot.inputInfo.inputConfig.test(
  ...
  if (noValidDisplay) {
    // Do not let the window receive touches if it is not associated with a
    // valid display transform. We still allow the window to receive keys and
    // prevent ANRs.
  ...
  // If the window will be blacked out on a display because the display does not
  // have the secure flag and the layer has the secure flag set, then drop
  // input.
  ...
  if (!cropLayerSnapshot && snapshot.inputInfo.replaceTouchableRegionWithCrop) {
    FloatRect inputBounds =
        getInputBounds(snapshot, /*fillParentBounds=*/true).first;
    Rect inputBoundsInDisplaySpace = getInputBoundsInDisplaySpace(
        snapshot, inputBounds, displayInfo.transform);
    snapshot.inputInfo.touchableRegion = Region(inputBoundsInDisplaySpace);
  }

  // Inherit the trusted state from the parent hierarchy, but don't clobber the
  // trusted state if it was set by WM for a known system overlay
  ...
  // If the layer is a clone, we need to crop the input region to cloned root to
  // prevent touches from going outside the cloned area.
  if (path.isClone()) {
    snapshot.inputInfo.inputConfig |= InputConfig::CLONE;
    // Cloned layers shouldn't handle watch outside since their z order is not
    // determined by WM or the client.
    snapshot.inputInfo.inputConfig.clear(InputConfig::WATCH_OUTSIDE_TOUCH);
  }
}

std::vector<std::unique_ptr<LayerSnapshot>> &
LayerSnapshotBuilder::getSnapshots() {
  return mSnapshots;
}

void LayerSnapshotBuilder::forEachVisibleSnapshot(
    const ConstVisitor &visitor) const {
...
void LayerSnapshotBuilder::forEachVisibleSnapshot(
    const ConstVisitor &visitor, const LayerHierarchy &root) const {
  root.traverseInZOrder(
      [this,
       visitor](const LayerHierarchy &,
                const LayerHierarchy::TraversalPath &traversalPath) -> bool {
        LayerSnapshot *snapshot = getSnapshot(traversalPath);
...
void LayerSnapshotBuilder::forEachInputSnapshot(
    const ConstVisitor &visitor) const {
  for (int i = mNumInterestingSnapshots - 1; i >= 0; i--) {
    LayerSnapshot &snapshot = *mSnapshots[(size_t)i];
    if (!snapshot.hasInputInfo())
      continue;
    if (FlagManager::getInstance().skip_invisible_windows_in_input() &&
        snapshot.inputInfo.inputConfig.test(
...
void LayerSnapshotBuilder::updateTouchableRegionCrop(const Args &args) {
  ...
  for (auto &path : mNeedsTouchableRegionCrop) {
    ...
    bool needsUpdate = args.forceUpdate == ForceUpdateFlags::ALL ||
                       snapshot->changes.any(AFFECTS_INPUT);
    auto cropLayerSnapshot = getSnapshot(snapshot->touchCropId);
    needsUpdate =
        needsUpdate ||
        (cropLayerSnapshot && cropLayerSnapshot->changes.any(AFFECTS_INPUT));
    auto clonedRootSnapshot =
        path.isClone() ? getSnapshot(snapshot->mirrorRootPath) : nullptr;
    needsUpdate =
        needsUpdate ||
    ...
    if (snapshot->inputInfo.replaceTouchableRegionWithCrop) {
      ...
      if (!cropLayerSnapshot) {
        FloatRect inputBounds =
            getInputBounds(*snapshot, /*fillParentBounds=*/true).first;
        inputBoundsInDisplaySpace = getInputBoundsInDisplaySpace(
            *snapshot, inputBounds, displayInfo.transform);
      } else {
        FloatRect inputBounds =
            getInputBounds(*cropLayerSnapshot, /*fillParentBounds=*/true).first;
        inputBoundsInDisplaySpace = getInputBoundsInDisplaySpace(
            *cropLayerSnapshot, inputBounds, displayInfo.transform);
      }
      snapshot->inputInfo.touchableRegion = Region(inputBoundsInDisplaySpace);
      ...
      snapshot->inputInfo.touchableRegion =
          snapshot->inputInfo.touchableRegion.intersect(
              inputBoundsInDisplaySpace);
    }

    // If the layer is a clone, we need to crop the input region to cloned root
    // to prevent touches from going outside the cloned area.
    if (clonedRootSnapshot) {
      const Rect rect = displayInfo.transform.transform(
          Rect{clonedRootSnapshot->transformedBounds});
      snapshot->inputInfo.touchableRegion =

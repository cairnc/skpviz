...
RequestedLayerState::RequestedLayerState(const LayerCreationArgs &args)
    : id(args.sequence), name(args.name + "#" + std::to_string(args.sequence)),
      canBeRoot(args.addToRoot), layerCreationFlags(args.flags),
      ownerUid(args.ownerUid), ownerPid(args.ownerPid), parentId(args.parentId),
  ...
  flags = 0;
  if (args.flags & ISurfaceComposerClient::eHidden)
    flags |= layer_state_t::eLayerHidden;
  if (args.flags & ISurfaceComposerClient::eOpaque)
    flags |= layer_state_t::eLayerOpaque;
  if (args.flags & ISurfaceComposerClient::eSecure)
    flags |= layer_state_t::eLayerSecure;
  if (args.flags & ISurfaceComposerClient::eSkipScreenshot) {
    flags |= layer_state_t::eLayerSkipScreenshot;
  }
  premultipliedAlpha =
      !(args.flags & ISurfaceComposerClient::eNonPremultiplied);
  ...
  defaultFrameRateCompatibility =
      static_cast<int8_t>(scheduler::FrameRateCompatibility::Default);
  frameRateCategory = static_cast<int8_t>(FrameRateCategory::Default);
  frameRateCategorySmoothSwitchOnly = false;
  frameRateSelectionStrategy = static_cast<int8_t>(
...
void RequestedLayerState::merge(
  ...
  what = clientChanges;
  LLOGV(layerId, "requested=%" PRIu64 " flags=%" PRIu64 " ", clientState.what,
        clientChanges);
  ...
  if (clientState.what & layer_state_t::eBufferChanged) {
    ...
    if (hasBuffer || hasBuffer != hadBuffer) {
      ...
      const ui::Size newBufferSize =
          hasBuffer ? ui::Size(externalTexture->getWidth(),
                               externalTexture->getHeight())
                    : ui::Size();
    ...
    if (hasBuffer) {
      const bool frameNumberChanged = bufferData->flags.test(
      ...
      if ((barrierProducerId > bufferData->producerId) ||
        ...
        ALOGE("Out of order buffers detected for %s producedId=%d "
              "frameNumber=%" PRIu64 " -> producedId=%d frameNumber=%" PRIu64,
    ...
    const bool newBufferFormatOpaque = LayerSnapshot::isOpaqueFormat(
        externalTexture ? externalTexture->getPixelFormat()
                        : PIXEL_FORMAT_NONE);
  ...
  if (clientState.what & layer_state_t::eReparent) {
    ...
    // Once a layer has be reparented, it cannot be placed at the root. It
    // sounds odd but thats the existing logic and until we make this behavior
    // more explicit, we need to maintain this logic.
    canBeRoot = false;
  }
  ...
  if ((clientState.what & layer_state_t::eLayerChanged ||
       (clientState.what & layer_state_t::eReparent &&
  ...
  if (clientState.what & layer_state_t::eReparent &&
    ...
    // provide a hint that we are are now a direct child and not a relative
    // child.
    changes |= RequestedLayerState::Changes::RelativeParent;
  }
  ...
  if (clientState.what & layer_state_t::eMetadataChanged) {
    ...
    if (requestedGameMode != -1) {
      // The transaction will be received on the Task layer and needs to be
  ...
  if (clientState.what & layer_state_t::eFrameRateChanged) {
    const auto compatibility = Layer::FrameRate::convertCompatibility(
        clientState.frameRateCompatibility);
    const auto strategy = Layer::FrameRate::convertChangeFrameRateStrategy(
        clientState.changeFrameRateStrategy);
    requestedFrameRate.vote = Layer::FrameRate::FrameRateVote(
        Fps::fromValue(clientState.frameRate), compatibility, strategy);
    changes |= RequestedLayerState::Changes::FrameRate;
  }
  if (clientState.what & layer_state_t::eFrameRateCategoryChanged) {
    const auto category =
        Layer::FrameRate::convertCategory(clientState.frameRateCategory);
    requestedFrameRate.category = category;
    changes |= RequestedLayerState::Changes::FrameRate;
  }
}

ui::Size RequestedLayerState::getUnrotatedBufferSize(
    uint32_t displayRotationFlags) const {
...
ui::Transform
RequestedLayerState::getTransform(uint32_t displayRotationFlags) const {
  if ((flags & layer_state_t::eIgnoreDestinationFrame) ||
      destinationFrame.isEmpty()) {
  ...
  if (!externalTexture) {
    ...
    transform.set(static_cast<float>(destRect.left),
...
std::string RequestedLayerState::getDebugString() const {
  ...
  if (relativeParentId != UNASSIGNED_LAYER_ID)
    debug << " relativeParentId=" << relativeParentId;
  if (!mirrorIds.empty())
    debug << " mirrorId=" << layerIdsToString(mirrorIds);
  if (!handleAlive)
    debug << " !handle";
  if (z != 0)
    debug << " z=" << z;
  if (layerStack.id != 0)
    debug << " layerStack=" << layerStack.id;
  debug << "}";
  return debug.str();
}

std::ostream &operator<<(std::ostream &out,
                         const scheduler::LayerInfo::FrameRate &obj) {
  out << obj.vote.rate;
...
std::ostream &operator<<(std::ostream &out, const RequestedLayerState &obj) {
  out << obj.debugName;
  if (obj.relativeParentId != UNASSIGNED_LAYER_ID)
    out << " parent=" << obj.parentId;
  if (!obj.handleAlive)
    out << " handleNotAlive";
...
Rect RequestedLayerState::getBufferSize(uint32_t displayRotationFlags) const {
  ...
  return Rect(0, 0, static_cast<int32_t>(bufWidth),
              static_cast<int32_t>(bufHeight));
}

FloatRect
RequestedLayerState::getCroppedBufferSize(const Rect &bufferSize) const {
...
Rect RequestedLayerState::getBufferCrop() const {
  ...
  if (!bufferCrop.isEmpty() && externalTexture != nullptr) {
    // if the buffer crop is defined and there's a valid buffer, intersect
    // buffer size and crop since the crop should never exceed the size of the
    // buffer.
    Rect sizeAndCrop;
...
RequestedLayerState::getCompositionType() const {
...
// Returns true if the layer has a relative parent that is not its own parent.
// This is an input error from the client, and this check allows us to handle it
// gracefully. If both parentId and relativeParentId is unassigned then the
// layer does not have a valid relative parent. If the relative parentid is
// unassigned, the layer will be considered relative but won't be reachable.
...
bool RequestedLayerState::hasReadyFrame() const {
  return hasFrameUpdate() || changes.test(Changes::SidebandStream) ||
         autoRefresh;
}
...
bool RequestedLayerState::isSimpleBufferUpdate(const layer_state_t &s) const {
  ...
  if ((s.what & requiredFlags) != requiredFlags) {
    SFTRACE_FORMAT_INSTANT("%s: false [missing required flags 0x%" PRIx64 "]",
                           __func__, (s.what | requiredFlags) & ~s.what);
    return false;
  }

  const uint64_t deniedFlags =
      layer_state_t::eProducerDisconnect | layer_state_t::eLayerChanged |
      layer_state_t::eRelativeLayerChanged |
  ...
  if (s.what & deniedFlags) {
    SFTRACE_FORMAT_INSTANT("%s: false [has denied flags 0x%" PRIx64 "]",
                           __func__, s.what & deniedFlags);
    return false;
  }

  const uint64_t changedFlags = diff(s);
  const uint64_t deniedChanges =
      layer_state_t::ePositionChanged | layer_state_t::eAlphaChanged |
      layer_state_t::eColorTransformChanged |
      ...
      layer_state_t::eCropChanged | layer_state_t::eDataspaceChanged |
      layer_state_t::eHdrMetadataChanged |
      layer_state_t::eSidebandStreamChanged |
      layer_state_t::eColorSpaceAgnosticChanged |
      ...
      layer_state_t::eDimmingEnabledChanged |
  ...
  if (changedFlags & deniedChanges) {
    SFTRACE_FORMAT_INSTANT("%s: false [has denied changes flags 0x%" PRIx64 "]",
                           __func__, changedFlags & deniedChanges);
    return false;
  }
...
bool RequestedLayerState::isProtected() const {
  return externalTexture &&
         externalTexture->getUsage() & GRALLOC_USAGE_PROTECTED;
}

bool RequestedLayerState::hasSomethingToDraw() const {
  return externalTexture != nullptr || sidebandStream != nullptr ||
         shadowRadius > 0.f || backgroundBlurRadius > 0 ||
         blurRegions.size() > 0 ||

...
// Frames per second, stored as floating-point frequency. Provides conversion
// from/to period in nanoseconds, and relational operators with precision
// threshold.
//
//     const Fps fps = 60_Hz;
//
...
public:
  ...
  static constexpr Fps fromValue(float frequency) {
    return frequency > 0.f
               ? Fps(frequency, static_cast<nsecs_t>(1e9f / frequency))
               : Fps();
  }
...
private:
  constexpr Fps(float frequency, nsecs_t period)
      : mFrequency(frequency), mPeriod(period) {}
...
inline bool operator==(Fps lhs, Fps rhs) { return isApproxEqual(lhs, rhs); }

inline bool operator<(Fps lhs, Fps rhs) { return isApproxLess(lhs, rhs); }

inline bool operator!=(Fps lhs, Fps rhs) { return !isApproxEqual(lhs, rhs); }

inline bool operator>(Fps lhs, Fps rhs) { return isApproxLess(rhs, lhs); }
...
inline bool operator!=(FpsRange lhs, FpsRange rhs) { return !(lhs == rhs); }
...
constexpr Fps operator/(Fps fps, unsigned divisor) {
  return Fps::fromPeriodNsecs(fps.getPeriodNsecs() *
                              static_cast<nsecs_t>(divisor));
}
...
inline std::string to_string(FpsRanges ranges) {
  ...
  return base::StringPrintf("{physical=%s, render=%s}",
                            to_string(physical).c_str(),

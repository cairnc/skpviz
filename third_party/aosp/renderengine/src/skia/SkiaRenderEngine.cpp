...
/**
 *  Verifies that common, simple bounds + clip combinations can be converted
 * into a single RRect draw call returning true if possible. If true the radii
 * parameter will be filled with the correct radii values that combined with
 * bounds param will produce the insected roundRect. If false, the returned
 * state of the radii param is undefined.
 */
static bool intersectionIsRoundRect(const SkRect &bounds, const SkRect &crop,
  ...
  // In the event that the corners of the bounds only partially align with the
  // crop we need to ensure that the resulting shape can still be represented as
  // a round rect. In particular the round rect implementation will scale the
  // value of all corner radii if the sum of the radius along any edge is
  // greater than the length of that edge. See
  ...
  // Check each cropped corner to ensure that it exactly matches the crop or its
  // corner is contained within the cropped shape and does not need rounded.
...
static inline std::pair<SkRRect, SkRRect>
getBoundsAndClip(const android::FloatRect &boundsRect,
  ...
  if (cornerRadius.x > 0 && cornerRadius.y > 0) {
    // it the crop and the bounds are equivalent or there is no crop then we
    // don't need a clip
    if (bounds == crop || crop.isEmpty()) {
      return {SkRRect::MakeRectXY(bounds, cornerRadius.x, cornerRadius.y),
              clip};
    }

    // This makes an effort to speed up common, simple bounds + clip
    // combinations by converting them to a single RRect draw. It is possible
    // there are other cases that can be converted.
    if (crop.contains(bounds)) {
      const auto insetCrop = crop.makeInset(cornerRadius.x, cornerRadius.y);
      if (insetCrop.contains(bounds)) {
        return {SkRRect::MakeRect(bounds),
                clip}; // clip is empty - no rounding required
      }

      SkVector radii[4];
      if (intersectionIsRoundRect(bounds, crop, insetCrop, cornerRadius,
                                  radii)) {
  ...
  // if we hit this point then we either don't have rounded corners or we are
  // going to rely on the clip to round the corners for us
  return {SkRRect::MakeRect(bounds), clip};
}

static inline bool
layerHasBlur(const android::renderengine::LayerSettings &layer,
             bool colorTransformModifiesAlpha) {
  if (layer.backgroundBlurRadius > 0 || layer.blurRegions.size()) {
    // return false if the content is opaque and would therefore occlude the
    // blur
    const bool opaqueContent =
        !layer.source.buffer.buffer || layer.source.buffer.isOpaque;
    const bool opaqueAlpha =
        layer.alpha == 1.0f && !colorTransformModifiesAlpha;
...
void SkiaRenderEngine::SkSLCacheMonitor::store(const SkData &key,
                                               const SkData &data,
...
// To be called from backend dtors. Used to clean up Skia objects before GPU API
// contexts are destroyed by subclasses.
void SkiaRenderEngine::finishRenderingAndAbandonContexts() {
  ...
  // Leftover textures may hold refs to backend-specific Skia contexts, which
  // must be released before ~SkiaGpuContext is called.
...
void SkiaRenderEngine::useProtectedContext(bool useProtectedContext) {
  ...
  if (useProtectedContextImpl(useProtectedContext ? GrProtected::kYes
                                                  : GrProtected::kNo)) {
...
static SkColorMatrix toSkColorMatrix(const android::mat4 &matrix) {
  return SkColorMatrix(matrix[0][0], matrix[1][0], matrix[2][0], matrix[3][0],
                       0, matrix[0][1], matrix[1][1], matrix[2][1],
                       matrix[3][1], 0, matrix[0][2], matrix[1][2],
                       matrix[2][2], matrix[3][2], 0, matrix[0][3],
                       matrix[1][3], matrix[2][3], matrix[3][3], 0);
}

static bool needsToneMapping(ui::Dataspace sourceDataspace,
...
void SkiaRenderEngine::mapExternalTextureBuffer(const sp<GraphicBuffer> &buffer,
  ...
  // We don't attempt to map a buffer if the buffer contains protected content.
  // In GL this is important because GPU resources for protected buffers are
  // much more limited. (In Vk we simply match the existing behavior for
  // protected buffers.)  We also never cache any buffers while in a protected
  // context.
  const bool isProtectedBuffer = buffer->getUsage() & GRALLOC_USAGE_PROTECTED;
  // Don't attempt to map buffers if we're not gpu sampleable. Callers shouldn't
  // send a buffer over to RenderEngine.
  ...
  // If we were to support caching protected buffers then we will need to switch
  // the currently bound context if we are not already using the protected
  // context (and subsequently switch back after the buffer is cached).
  ...
  if (const auto &iter = cache.find(buffer->getId()); iter == cache.end()) {
    ...
    auto imageTextureRef = std::make_shared<AutoBackendTexture::LocalRef>(
...
void SkiaRenderEngine::unmapExternalTextureBuffer(sp<GraphicBuffer> &&buffer) {
  ...
  if (const auto &iter = mGraphicBufferExternalRefs.find(buffer->getId());
    ...
    // even when a protected context and an unprotected context are part of the
    // same share group, protected surfaces may not be accessed by an
    // unprotected context, implying that protected surfaces may only be freed
    // when a protected context is active.
    ...
    // Swap back to the previous context so that cached values of isProtected in
    // SurfaceFlinger are up-to-date.
...
SkiaRenderEngine::getOrCreateBackendTexture(const sp<GraphicBuffer> &buffer,
                                            bool isOutputBuffer) {
  // Do not lookup the buffer in the cache for protected contexts
  if (!isProtected()) {
    if (const auto &it = mTextureCache.find(buffer->getId());
        it != mTextureCache.end()) {
      return it->second;
    }
  }
  std::unique_ptr<SkiaBackendTexture> backendTexture =
      getActiveContext()->makeBackendTexture(buffer->toAHardwareBuffer(),
                                             isOutputBuffer);
  return std::make_shared<AutoBackendTexture::LocalRef>(
      std::move(backendTexture), mTextureCleanupMgr);
}
...
sk_sp<SkShader> SkiaRenderEngine::createRuntimeEffectShader(
  ...
  if (graphicBuffer && parameters.shader) {
    ...
    // The given surface requires to be filled outside of its buffer bounds if
    // the edge extension is required
    const auto &edgeExtensionEffect = parameters.layer.edgeExtensionEffect;
    if (edgeExtensionEffect.hasEffect()) {
      shader = mEdgeExtensionShaderFactory.createSkShader(
          shader, parameters.layer, parameters.imageBounds);
    }
  }
  ...
  if (parameters.requiresLinearEffect) {
    ...
    const auto hdrType = getHdrRenderType(parameters.layer.sourceDataspace,
                                          format, parameters.layerDimmingRatio);

    const auto usingLocalTonemap =
        parameters.display.tonemapStrategy ==
            DisplaySettings::TonemapStrategy::Local &&
    ...
    if (usingLocalTonemap) {
      const float inputRatio = hdrType == HdrRenderType::GENERIC_HDR
                                   ? 1.0f
    ...
    auto inputDataspace =
        usingLocalTonemap || (graphicBuffer && parameters.layer.luts)
            ? parameters.outputDataSpace
            : parameters.layer.sourceDataspace;
    auto effect = shaders::LinearEffect{
        .inputDataspace = inputDataspace,
    ...
    if (!usingLocalTonemap) {
      colorTransform *= mat4::scale(vec4(parameters.layerDimmingRatio,
                                         parameters.layerDimmingRatio,
                                         parameters.layerDimmingRatio, 1.f));
    }

    const auto hardwareBuffer =
        graphicBuffer ? graphicBuffer->toAHardwareBuffer() : nullptr;
    return createLinearEffectShader(
        shader, effect, runtimeEffect, std::move(colorTransform),
        parameters.display.maxLuminance,
        parameters.display.currentLuminanceNits,
        parameters.layer.source.buffer.maxLuminanceNits, hardwareBuffer,
        parameters.display.renderIntent);
  }
  return shader;
}

void SkiaRenderEngine::initCanvas(SkCanvas *canvas,
  ...
  if (CC_UNLIKELY(mCapture->isCaptureRunning())) {
    ...
    canvas->drawAnnotation(
        SkRect::MakeEmpty(), "DisplaySettings",
        SkData::MakeWithCString(displaySettings.str().c_str()));
  }

  // Before doing any drawing, let's make sure that we'll start at the origin of
  // the display. Some displays don't start at 0,0 for example when we're
  // mirroring the screen. Also, virtual displays might have different scaling
  // when compared to the physical screen.
  ...
  // Canvas rotation is done by centering the clip window at the origin,
  // rotating, translating back so that the top left corner of the clip is at
  // (0, 0).
...
public:
  AutoSaveRestore(SkCanvas *canvas) : mCanvas(canvas) {
...
static SkRRect getBlurRRect(const BlurRegion &region) {
  const auto rect =
      SkRect::MakeLTRB(region.left, region.top, region.right, region.bottom);
  const SkVector radii[4] = {
      SkVector::Make(region.cornerRadiusTL, region.cornerRadiusTL),
...
static bool equalsWithinMargin(float expected, float value,
...
namespace {
template <typename T> void logSettings(const T &t) {
...
void SkiaRenderEngine::drawLayersInternal(
  ...
  LOG_ALWAYS_FATAL_IF(context->isAbandonedOrDeviceLost(),
                      "Context is abandoned/device lost at start of %s",
                      __func__);

  // any AutoBackendTexture deletions will now be deferred until
  // cleanupPostRender is called
  ...
  sk_sp<SkSurface> dstSurface =
      surfaceTextureRef->getOrCreateSurface(display.outputDataspace);
  ...
  if (display.colorTransform != mat4() &&
      !display.deviceHandlesColorTransform) {
  ...
  // Find the max layer white point to determine the max luminance of the
  // scene...
  ...
  const float displayDimmingRatio =
      display.targetLuminanceNits > 0.f && maxLayerWhitePoint > 0.f
          ? maxLayerWhitePoint / display.targetLuminanceNits
          : 1.f;

  // Find if any layers have requested blur, we'll use that info to decide when
  // to render to an offscreen buffer and when to render to the native buffer.
  ...
  for (const auto &layer : layers) {
    ...
    if (blurCompositionLayer == &layer) {
      ...
      // save a snapshot of the activeSurface to use as input to the blur
      // shaders
      blurInput = activeSurface->makeImageSnapshot();

      // blit the offscreen framebuffer into the destination AHB. This ensures
      // that even if the blurred image does not cover the screen (for example,
      // during a rotation animation, or if blur regions are used), the entire
      // screen is initialized.
      if (layer.blurRegions.size() ||
          FlagManager::getInstance().restore_blur_step()) {
        SkPaint paint;
        paint.setBlendMode(SkBlendMode::kSrc);
        if (CC_UNLIKELY(mCapture->isCaptureRunning())) {
          uint64_t id = mCapture->endOffscreenCapture(&offscreenCaptureState);
          dstCanvas->drawAnnotation(
              SkRect::Make(dstCanvas->imageInfo().dimensions()),
              String8::format("SurfaceID|%" PRId64, id).c_str(), nullptr);
      ...
      // assign dstCanvas to canvas and ensure that the canvas state is up to
      // date
      canvas = dstCanvas;
    ...
    if (CC_UNLIKELY(mCapture->isCaptureRunning())) {
      ...
      canvas->drawAnnotation(
          SkRect::MakeEmpty(), layer.name.c_str(),
          SkData::MakeWithCString(layerSettings.str().c_str()));
    }
    // Layers have a local transform that should be applied to them
    canvas->concat(getSkM44(layer.geometry.positionTransform).asM33());

    const auto [bounds, roundRectClip] = getBoundsAndClip(
        layer.geometry.boundaries, layer.geometry.roundedCornersCrop,
        layer.geometry.roundedCornersRadius);
    if (mBlurFilter && layerHasBlur(layer, ctModifiesAlpha)) {
      std::unordered_map<uint32_t, sk_sp<SkImage>> cachedBlurs;

      // if multiple layers have blur, then we need to take a snapshot now
      // because only the lowest layer will have blurImage populated earlier
      if (!blurInput) {
        blurInput = activeSurface->makeImageSnapshot();
      }
      ...
      if (blurRect.width() > 0 && blurRect.height() > 0) {
        if (layer.backgroundBlurRadius > 0) {
          ...
          mBlurFilter->drawBlurRegion(canvas, bounds,
                                      layer.backgroundBlurRadius, 1.0f,
        ...
        for (auto region : layer.blurRegions) {
          if (cachedBlurs[region.blurRadius] == nullptr) {
            ...
            cachedBlurs[region.blurRadius] = mBlurFilter->generate(
                context, region.blurRadius, blurInput, blurRect);
          }

          mBlurFilter->drawBlurRegion(
              canvas, getBlurRRect(region), region.blurRadius, region.alpha,
              blurRect, cachedBlurs[region.blurRadius], blurInput);
        }
      }
    }
    ...
    if (layer.shadow.length > 0) {
      ...
      if (layer.geometry.boundaries == layer.shadow.boundaries) {
        ...
        std::tie(shadowBounds, shadowClip) = getBoundsAndClip(
            layer.shadow.boundaries, layer.geometry.roundedCornersCrop,
            layer.geometry.roundedCornersRadius);
      }
      ...
      const auto &rrect = shadowBounds.isRect() && !shadowClip.isEmpty()
                              ? shadowClip
                              : shadowBounds;
      drawShadow(canvas, rrect, layer.shadow);
    }

    const float layerDimmingRatio =
        layer.whitePointNits <= 0.f
            ? displayDimmingRatio
            : (layer.whitePointNits / maxLayerWhitePoint) * displayDimmingRatio;

    const bool dimInLinearSpace =
        display.dimmingStage !=
        aidl::android::hardware::graphics::composer3::DimmingStage::GAMMA_OETF;

    const bool isExtendedHdr =
        (layer.sourceDataspace & ui::Dataspace::RANGE_MASK) ==
    ...
    const ui::Dataspace fakeDataspace =
    ...
    // If the input dataspace is range extended, the output dataspace transfer
    // is sRGB and dimmingStage is GAMMA_OETF, dim in linear space instead, and
    ...
    const bool requiresLinearEffect =
        layer.colorTransform != mat4() ||
    ...
    if (layer.skipContentDraw ||
        (layer.alpha == 0 && !requiresLinearEffect && !layer.disableBlending &&
         (!displayColorTransform ||
          displayColorTransform->isAlphaUnchanged()))) {
      continue;
    }
    ...
    if (layer.source.buffer.buffer) {
      ...
      auto imageTextureRef =
          getOrCreateBackendTexture(item.buffer->getBuffer(), false);

      // if the layer's buffer has a fence, then we must respect the fence prior
      // to using the buffer.
      ...
      sk_sp<SkImage> image =
          imageTextureRef->makeImage(layerDataspace, alphaType);

      auto texMatrix = getSkM44(item.textureTransform).asM33();
      // textureTansform was intended to be passed directly into a shader, so
      // when building the total matrix with the textureTransform we need to
      // first normalize it, then apply the textureTransform, then scale back
      // up.
      ...
      // The shader does not respect the translation, so we add it to the
      // texture transform for the SkImage. This will make sure that the correct
      // layer contents are drawn in the correct part of the screen.
      ...
      if (useIsOpaqueWorkaround) {
        shader = SkShaders::Blend(
            SkBlendMode::kPlus, shader,
            SkShaders::Color(SkColors::kBlack, toSkColorSpace(layerDataspace)));
      }
      ...
      // Most HDR standards require at least 10-bits of color depth for source
      // content, so we can just extract the transfer function rather than dig
      // into precise gralloc layout. Furthermore, we can assume that the only
      // 8-bit target we support is RGBA8888.
      ...
      if (imageTextureRef->colorType() == kAlpha_8_SkColorType) {
        ...
        SkColorMatrix colorMatrix(0, 0, 0, 0, black[0], 0, 0, 0, 0, black[1], 0,
                                  0, 0, 0, black[2], 0, 0, 0, -1, 1);
        if (display.colorTransform != mat4() &&
            !display.deviceHandlesColorTransform) {
      ...
      sk_sp<SkShader> shader = SkShaders::Color(
          SkColor4f{
              .fR = color.r, .fG = color.g, .fB = color.b, .fA = layer.alpha},
    ...
    if (!paint.getColorFilter()) {
      if (!dimInLinearSpace && !equalsWithinMargin(1.0, layerDimmingRatio)) {
        // If we don't dim in linear space, then when we gamma correct the
        // dimming ratio we can assume a gamma 2.2 transfer function.
        ...
        auto dimmingMatrix = mat4::scale(vec4(gammaCorrectedDimmingRatio,
                                              gammaCorrectedDimmingRatio,
    ...
    if (kGaneshFlushAfterEveryLayer) {
      ...
      // No-op in Graphite. If "flushing" Skia's drawing commands after each
      // layer is desired in Graphite, then a graphite::Recording would need to
      // be snapped and tracked for each layer, which is likely possible but
      // adds non-trivial complexity (in both bookkeeping and refactoring).
...
void SkiaRenderEngine::drawGainmapInternal(
  ...
  std::lock_guard<std::mutex> lock(mRenderingMutex);
  auto context = getActiveContext();
  auto surfaceTextureRef =
      getOrCreateBackendTexture(gainmap->getBuffer(), true);
  sk_sp<SkSurface> dstSurface =
  ...
  const auto sdrImage =
      sdrTextureRef->makeImage(dataspace, kPremul_SkAlphaType);
  const auto sdrShader = sdrImage->makeShader(
      SkTileMode::kClamp, SkTileMode::kClamp,
  ...
  const auto hdrImage =
      hdrTextureRef->makeImage(dataspace, kPremul_SkAlphaType);
  const auto hdrShader = hdrImage->makeShader(
      SkTileMode::kClamp, SkTileMode::kClamp,
      SkSamplingOptions({SkFilterMode::kLinear, SkMipmapMode::kNone}), nullptr);

  static GainmapFactory kGainmapFactory;
  const auto gainmapShader =
      kGainmapFactory.createSkShader(sdrShader, hdrShader, hdrSdrRatio);
...
void SkiaRenderEngine::drawShadow(SkCanvas *canvas, const SkRRect &casterRRect,
                                  const ShadowSettings &settings) {
  SFTRACE_CALL();
  const float casterZ = settings.length / 2.0f;
  const auto flags = settings.casterIsTranslucent
                         ? kTransparentOccluder_ShadowFlag
                         : kNone_ShadowFlag;

  SkShadowUtils::DrawShadow(
      canvas, SkPath::RRect(casterRRect), SkPoint3::Make(0, 0, casterZ),
      getSkPoint3(settings.lightPos), settings.lightRadius,
      getSkColor(settings.ambientColor), getSkColor(settings.spotColor), flags);
}
...
void SkiaRenderEngine::onActiveDisplaySizeChanged(ui::Size size) {
  ...
  // to the screen resolution. Looking at the worst case memory needed by blur
  // (~1.5x), shadows (~1x), and general data structures (e.g. vertex buffers)
  // we selected this as a conservative default based on that analysis.
  const float SURFACE_SIZE_MULTIPLIER =
      3.5f * bytesPerPixel(mDefaultPixelFormat);
  const int maxResourceBytes =
      size.width * size.height * SURFACE_SIZE_MULTIPLIER;
  ...
  if (mInProtectedContext != originalProtectedState) {
    getActiveContext()->setResourceCacheLimit(maxResourceBytes);
    // reset back to the initial context that was active when this method was
    // called
...
void SkiaRenderEngine::dump(std::string &result) {
  ...
  StringAppendF(&result,
                "RenderEngine shaders cached since last dump/primeCache: %d\n",
...

  {
    ...
    StringAppendF(&result, "RenderEngine AHB/BackendTexture cache size: %zu\n",
                  mTextureCache.size());
    StringAppendF(&result, "Dumping buffer ids...\n");
    // TODO(178539829): It would be nice to know which layer these are coming
    // from and what the texture sizes are.
    ...
    for (const auto &[linearEffect, unused] : mRuntimeEffects) {
      StringAppendF(&result, "- inputDataspace: %s\n",
                    dataspaceDetails(static_cast<android_dataspace>(
                                         linearEffect.inputDataspace))
                        .c_str());
      StringAppendF(&result, "- outputDataspace: %s\n",
                    dataspaceDetails(static_cast<android_dataspace>(
                                         linearEffect.outputDataspace))
                        .c_str());

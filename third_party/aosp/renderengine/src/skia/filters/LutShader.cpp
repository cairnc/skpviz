...
sk_sp<SkShader> LutShader::generateLutShader(
    sk_sp<SkShader> input, const std::vector<float> &buffers,
    const int32_t offset, const int32_t length, const int32_t dimension,
    const int32_t size, const int32_t samplingKey, ui::Dataspace srcDataspace) {
   ...
   */
  SkImageInfo info =
      SkImageInfo::Make(length /* the number of rgba */ * 4, 1,
  ...
  if (static_cast<LutProperties::SamplingKey>(samplingKey) ==
      LutProperties::SamplingKey::CIE_Y) {
    // Use predefined colorspaces of input dataspace so that we can get D65
    // illuminant
...
sk_sp<SkShader> LutShader::lutShader(
    sk_sp<SkShader> &input, std::shared_ptr<gui::DisplayLuts> displayLuts,
    ui::Dataspace srcDataspace, sk_sp<SkColorSpace> outColorSpace) {
  if (mBuilder == nullptr) {
    const static SkRuntimeEffect::Result instance =
        SkRuntimeEffect::MakeForShader(kShader);
  ...
  if (fd.ok()) {
    ...
    SkImage *baseImage =
        input->isAImage((SkMatrix *)nullptr, (SkTileMode *)nullptr);
    ...
    float *ptr = (float *)mmap(NULL, bufferSize, PROT_READ | PROT_WRITE,
                               MAP_SHARED, fd.get(), 0);
    if (ptr == MAP_FAILED) {
    ...
    for (size_t i = 0; i < offsets.size(); i++) {
      int bufferSizePerLut = (i == offsets.size() - 1)
                                 ? buffers.size() - offsets[i]
      ...
      input =
          generateLutShader(input, buffers, offsets[i], bufferSizePerLut,
                            lutProperties[i].dimension, lutProperties[i].size,
                            lutProperties[i].samplingKey, srcDataspace);
    }

    auto colorXformLutToDst = SkColorFilterPriv::MakeColorSpaceXform(
        lutMathColorSpace, outColorSpace);

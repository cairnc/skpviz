...
class Looper : public RefBase
{
  ...
  public:
     ...
     *
     * The callback may either be specified as a bare function pointer or as a
     * smart pointer callback object.  The smart pointer should be preferred
     * because it is easier to avoid races when the callback is removed from a
     * different thread. See removeFd() for details.
     */
    int addFd(int fd, int ident, int events, Looper_callbackFunc callback,
              void *data);
    ...
    bool getFdStateDebug(int fd, int *ident, int *events,
                         sp<LooperCallback> *cb, void **data);
     ...
     * LooperCallback will be released at the appropriate time by the Looper.
     *
    ...
    /**
     * Enqueues a message to be processed by the specified handler after all
     * pending messages after the specified delay.
     *
     * The time delay is specified in uptime nanoseconds.
     * The handler must not be null.
     * This method can be called on any thread.
     */
    void sendMessageDelayed(nsecs_t uptimeDelay,
                            const sp<MessageHandler> &handler,
                            const Message &message);

    /**
     * Enqueues a message to be processed by the specified handler after all
     * pending messages at the specified time.
     *
    ...
    /**
     * Removes all messages of a particular type for the specified handler from
     * the queue.
     *
     * The handler must not be null.
     * This method can be called on any thread.
  ...
  private:
    ...
    android::base::unique_fd
        mEpollFd; // guarded by mLock but only modified on the looper thread
    ...
    std::unordered_map<int /*fd*/, SequenceNumber>
        mSequenceNumberByFd; // guarded by mLock
    ...
    // This state is only used privately by pollOnce and does not require a lock
    // since it runs on a single thread.

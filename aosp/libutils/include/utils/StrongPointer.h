...
template <typename T> class sp
{
  public:
    ...
    //
    //     friend class sp<Foo>;
    template <typename... Args> static inline sp<T> make(Args &&...args);
    ...
    template <typename U> static inline sp<T> cast(const sp<U> &other);
    ...
    // Releases the ownership of the object managed by this instance of sp, if
    // any. The caller is now responsible for managing it. That is, the caller
    // must ensure decStrong() is called when the pointer is no longer used.
...
// TODO: Ideally we should find a way to increment the reference count before
// running the constructor, so that generating an sp<> to this in the
// constructor is no longer dangerous.
template <typename T>
...
template <typename T> sp<T> sp<T>::fromExisting(T *other)
{
    if (other)
    {
...
template <typename T> sp<T>::sp(sp<T> &&other) noexcept : m_ptr(other.m_ptr)
{
    other.m_ptr = nullptr;
}
...
template <typename T> sp<T> &sp<T>::operator=(sp<T> &&other) noexcept
{
    T *oldPtr(*const_cast<T *volatile *>(&m_ptr));
    if (oldPtr)
        oldPtr->decStrong(this);
...
#if !defined(ANDROID_UTILS_REF_BASE_DISABLE_IMPLICIT_CONSTRUCTION)
template <typename T> template <typename U> sp<T> &sp<T>::operator=(U *other)
{
    T *oldPtr(*const_cast<T *volatile *>(&m_ptr));
    if (other)
        (static_cast<T *>(other))->incStrong(this);
...
template <typename T> void sp<T>::force_set(T *other)
{

...
// It would have been better if this file (ErrorsMacros.h) is entirely in
// utils/Errors.h. However that is infeasible as some (actually many) are using
// utils/Errors.h via the implicit include path `system/core/include` [1]. Since
// such users are not guaranteed to specify the dependency to libbase_headers,
// the following headers from libbase_headers can't be found. [1]
// build/soong/cc/config/global.go#commonGlobalIncludes
...
// StatusT is a wrapper class for status_t. Use this type instead of status_t
// when instantiating Result<T, E> and Error<E> template classes. This is
// required to distinguish status_t from other integer-based error code types
// like errno, and also to provide utility functions like print().
struct StatusT
{
    StatusT() : val_(OK)
    {
...
template <> struct ResultError<StatusT, false>
{
...
template <> struct ResultError<StatusT, true>
{
    template <typename T>
    ResultError(T &&message, status_t s)
        : val_(s), message_(std::forward<T>(message))
    {
        LOG_FATAL_IF(s == OK, "Result error should not hold success");
    }
    ...
    std::string message() const
    {
        return statusToString(val_) + message_;
    }
...
// Specialization of android::base::OkOrFail<V> for V = status_t. This is used
// to use the OR_RETURN and OR_FATAL macros with statements that yields a value
// of status_t. See android-base/errors.h for the detailed contract.
template <> struct OkOrFail<status_t>
{
    static_assert(std::is_same_v<status_t, int>);
    // Tests if status_t is a success value of not.
    static bool IsOk(const status_t &s)
    {
        return s == OK;
    }

    // Unwrapping status_t in the success case is just asserting that it is
    // actually a success. We don't return OK because it would be redundant.
    static void Unwrap([[maybe_unused]] status_t &&s)
    {
        assert(IsOk(s));
    }
    ...
    // And converts back into status_t. This is used when OR_RETURN is used in a
    ...
    template <typename T> operator Result<T, StatusT, false>() &&
    {
...
#define SPECIALIZED_CONVERSION(type)                                           \
    operator Result<type, StatusT>() &&                                        \
    {                                                                          \
        return ResultError<StatusT>(std::move(val_));                          \
    }                                                                          \
    operator Result<type, StatusT, false>() &&                                 \
    {                                                                          \
        return ResultError<StatusT, false>(std::move(val_));                   \
    }
...
#pragma pop_macro("SPECIALIZED_CONVERSION")
    ...
    static std::string ErrorMessage(const status_t &s)
    {
...
bool operator==(const base::ResultError<StatusT, include_message> &l,
                const status_t &r)
{
    return (l.code() == r);
}
template <bool include_message>
bool operator==(const status_t &l,
                const base::ResultError<StatusT, include_message> &r)
{
    return (l == r.code());
}

...
template <typename T, typename U> struct aggregate_traits
{
    enum
    {
        is_pointer = false,
...
#define ANDROID_TRIVIAL_MOVE_TRAIT(T)                                          \
    template <> struct trait_trivial_move<T>                                   \
    {                                                                          \
        enum                                                                   \
        {                                                                      \
            value = true                                                       \
...
template <typename T> struct trait_trivial_ctor<T *>
{
    enum
    {
        value = true
...
template <typename T> struct trait_trivial_dtor<T *>
{
    enum
    {
        value = true
...
template <typename T> struct trait_trivial_copy<T *>
{
    enum
    {
        value = true
...
template <typename T> struct trait_trivial_move<T *>
{
    enum
    {
        value = true
...
template <typename TYPE> inline void construct_type(TYPE *p, size_t n)
{
...
template <typename TYPE> inline void destroy_type(TYPE *p, size_t n)
{
...
typename std::enable_if<
    use_trivial_move<TYPE>::value>::type inline move_forward_type(TYPE *d,
                                                                  const TYPE *s,
                                                                  size_t n = 1)
{
...
template <typename KEY, typename VALUE> struct key_value_pair_t
{
    typedef KEY key_t;
...
template <typename TKey> hash_t hash_type(const TKey &key);

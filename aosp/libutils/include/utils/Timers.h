...
extern "C"
{
    ...
    enum
    {
        ...
        SYSTEM_TIME_BOOTTIME =
            4, // same as SYSTEM_TIME_MONOTONIC, but including CPU suspend time
    };
...
nsecs_t systemTime(int clock);
    ...
    /**
     * Returns the number of milliseconds to wait between the reference time and
     * the timeout time. If the timeout is in the past relative to the reference
     * time, returns 0. If the timeout is more than INT_MAX milliseconds in the
     * future relative to the reference time, such as when timeoutTime ==
     * LLONG_MAX, returns -1 to indicate an infinite timeout delay. Otherwise,
     * returns the difference between the reference time and timeout time

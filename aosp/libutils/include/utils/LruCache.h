 ...
 */
template <typename EntryKey, typename EntryValue> class OnEntryRemoved
{
  public:
...
template <typename TKey, typename TValue> class LruCache
{
  ...
  private:
    ...
    class Entry final : public KeyedEntry
    {
      public:
        ...
        Entry(TKey _key, TValue _value)
            : key(_key), value(_value), parent(nullptr), child(nullptr)
        {
        }
        const TKey &getKey() const final
        {
            return key;
        }
    };
    ...
    typedef std::unordered_set<KeyedEntry *, HashForEntry,
                               EqualityForHashedEntries>
        LruCacheSet;
...
LruCache<TKey, TValue>::LruCache(uint32_t maxCapacity)
    : mSet(new LruCacheSet()), mListener(nullptr), mOldest(nullptr),
      mYoungest(nullptr), mMaxCapacity(maxCapacity), mNullValue{}
{
    mSet->max_load_factor(1.0);
};

template <typename TKey, typename TValue> LruCache<TKey, TValue>::~LruCache()
{
...
template <typename TKey, typename TValue> void LruCache<TKey, TValue>::clear()
{
    if (mListener)
    {

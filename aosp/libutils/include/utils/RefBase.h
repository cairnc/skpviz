...
// i.e. it's destructor is called. HOWEVER, parts of its associated memory is
// not freed until the last weak pointer is released.
...
// destroyed because it ran out of strong pointers. This makes them good
// candidates for keys in a cache for instance.

// Weak pointers remain valid for comparison purposes even after the underlying
// object has been destroyed. Even if object A is destroyed and its memory
// reused for B, A remaining weak pointer to A will not compare equal to one to
// B. This again makes them attractive for use as keys.
...
// Such strong or weak pointers can be safely created in the RefBase
// onFirstRef() callback.
...
#define COMPARE_WEAK(_op_)                                                     \
    template <typename U> inline bool operator _op_(const U *o) const          \
    {                                                                          \
        return m_ptr _op_ o;                                                   \
    }                                                                          \
...
template <typename T> class wp
{
  public:
    ...
    {
        return m_refs ==
               o.m_refs; // Implies m_ptr == o.mptr; see invariants below.
    }
...
// 3) *m_ptr is no longer live, and m_refs points to the weakref_type object
// that corresponded
//    to m_ptr while it was live. *m_refs remains live while a wp<> refers to
//    it.
//
// The m_refs field in a RefBase object is allocated on construction, unique to
// that RefBase object, and never changes. Thus if two wp's have identical
// m_refs fields, they are either both null or point to the same object. If two
// wp's have identical m_ptr fields, they either both point to the same live
// object and thus have the same m_ref fields, or at least one of the objects is
// no longer live.
//
// Note that the above comparison operations go out of their way to provide an
// ordering consistent with ordinary pointer comparison; otherwise they could
// ignore m_ptr, and just compare m_refs.

template <typename T> wp<T> wp<T>::fromExisting(T *other)
{
    if (!other)
        return nullptr;
...
template <typename T> wp<T> &wp<T>::operator=(T *other)
{
...
template <typename T> template <typename U> wp<T> &wp<T>::operator=(U *other)
{
...
template <typename T> wp<T> &wp<T>::operator=(const wp<T> &other)
{
...
template <typename T> wp<T> &wp<T>::operator=(const sp<T> &other)
{
...
template <typename T> void wp<T>::clear()
{
    if (m_ptr)
    {
        m_refs->decWeak(this);
...
class ReferenceMover
{
  public:
    ...
    static inline void move_references(sp<TYPE> *dest, sp<TYPE> const *src,
                                       size_t n)
    {
    ...
    template <typename TYPE>
    static inline void move_references(wp<TYPE> *dest, wp<TYPE> const *src,
                                       size_t n)
    {
...
template <typename TYPE>
inline void move_forward_type(sp<TYPE> *d, sp<TYPE> const *s, size_t n)
{
    ReferenceMover::move_references(d, s, n);
}

template <typename TYPE>
inline void move_backward_type(sp<TYPE> *d, sp<TYPE> const *s, size_t n)
{
    ReferenceMover::move_references(d, s, n);
}

template <typename TYPE>
inline void move_forward_type(wp<TYPE> *d, wp<TYPE> const *s, size_t n)
{
    ReferenceMover::move_references(d, s, n);
}

template <typename TYPE>
inline void move_backward_type(wp<TYPE> *d, wp<TYPE> const *s, size_t n)
{
    ReferenceMover::move_references(d, s, n);
}
...
template <typename T, typename... Args,
          typename std::enable_if<libutilsinternal::is_complete_type<T>::value,
                                  bool>::value = true,
          typename std::enable_if<std::is_base_of<android::RefBase, T>::value,
                                  bool>::value = true>
shared_ptr<T> make_shared(Args...)
{ // SEE COMMENT ABOVE.
    static_assert(!std::is_base_of<android::RefBase, T>::value,
                  "Must use RefBase with sp<>");
}

...
{
  public:
    template <size_t N> static size_t align(size_t size)
    {
        static_assert(!(N & (N - 1)), "Can only align to a power of 2.");
        return (size + (N - 1)) & ~(N - 1);
    }

    template <size_t N> static size_t align(void const *&buffer)
    {
        static_assert(!(N & (N - 1)), "Can only align to a power of 2.");
        uintptr_t b = uintptr_t(buffer);
        buffer =
            reinterpret_cast<void *>((uintptr_t(buffer) + (N - 1)) & ~(N - 1));
        return size_t(uintptr_t(buffer) - b);
    }

    template <size_t N> static size_t align(void *&buffer)
    {
        static_assert(!(N & (N - 1)), "Can only align to a power of 2.");
        void *b = buffer;
        buffer =
...
template <typename T> class Flattenable
{
  public:
    ...
    // flatten() if needed).
    inline status_t flatten(void *&buffer, size_t &size, int *&fds,
                            size_t &count) const;
...
template <typename T> inline size_t Flattenable<T>::getFlattenedSize() const
{
    return static_cast<T const *>(this)->T::getFlattenedSize();
}
template <typename T> inline size_t Flattenable<T>::getFdCount() const
{
...
template <typename T>
inline status_t Flattenable<T>::unflatten(void const *&buffer, size_t &size,
                                          int const *&fds, size_t &count)
{
...
template <typename T> inline bool LightFlattenable<T>::isFixedSize() const
{

...
// Collect/print the call stack (function, file, line) traces for a single
// thread.
class CallStack
{
  public:
    ...
    // Dump a serialized representation of the stack trace to the specified
    // printer.
...
#ifdef CALLSTACK_WEAKS_AVAILABLE
    static CallStackUPtr
        CALLSTACK_ALWAYS_INLINE getCurrent(int32_t ignoreDepth = 1)
    {
...
#ifdef CALLSTACK_WEAKS_AVAILABLE
    static void CALLSTACK_ALWAYS_INLINE
    logStack(const char *logtag, CallStack *stack = getCurrent().get(),
             android_LogPriority priority = ANDROID_LOG_DEBUG)
    {
...
#else
    static void CALLSTACK_ALWAYS_INLINE
    logStack(const char *logtag, CallStack * = getCurrent().get(),
    ...
    stackToString(const char *prefix = nullptr,
                  const CallStack *stack = getCurrent().get())
    {
        if (reinterpret_cast<uintptr_t>(stackToStringInternal) != 0 &&
            stack != nullptr)
        {
        ...
        {
            return String8::format("%s<CallStack package not linked>",
                                   (prefix ? prefix : ""));
        }
    }
#else  // !CALLSTACK_WEAKS_AVAILABLE
    static String8 CALLSTACK_ALWAYS_INLINE stackToString(
        const char *prefix = nullptr, const CallStack * = getCurrent().get())
    {
        return String8::format("%s<CallStack package not linked>",
                               (prefix ? prefix : ""));
    }
  ...
  private:
    ...
    static void CALLSTACK_WEAK logStackInternal(const char *logtag,
                                                const CallStack *stack,
                                                android_LogPriority priority);
    static String8 CALLSTACK_WEAK stackToStringInternal(const char *prefix,
                                                        const CallStack *stack);

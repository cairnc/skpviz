 ...
 *
 * Consider using std::bitset<32> or std::bitset<64> if all you want is a class
 * to do basic bit manipulation (i.e. AND / OR / XOR / flip / etc). These
 * classes are only needed if you want to efficiently perform operations like
 * finding the first set bit in a bitset and you want to avoid using the
 * built-in functions (e.g. __builtin_clz) on std::bitset::to_ulong.
 */
...
struct BitSet32
{
    ...
    static inline void clear(uint32_t &value)
    {
        value = 0UL;
    }
    ...
    static inline bool isEmpty(uint32_t value)
    {
        return !value;
    }
    ...
    static inline bool isFull(uint32_t value)
    {
    ...
    {
        return hasBit(value, n);
    }

    static inline bool hasBit(uint32_t value, uint32_t n)
    {
    ...
    static inline void markBit(uint32_t &value, uint32_t n)
    {
        value |= valueForBit(n);
    }
    ...
    static inline void clearBit(uint32_t &value, uint32_t n)
    {
        value &= ~valueForBit(n);
    }
    ...
    {
        return firstMarkedBit(value);
    }

    static uint32_t firstMarkedBit(uint32_t value)
    {
        return clz_checked(value);
    }
    ...
    {
        return firstUnmarkedBit(value);
    }

    static inline uint32_t firstUnmarkedBit(uint32_t value)
    {
        return clz_checked(~value);
    }
    ...
    {
        return lastMarkedBit(value);
    }

    static inline uint32_t lastMarkedBit(uint32_t value)
    {
        return 31 - ctz_checked(value);
    }
    ...
    {
        return clearFirstMarkedBit(value);
    }
    ...
    // index. Result is undefined if all bits are marked.
    inline uint32_t markFirstUnmarkedBit()
    {
        return markFirstUnmarkedBit(value);
    }
    ...
    {
        return clearLastMarkedBit(value);
    }
    ...
    static inline uint32_t getIndexOfBit(uint32_t value, uint32_t n)
    {
        return static_cast<uint32_t>(
            __builtin_popcountl(value & ~(0xffffffffUL >> n)));
    }
  ...
  private:
    // We use these helpers as the signature of __builtin_c{l,t}z has "unsigned
    // int" for the input, which is only guaranteed to be 16b, not 32. The
...
struct BitSet64
{
    ...
    // Gets the value associated with a particular bit index.
    static inline uint64_t valueForBit(uint32_t n)
    {
        return 0x8000000000000000ULL >> n;
    }
    ...
    static inline void clear(uint64_t &value)
    {
        value = 0ULL;
    }
    ...
    static inline bool isEmpty(uint64_t value)
    {
        return !value;
    }
    ...
    static inline bool isFull(uint64_t value)
    {
    ...
    static inline bool hasBit(uint64_t value, uint32_t n)
    {
        return value & valueForBit(n);
    }
    ...
    static inline void markBit(uint64_t &value, uint32_t n)
    {
        value |= valueForBit(n);
    }
    ...
    static inline void clearBit(uint64_t &value, uint32_t n)
    {
        value &= ~valueForBit(n);
    }
    ...
    inline bool operator==(const BitSet64 &other) const
    {
        return value == other.value;
    }
    inline bool operator!=(const BitSet64 &other) const

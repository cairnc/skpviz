...
extern "C"
{
                       ...
                       size_t dst_len)
    {
        ...
        auto err_out = [&out, &out_end, &dst_len]()
        {
            LOG_ALWAYS_FATAL_IF(out >= out_end,
                                "target utf8 string size %zu too short",
                                dst_len);
        };
        ...
        while (in < in_end)
        {
            ...
            if (in < in_end && (w2 = *in, is_surrogate_pair(w, w2)))
            {
                if (out + 4 > out_end)
                    return err_out();
                char32_t dw =
                    (char32_t)(0x10000 + ((w - 0xd800) << 10) + (w2 - 0xdc00));
    ...
    static char32_t utf8_4b_to_utf32(uint8_t c1, uint8_t c2, uint8_t c3,
                                     uint8_t c4)
    {
        return ((c1 & 0x07) << 18) | ((c2 & 0x3f) << 12) | ((c3 & 0x3f) << 6) |
               (c4 & 0x3f);
    }
                                 ...
                                 bool overreadIsFatal)
    {
        ...
        if (overreadIsFatal)
            LOG_ALWAYS_FATAL(
                "Attempt to overread computing length of utf8 string");
        return -1;
    }

    char16_t *utf8_to_utf16(const uint8_t *u8str, size_t u8len,
                            char16_t *u16str, size_t u16len)
    {

...
{
    LOG_ALWAYS_FATAL("RefBase used with stack pointer argument");
}

// Check whether address is definitely on the calling stack.  We actually check
// whether it is on the same 4K page as the frame pointer.
//
// Assumptions:
// - Pages are never smaller than 4K (MIN_PAGE_SIZE)
// - Malloced memory never shares a page with a stack.
//
// It does not appear safe to broaden this check to include adjacent pages;
// apparently this code is used in environments where there may not be a guard
// page below (at higher addresses than) the bottom of the stack.
static void check_not_on_stack(const void *ptr)
{
    static constexpr int MIN_PAGE_SIZE =
        0x1000; // 4K. Safer than including sys/user.h.
    static constexpr uintptr_t MIN_PAGE_MASK =
        ~static_cast<uintptr_t>(MIN_PAGE_SIZE - 1);
    uintptr_t my_frame_address = reinterpret_cast<uintptr_t>(
        __builtin_frame_address(0 /* this frame */));
    if (((reinterpret_cast<uintptr_t>(ptr) ^ my_frame_address) &
         MIN_PAGE_MASK) == 0)
    {
...
class RefBase::weakref_impl : public RefBase::weakref_type
{
  public:
    ...
    weakref_impl(RefBase *base)
        : mStrong(INITIAL_STRONG_VALUE), mWeak(0), mBase(base),
          mFlags(OBJECT_LIFETIME_STRONG), mStrongRefs(NULL), mWeakRefs(NULL),
          mTrackEnabled(!!DEBUG_REFS_ENABLED_BY_DEFAULT), mRetain(false)
    {
    }
...
void RefBase::incStrongRequireStrong(const void *id) const
{
    ...
    LOG_ALWAYS_FATAL_IF(
        c <= 0 || c == INITIAL_STRONG_VALUE,
        "incStrongRequireStrong() called on %p which isn't already owned",
        refs);
#if PRINT_REFS
...
void RefBase::weakref_type::incWeak(const void *id)
{
    ...
    impl->addWeakRef(id);
    const int32_t c __unused =
        impl->mWeak.fetch_add(1, std::memory_order_relaxed);
...
void RefBase::weakref_type::incWeakRequireWeak(const void *id)
{
    ...
    const int32_t c __unused =
        impl->mWeak.fetch_add(1, std::memory_order_relaxed);
    LOG_ALWAYS_FATAL_IF(
        c <= 0, "incWeakRequireWeak called on %p which has no weak refs", this);
}

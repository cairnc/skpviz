...
Tokenizer::Tokenizer(const String8 &filename, FileMap *fileMap, char *buffer,
    ...
    : mFilename(filename), mFileMap(fileMap), mBuffer(buffer),
      mOwnBuffer(ownBuffer), mLength(length), mCurrent(buffer), mLineNumber(1)
{
}
...
status_t Tokenizer::open(const String8 &filename, Tokenizer **outTokenizer)
{
    *outTokenizer = nullptr;
    ...
    else
    {
        ...
        {
            result = -errno;
            ALOGE("Error getting size of file '%s': %s", filename.c_str(),
                  strerror(errno));
        }
        else
        {
            ...
            else
            {
                ...
                {
                    result = -errno;
                    ALOGE("Error reading file '%s': %s", filename.c_str(),
                          strerror(errno));
                    delete[] buffer;
                    buffer = nullptr;
                }
            ...
            {
                *outTokenizer =
                    new Tokenizer(filename, fileMap, buffer, ownBuffer, length);
            }
        }
        close(fd);
    }
...
{
    *outTokenizer =
        new Tokenizer(filename, nullptr, const_cast<char *>(contents), false,
                      strlen(contents));
    return OK;
}

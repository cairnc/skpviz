...
VectorImpl::VectorImpl(const VectorImpl &rhs)
    : mStorage(rhs.mStorage), mCount(rhs.mCount), mFlags(rhs.mFlags),
...
VectorImpl &VectorImpl::operator=(const VectorImpl &rhs)
{
    LOG_ALWAYS_FATAL_IF(mItemSize != rhs.mItemSize,
                        "Vector<> have different types (this=%p, rhs=%p)", this,
                        &rhs);
    if (this != &rhs)
    {
        release_storage();
...
ssize_t VectorImpl::insertArrayAt(const void *array, size_t index,
                                  size_t length)
{
    if (index > size())
        return BAD_INDEX;
        ...
        while (i < count)
        {
            ...
            if (cmp(curr, item, state) > 0)
            {
                ...
                if (!temp)
                {
                    ...
                    curr =
                        reinterpret_cast<char *>(array) + mItemSize * (i - 1);
                }
                else
                {
                    _do_destroy(temp, 1);
                ...
                do
                {
                    ...
                    {
                        curr =
                            reinterpret_cast<char *>(array) + mItemSize * (j);
                    }
...
{
    ALOG_ASSERT(index < size(), "[%p] replace: index=%d, size=%d", this,
                (int)index, (int)size());
...
{
    size_t end;
    LOG_ALWAYS_FATAL_IF(__builtin_add_overflow(index, count, &end),
                        "overflow: index=%zu count=%zu", index, count);
    if (end > size())
        return BAD_VALUE;
    _shrink(index, count);
    return index;
...
void *VectorImpl::editItemLocation(size_t index)
{
    ALOG_ASSERT(index < capacity(),
                "[%p] editItemLocation: index=%d, capacity=%d, count=%d", this,
                (int)index, (int)capacity(), (int)mCount);
...
{
    ALOG_ASSERT(index < capacity(),
                "[%p] itemLocation: index=%d, capacity=%d, count=%d", this,
                (int)index, (int)capacity(), (int)mCount);
...
void *VectorImpl::_grow(size_t where, size_t amount)
{
    ...
    ALOG_ASSERT(where <= mCount, "[%p] _grow: where=%d, amount=%d, count=%d",
                this, (int)where, (int)amount,
                (int)mCount); // caller already checked

    size_t new_size;
    LOG_ALWAYS_FATAL_IF(__builtin_add_overflow(mCount, amount, &new_size),
                        "new_size overflow");
    ...
    if (capacity() < new_size)
    {
        ...
        LOG_ALWAYS_FATAL_IF(__builtin_add_overflow(new_capacity,
        ...
        if ((mStorage) && (mCount == where) && (mFlags & HAS_TRIVIAL_COPY) &&
    ...
    else
    {
        ...
        {
            const void *from =
                reinterpret_cast<const uint8_t *>(array) + where * mItemSize;
...
void VectorImpl::_shrink(size_t where, size_t amount)
{
    ...
    ALOG_ASSERT(where + amount <= mCount,
                "[%p] _shrink: where=%d, amount=%d, count=%d", this, (int)where,
                (int)amount, (int)mCount); // caller already checked
    ...
    if (new_size < (prev_capacity / 2) && prev_capacity > kMinVectorCapacity)
    {
        ...
        if ((where == new_size) && (mFlags & HAS_TRIVIAL_COPY) &&
            (mFlags & HAS_TRIVIAL_DTOR))
        {
            const SharedBuffer *cur_sb = SharedBuffer::bufferFromData(mStorage);
            ...
            if (sb)
            {
                ...
                if (where != new_size)
                {
                    const void *from =
                        ...
                        (where + amount) * mItemSize;
                    void *dest =
                        reinterpret_cast<uint8_t *>(array) + where * mItemSize;
...
void VectorImpl::_do_move_forward(void *dest, const void *from,
                                  size_t num) const
{
    do_move_forward(dest, from, num);
}

void VectorImpl::_do_move_backward(void *dest, const void *from,
                                   size_t num) const
{
...
SortedVectorImpl::SortedVectorImpl(const VectorImpl &rhs) : VectorImpl(rhs)
{
}
...
{
    return static_cast<SortedVectorImpl &>(
...
ssize_t SortedVectorImpl::merge(const SortedVectorImpl &vector)
{
    ...
    if (!vector.isEmpty())
    {
        ...
        if (do_compare(vector.itemLocation(vector.size() - 1), arrayImpl()) <=
            0)
        {
            err = VectorImpl::insertVectorAt(
                static_cast<const VectorImpl &>(vector), 0);
        }
        else if (do_compare(vector.arrayImpl(), itemLocation(size() - 1)) >= 0)
        {
            err = VectorImpl::appendVector(
                static_cast<const VectorImpl &>(vector));
        }
        else
        {

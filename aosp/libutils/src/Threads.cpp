                              ...
                              android_thread_id_t *threadId)
{
    ...
    int result =
        pthread_create(&thread, &attr, libutil_thread_trampoline, pthread_arg);
...
int androidCreateRawThreadEtc(android_thread_func_t fn, void *userData,
...
int androidCreateThreadGetID(android_thread_func_t fn, void *arg,
                             android_thread_id_t *id)
{
    return createThreadEtc(fn, arg, "android:unnamed_thread", PRIORITY_DEFAULT,
                           0, id);
}

static android_create_thread_fn gCreateThreadFn = androidCreateRawThreadEtc;

int androidCreateThreadEtc(android_thread_func_t entryFunction, void *userData,
...
typedef struct WinCondition
{
    ...
    status_t wait(WinCondition *condState, HANDLE hMutex, nsecs_t *abstime)
    {
        ...
        bool lastWaiter =
            (condState->wasBroadcast && condState->waitersCount == 0);
        ...
        if (lastWaiter)
        {
            ...
            SignalObjectAndWait(condState->waitersDone,
                                condState->internalMutex, INFINITE, FALSE);
        }
        else
        {
...
Thread::Thread(bool canCallJava)
    : mCanCallJava(canCallJava), mThread(thread_id_t(-1)),
      mLock("Thread::mLock"), mStatus(OK), mExitPending(false), mRunning(false)
#if defined(__ANDROID__)
      ,
      mTid(-1)
#endif
...
{
    LOG_ALWAYS_FATAL_IF(name == nullptr,
                        "thread name not provided to Thread::run");
...
status_t Thread::requestExitAndWait()
{
    ...
    {
        ALOGW("Thread (this=%p): don't call waitForExit() from this "
...
status_t Thread::join()
{
    ...
    if (mThread == getThreadId())
    {
        ALOGW("Thread (this=%p): don't call join() from this "
              "Thread object's thread. It's a guaranteed deadlock!",
              this);

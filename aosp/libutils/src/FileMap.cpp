...
FileMap::FileMap(void)
    : mFileName(nullptr), mBasePtr(nullptr), mBaseLength(0), mDataPtr(nullptr),
      mDataLength(0)
#if defined(__MINGW32__)
      ,
      mFileHandle(INVALID_HANDLE_VALUE), mFileMapping(NULL)
#endif
{
}
...
FileMap::FileMap(FileMap &&other) noexcept
    : mFileName(other.mFileName), mBasePtr(other.mBasePtr),
      mBaseLength(other.mBaseLength), mDataOffset(other.mDataOffset),
      mDataPtr(other.mDataPtr), mDataLength(other.mDataLength)
#if defined(__MINGW32__)
      ,
...
bool FileMap::create(const char *origFileName, int fd, off64_t offset,
                     size_t length, bool readOnly)
{
#if defined(__MINGW32__)
    int adjust;
    off64_t adjOffset;
    ...
    mBasePtr = MapViewOfFile(mFileMapping,
                             readOnly ? FILE_MAP_READ : FILE_MAP_ALL_ACCESS, 0,
                             (DWORD)(adjOffset), adjLength);
    if (mBasePtr == NULL)
    {

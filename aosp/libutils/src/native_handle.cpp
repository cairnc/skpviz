...
{
    return android_fdsan_create_owner_tag(
        ANDROID_FDSAN_OWNER_TYPE_NATIVE_HANDLE,
...
int close_internal(const native_handle_t *h, bool allowUntagged)
{
    ...
    uint64_t tag;
    if (allowUntagged && numFds > 0 &&
        android_fdsan_get_owner_tag(h->data[0]) == 0)
    {
        tag = 0;
    }
    else
...
void swap_fdsan_tags(const native_handle_t *handle, uint64_t expected_tag,
                     uint64_t new_tag)
{
    if (!handle || handle->version != sizeof(native_handle_t))
        return;
    ...
    for (int i = 0; i < handle->numFds; i++)
    {
        ...
        {
            android_fdsan_exchange_owner_tag(handle->data[i], expected_tag,
                                             new_tag);
        }
    }
}

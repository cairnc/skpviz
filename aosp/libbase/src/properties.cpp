...
struct prop_info
{
    ...
    prop_info(const char *key, const char *value)
        : key(key), value(value), serial(0)
    {
    }
};
...
SYSPROP_WEAK int __system_property_set(const char *key, const char *value)
{
    ...
    if (!read_only && strlen(value) >= PROP_VALUE_MAX)
        return -1;
...
SYSPROP_WEAK void __system_property_read_callback(
    const prop_info *pi,
...
static void WaitForPropertyCallback(void *data_ptr, const char *,
                                    const char *value, unsigned serial)
{
    WaitForPropertyData *data =
        reinterpret_cast<WaitForPropertyData *>(data_ptr);
...
static void DurationToTimeSpec(timespec &ts, const std::chrono::milliseconds d)
{
    ...
    ts.tv_sec = std::min<std::chrono::seconds::rep>(
        s.count(), std::numeric_limits<time_t>::max());
    ts.tv_nsec = ns.count();
}
...
static const prop_info *
WaitForPropertyCreation(const std::string &key,
                        ...
                        const AbsTime &start_time)
{
    ...
    while ((pi = __system_property_find(key.c_str())) == nullptr)
    {
        ...
        if (!__system_property_wait(nullptr, global_serial, &global_serial,
                                    &ts))
            return nullptr;
    }
    return pi;
                     ...
                     std::chrono::milliseconds relative_timeout)
{
    ...
    const prop_info *pi =
        WaitForPropertyCreation(key, relative_timeout, start_time);
    if (pi == nullptr)
        return false;
    ...
    while (true)
    {
        ...
        UpdateTimeSpec(ts, relative_timeout, start_time);
        uint32_t unused;
        if (!__system_property_wait(pi, data.last_read_serial, &unused, &ts))
            return false;
    }
}
...
{
    auto start_time = std::chrono::steady_clock::now();
    return (WaitForPropertyCreation(key, relative_timeout, start_time) !=
            nullptr);
}

CachedProperty::CachedProperty(std::string property_name)
    : property_name_(std::move(property_name)), prop_info_(nullptr),
      cached_area_serial_(0), cached_property_serial_(0),
    ...
    if (prop_info_ != nullptr)
    {
        ...
        if (property_serial != cached_property_serial_)
        {
            __system_property_read_callback(
                ...
                {
                    CachedProperty *instance =
                        reinterpret_cast<CachedProperty *>(data);
...
const char *
CachedProperty::WaitForChange(std::chrono::milliseconds relative_timeout)
{
    if (!prop_info_)
    {
        auto start_time = std::chrono::steady_clock::now();
        prop_info_ = WaitForPropertyCreation(property_name_, relative_timeout,
                                             start_time);
        if (!prop_info_)
        {
            return nullptr;
    ...
    else
    {
        ...
        uint32_t new_serial;
        if (!__system_property_wait(prop_info_, old_serial, &new_serial, &ts))
            return nullptr;
    }

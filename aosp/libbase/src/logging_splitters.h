                              ...
                              const F &log_function)
{
    ...
    bool add_file = file != nullptr &&
                    (severity == FATAL || severity == FATAL_WITHOUT_ABORT);
    ...
    auto write_to_logd_chunk = [&](const char *message, int length)
    {
        ...
        {
            size_written =
                snprintf(logd_chunk + chunk_position,
    ...
    else
    {
        ...
        if (add_file)
        {
            snprintf(logd_chunk, sizeof(logd_chunk), "%s%s",
                     file_header.c_str(), msg);
            log_function(log_id, severity, tag, logd_chunk);
        }
        else
        {
...
static std::string StderrOutputGenerator(const struct timespec &ts, int pid,
                                         uint64_t tid, LogSeverity severity,
                                         const char *tag, const char *file,
                                         unsigned int line, const char *message)
{
    ...
    static_assert(
        arraysize(log_characters) - 1 == FATAL + 1,
    ...
    {
        line_prefix =
            StringPrintf("%s %5d %5" PRIu64 " %c %-8s: %s:%u ", timestamp, pid,
                         tid, severity_char, real_tag, file, line);
    }
    else
    {
        line_prefix = StringPrintf("%s %5d %5" PRIu64 " %c %-8s: ", timestamp,
                                   pid, tid, severity_char, real_tag);
    }

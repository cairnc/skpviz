...
ssize_t ReceiveFileDescriptorVector(int sockfd, void *data, size_t len,
                                    size_t max_fds, std::vector<unique_fd> *fds)
{
    return ReceiveFileDescriptorVector(borrowed_fd(sockfd), data, len, max_fds,
                                       fds);
}
#endif
...
MAPPEDFILE_FROMFD(int fd, off64_t offset, size_t length, int prot)
{

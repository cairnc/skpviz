...
static int mkstemp(char *name_template, size_t size_in_chars)
{
    ...
    int fd = _wopen(path.c_str(), O_CREAT | O_EXCL | O_RDWR | O_BINARY,
                    S_IRUSR | S_IWUSR);
    if (fd < 0)
    {
        return -1;
    ...
    CHECK(android::base::WideToUTF8(path, &path_utf8))
...
std::string GetSystemTempDir()
{
    ...
    DWORD result =
        GetTempPathW(std::size(tmp_dir_w), tmp_dir_w); // checks TMP env
    ...
    std::string tmp_dir;
    CHECK(android::base::WideToUTF8(tmp_dir_w, &tmp_dir))
        << "path can't be converted to utf8";
...
bool ReadFdToString(borrowed_fd fd, std::string *content)
{
    ...
    if (fstat(fd.get(), &sb) != -1 && sb.st_size > 0 && sb.st_size <= SSIZE_MAX)
    {
        ...
        else if (fd_size < content->capacity() &&
                 content->capacity() - fd_size >= 64)
        {
                      ...
                      bool follow_symlinks)
{
    ...
    int flags =
        O_RDONLY | O_CLOEXEC | O_BINARY | (follow_symlinks ? 0 : O_NOFOLLOW);
                       ...
                       bool follow_symlinks)
{
    ...
    android::base::unique_fd fd(
        TEMP_FAILURE_RETRY(open(path.c_str(), flags, mode)));
    if (fd == -1)
    {
                       ...
                       bool follow_symlinks)
{
    int flags = O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY |
                (follow_symlinks ? 0 : O_NOFOLLOW);
    android::base::unique_fd fd(
        TEMP_FAILURE_RETRY(open(path.c_str(), flags, 0666)));
    if (fd == -1)
    {
        return false;
    }
...
#if defined(_WIN32)
// Windows implementation of pread. Note that this DOES move the file
// descriptors read position, but it does so atomically.
static ssize_t pread(borrowed_fd fd, void *data, size_t byte_count,
                     off64_t offset)
{
    DWORD bytes_read;
    ...
    {
        // In case someone tries to read errno (since this is masquerading as a
        // POSIX call)
        errno = EIO;
        return -1;
    }
...
static ssize_t pwrite(borrowed_fd fd, const void *data, size_t byte_count,
                      off64_t offset)
{
    DWORD bytes_written;
    OVERLAPPED overlapped;
...
bool ReadFullyAtOffset(borrowed_fd fd, void *data, size_t byte_count,
                       off64_t offset)
{
    uint8_t *p = reinterpret_cast<uint8_t *>(data);
    while (byte_count > 0)
...
bool WriteFullyAtOffset(borrowed_fd fd, const void *data, size_t byte_count,
                        off64_t offset)
{
...
bool RemoveFileIfExists(const std::string &path, std::string *err)
{
    ...
    bool file_type_removable =
        (result == 0 && (S_ISREG(st.st_mode) || S_ISLNK(st.st_mode)));
#endif
    if (result == -1)
    {
...
static int _basename_r(const char *path, size_t path_size, char *buffer,
                       size_t buffer_size)
{
...
static int _dirname_r(const char *path, size_t path_size, char *buffer,
                      size_t buffer_size)
{
    const char *endp = nullptr;
    int len;

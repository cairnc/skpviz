...
template <typename T> std::string Trim(T &&t)
{
    std::string_view sv;
    std::string s;
...
{
    using ElementType =
        typename std::remove_reference_t<ContainerT>::value_type;
    ...
    if constexpr (std::is_convertible_v<ElementType, std::string_view>)
    {
        ...
        if constexpr (sepIsChar)
            sepSize = 1;
        else
            sepSize = std::string_view(separator).size();

        const std::string_view::size_type total = std::accumulate(
            std::next(things.begin()), things.end(),
            std::string_view(*things.begin()).size(),
        ...
        {
            if constexpr (sepIsChar)
                result.append(1, separator).append(*it);
            else
...
// These cases were measured either to be used during build by more than one
// binary, or during runtime as a significant portion of total calls.
...
extern template std::string Join(const std::unordered_set<std::string> &,
                                 const char *);

...
template <typename E = Errno, bool include_message = true> struct ResultError
{
...
template <typename E> auto format_as(ResultError<E, true> error)
{
    return error.message();
}

template <typename E> struct ResultError<E, /* include_message */ false>
{
    template <typename P,
              typename = std::enable_if_t<std::is_convertible_v<P, E>>>
...
{
    template <typename T> DoNothingStream &operator<<(T &&)
    {
        return *this;
    }
...
class Error
{
  public:
    ...
    friend Error ErrorfImpl(fmt::format_string<Args...> fmt,
                            const Args &...args);

    template <typename... Args>
    friend Error ErrnoErrorfImpl(fmt::format_string<Args...> fmt,
                                 const Args &...args);
...
template <typename E> inline E ErrorCode(E code)
{
    return code;
}
...
__attribute__((noinline)) ResultError<Errno>
MakeResultErrorWithCode(std::string &&message, Errno code);

template <typename... Args>
inline ResultError<Errno> ErrorfImpl(fmt::format_string<Args...> fmt,
                                     const Args &...args)
{
...
template <typename... Args>
inline ResultError<Errno> ErrnoErrorfImpl(fmt::format_string<Args...> fmt,
                                          const Args &...args)
{
    Errno code{errno};
    return MakeResultErrorWithCode(
        fmt::vformat(fmt.get(), fmt::make_format_args(args...)), code);
}
...
template <typename T> struct ConversionBase
{
    ErrorType<T> error_;
...
#define SPECIALIZED_CONVERSION(type)                                           \
    operator expected<type, ErrorType<T>>() const &                            \
    {                                                                          \
        return unexpected(this->error_);                                       \
    }                                                                          \
    operator expected<type, ErrorType<T>>() &&                                 \
    {                                                                          \
    ...
    : public impl::NumericConversions<Result<T, E, include_message>>
{
  ...
  public:
    ...
    // templated expected ctor. We don't need this specialization if `C` is
    // numeric because that case is already covered by `NumericConversions`.
    operator Result<std::conditional_t<impl::IsNumeric<C>, impl::Never, C>, E,
                    include_message>() const &
    {
        return unexpected(this->error_);
    }
    operator Result<std::conditional_t<impl::IsNumeric<C>, impl::Never, C>, E,
    ...
    template <impl::Universal U> operator Result<U, E, include_message>() &&
    {
        return unexpected(std::move(this->error_));
    }
#else
    template <typename U> operator Result<U, E, include_message>() const &
    {
        return unexpected(this->error_);
    }
    template <typename U> operator Result<U, E, include_message>() &&
    {
        return unexpected(std::move(this->error_));
    }
#endif

    static const std::string &ErrorMessage(const V &val)
    {
...
// android::base::Result. These also work with base::android::expected. They
// assume the user depends on libgmock and includes gtest/gtest.h. For advanced
// matchers and customized error messages, see result-gmock.h.

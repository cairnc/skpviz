...
#define _EXPLICIT(cond)                                                        \
    _Pragma("clang diagnostic push") _Pragma(                                  \
        "clang diagnostic ignored \"-Wc++20-extensions\"") explicit(cond)      \
...
template <class T> inline constexpr bool is_expected_v = is_expected<T>::value;
...
#define _ASSIGN_UNEXPECTED_TO_EXPECTED(GF, ParamType, forward_func,            \
                                       extra_constraints)                      \
    template <class G _ENABLE_IF(std::is_constructible_v<E, GF>                \
                                     &&std::is_assignable_v<E &, GF>) &&       \
              extra_constraints>                                               \
    constexpr expected &operator=(ParamType e) noexcept(                       \
...
// Class expected
template <class T, class E> class _NODISCARD_ expected
{
    static_assert(std::is_object_v<T> && !std::is_array_v<T> &&
                  ...
                  "expected value type cannot be a reference, a function, an "
                  "array, in_place_t, "
...
#define _CONVERTING_CTOR(UF, GF, ParamType, forward_func)                      \
    template <                                                                 \
        class U,                                                               \
        class G _ENABLE_IF(                                                    \
            expected_internal::convert_value_constraints<T, U, G, UF>          \
                &&expected_internal::convert_error_constraints<E, U, G, GF>)>  \
    constexpr _EXPLICIT(                                                       \
        (!std::is_convertible_v<UF, T> || !std::is_convertible_v<GF, E>))      \
        expected(ParamType rhs) noexcept(                                      \
            std::is_nothrow_constructible_v<T, UF> &&                          \
            std::is_nothrow_constructible_v<E, GF>)                            \
        : var_(rhs.has_value() ? variant_type(std::in_place_index<0>,          \
                                              forward_func(rhs.value()))       \
                               : variant_type(std::in_place_index<1>,          \
                                              forward_func(rhs.error())))      \
    {                                                                          \
    }
...
#undef _CONVERTING_CTOR
    ...
    template <
        class U = T _ENABLE_IF(
            !std::is_same_v<expected_internal::remove_cvref_t<U>,
                            std::in_place_t> &&
            !std::is_same_v<expected_internal::remove_cvref_t<U>, expected> &&
            !expected_internal::is_unexpected_v<
                expected_internal::remove_cvref_t<U>> &&
            std::is_constructible_v<T, U> &&
            (!std::is_same_v<std::remove_cv_t<T>, bool> ||
             !expected_internal::is_expected_v<
                 expected_internal::remove_cvref_t<U>>))>
    ...
    constexpr explicit expected(unexpect_t, Args &&...args) noexcept(
        ...
        : var_(std::in_place_index<1>,
               unexpected_type(std::forward<Args>(args)...))
    {
    }
        ...
        Args
            &&...args) noexcept(std::
                                    is_nothrow_constructible_v<
                                        E, std::initializer_list<U> &, Args...>)
        : var_(std::in_place_index<1>,
               unexpected_type(il, std::forward<Args>(args)...))
    {
    }
    ...
    constexpr expected &
    operator=(U &&v) noexcept(std::is_nothrow_constructible_v<T, U> &&
    ...
    _ASSIGN_UNEXPECTED_TO_EXPECTED(
        const G &, const unexpected<G> &, ,
        (expected_internal::eh_assign_constraints<T, E, E, const G &>))
    _ASSIGN_UNEXPECTED_TO_EXPECTED(
        G, unexpected<G> &&, std::move,
    ...
    template <class U = T _ENABLE_IF(
                  std::is_same_v<U, T> &&std::is_swappable_v<T>
                      &&std::is_swappable_v<E> &&std::is_move_constructible_v<T>
    ...
    {
        return std::get<unexpected_type>(var_).error();
    }
    constexpr E &error() &
    {
        return std::get<unexpected_type>(var_).error();
    ...
    constexpr const E &&error() const &&
    {
        return std::move(std::get<unexpected_type>(var_)).error();
    }
    constexpr E &&error() &&
    {
        return std::move(std::get<unexpected_type>(var_)).error();
...
template <class E> class _NODISCARD_ expected<void, E>
{
  public:
...
#define _CONVERTING_CTOR(GF, ParamType, forward_func)                          \
    template <class U,                                                         \
              class G _ENABLE_IF(std::is_void_v<U> &&expected_internal::       \
                                     convert_error_constraints<E, U, G, GF>)>  \
    constexpr _EXPLICIT((!std::is_convertible_v<GF, E>)) expected(             \
        ParamType rhs) noexcept(std::is_nothrow_constructible_v<E, GF>)        \
        : var_(rhs.has_value()                                                 \
                   ? variant_type(std::in_place_index<0>, std::monostate())    \
                   : variant_type(std::in_place_index<1>,                      \
                                  forward_func(rhs.error())))                  \
    {                                                                          \
    }
...
#undef _CONVERTING_CTOR
    ...
    {
        var_.template emplace<0>(std::monostate());
    }

    // Swap. This function takes a template argument so that _ENABLE_IF works.
    template <class G = E _ENABLE_IF(std::is_same_v<G, E> &&std::is_swappable_v<
                                     E> &&std::is_move_constructible_v<E>)>
    constexpr void
    ...
    constexpr const E &&error() const &&
    {
        return std::move(std::get<1>(var_)).error();
    }
    constexpr E &&error() &&
    {
        return std::move(std::get<1>(var_)).error();
...
template <class T, class E,
          typename = decltype(std::declval<expected<T, E> &>().swap(
              std::declval<expected<T, E> &>()))>
...
template <class E> class unexpected
{
    static_assert(std::is_object_v<E> && !std::is_array_v<E> &&
                      !std::is_const_v<E> && !std::is_volatile_v<E> &&
                      !expected_internal::is_unexpected_v<E>,
                  "unexpected error type cannot be a reference, a function, an "
                  "array, cv-qualified, "
                  "or unexpected");
  ...
  public:
    ...
    template <class Err = E _ENABLE_IF(
                  !std::is_same_v<expected_internal::remove_cvref_t<Err>,
                                  unexpected> &&
                  !std::is_same_v<expected_internal::remove_cvref_t<Err>,
                                  std::in_place_t> &&
    ...
    // Swap. This function takes a template argument so that _ENABLE_IF works.
    template <typename G =
                  E _ENABLE_IF(std::is_same_v<G, E> &&std::is_swappable_v<E>)>

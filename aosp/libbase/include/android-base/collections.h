...
// Helpers for converting a variadic template parameter pack to a homogeneous
// collection. Parameters must be implictly convertible to the contained type
// (including via move/copy ctors).
//
// Use as follows:
//
...
// Assert that all of the arguments in a variadic template parameter pack are of
// a given type after std::decay.
template <typename T, typename Arg, typename... Args> void AssertType(Arg &&)
{

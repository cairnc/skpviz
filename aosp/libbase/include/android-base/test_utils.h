...
#define __LIBBASE_GENERIC_REGEX_SEARCH(__s, __pattern)                         \
    (std::regex_search(                                                        \
        __s,                                                                   \
        std::basic_regex<std::decay<decltype(__s[0])>::type>((__pattern))))
...
#define ASSERT_NOT_MATCH(__string, __pattern)                                  \
    ...
    {                                                                          \
        ...
        if (__LIBBASE_GENERIC_REGEX_SEARCH(__s, (__pattern)))                  \
        {                                                                      \
            FAIL() << "regex mismatch: expected to not find " << (__pattern)   \
                   << " in:\n"                                                 \
                   << __s;                                                     \
        }                                                                      \
    } while (0)
...
#define EXPECT_MATCH(__string, __pattern)                                      \
    ...
    {                                                                          \
        ...
        if (!__LIBBASE_GENERIC_REGEX_SEARCH(__s, (__pattern)))                 \
        {                                                                      \
            ADD_FAILURE() << "regex mismatch: expected " << (__pattern)        \
                          << " in:\n"                                          \
                          << __s;                                              \
        }                                                                      \
    } while (0)
...
#define EXPECT_NOT_MATCH(__string, __pattern)                                  \
    ...
    {                                                                          \
        ...
        if (__LIBBASE_GENERIC_REGEX_SEARCH(__s, (__pattern)))                  \
        {                                                                      \
            ADD_FAILURE() << "regex mismatch: expected to not find "           \
                          << (__pattern) << " in:\n"                           \
                          << __s;                                              \
        }                                                                      \
    } while (0)

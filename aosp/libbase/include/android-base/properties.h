...
// If the property is empty, doesn't exist, doesn't have an integer value, or is
// outside the optional bounds, returns `default_value`.
template <typename T>
...
// If the property is empty, doesn't exist, doesn't have an integer value, or is
// outside the optional bound, returns `default_value`.
template <typename T>
...
// Cached system property lookup. For code that needs to read the same property
// multiple times, this class helps optimize those lookups.
class CachedProperty
{
  public:
    ...
    //
    // Note: this can return the same value multiple times in a row if the
    // property was set to the same value or if multiple changes happened before
    // the current thread was resumed.
    const char *WaitForChange(std::chrono::milliseconds relative_timeout =
                                  std::chrono::milliseconds::max());
...
// Helper class for passing the output of CachedProperty to a parser function,
// and then caching that as well.
template <typename Parser> class CachedParsedProperty
{
  public:
    using value_type =
        std::remove_reference_t<std::invoke_result_t<Parser, const char *>>;
    ...
    // This function is internally-synchronized, so use from multiple threads is
    // safe (but ordering of course cannot be guaranteed without external
    // synchronization).
...
class CachedBoolProperty
{
  public:
    ...
    // Returns the parsed bool, or std::nullopt if it wasn't set or couldn't be
    // parsed.
    std::optional<bool> GetOptional();

    // Returns the parsed bool, or default_value if it wasn't set or couldn't be
    // parsed.
    bool Get(bool default_value);

  private:
    CachedParsedProperty<std::optional<bool> (*)(const char *)>
        cached_parsed_property_;
};

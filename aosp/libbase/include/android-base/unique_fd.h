...
template <typename Closer> class unique_fd_impl final
{
  public:
    unique_fd_impl()
    {
    ...
    explicit unique_fd_impl(int fd)
    {
        reset(fd);
    }
    ~unique_fd_impl()
  ...
  private:
    ...
    template <typename T = Closer> static void tag(long, void *, void *)
    {
        // No-op.
    }
...
template <typename Closer>
inline bool Socketpair(int domain, int type, int protocol,
...
// This helper hides the details of converting to unique_fd.
template <typename Closer>
inline bool Socketpair(int type, unique_fd_impl<Closer> *left,
                       unique_fd_impl<Closer> *right)
{

...
void KernelLogger(LogId log_buffer_id, LogSeverity severity, const char *tag,
                  const char *file, unsigned int line, const char *message);
// Log to stderr in the full logcat format (with pid/tid/time/tag details).
void StderrLogger(LogId log_buffer_id, LogSeverity severity, const char *tag,
                  const char *file, unsigned int line, const char *message);
...
// Useful for replacing printf(3)/perror(3)/err(3)/error(3) in command-line
// tools.
void StdioLogger(LogId log_buffer_id, LogSeverity severity, const char *tag,
                 const char *file, unsigned int line, const char *message);
...
{
    ~LogAbortAfterFullExpr() __attribute__((noreturn))
    {
        abort();
    }
...
#define WOULD_LOG(severity)                                                    \
    (UNLIKELY(::android::base::ShouldLog(SEVERITY_LAMBDA(severity),            \
                                         _LOG_TAG_INTERNAL)) ||                \
...
// Conditionally logs a message based on a specified severity level and a
// boolean condition. Logs to logcat on Android, or to stderr on host. See also
// LOG(severity) above.
//
// The message will only be logged if:
// 1. The provided 'cond' evaluates to true.
// 2. The 'severity' level is enabled for the current log tag (as determined by
// the logging configuration).
//
// Usage:
//
//   LOG_IF(INFO, some_condition) << "This message will be logged if
//   'some_condition' is true" <<
//       " and INFO level is enabled.";
//
// @param severity The severity level of the log message (e.g., VERBOSE, DEBUG,
// INFO, WARNING,
//      ERROR, FATAL).
// @param cond The boolean condition that determines whether to log the message.
#define LOG_IF(severity, cond)                                                 \
    if (UNLIKELY(cond) && WOULD_LOG(severity))                                 \
    LOG(severity)
...
#define LOGGING_PREAMBLE(severity)                                             \
    (WOULD_LOG(severity) &&                                                    \
     ABORT_AFTER_LOG_EXPR_IF(                                                  \
         (SEVERITY_LAMBDA(severity)) == ::android::base::FATAL, true) &&       \
...
#define CHECK(x)                                                               \
    LIKELY((x)) || ABORT_AFTER_LOG_FATAL_EXPR(false) ||                        \
        ::android::base::LogMessage(                                           \
            __FILE__, __LINE__, ::android::base::FATAL, _LOG_TAG_INTERNAL, -1) \
                .stream()                                                      \
            << "Check failed: " #x << " "
...
template <typename T> struct IsCharPointer
{
    ...
    static constexpr bool value =
        std::is_pointer_v<T> && (std::is_same_v<Pointee, char> ||
...
class LogMessage
{
  public:
    ...
    static void LogLine(const char *file, unsigned int line,
                        LogSeverity severity, const char *tag, const char *msg);

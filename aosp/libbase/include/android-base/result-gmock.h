...
/*
 * Matchers for android::base::Result<T> that produces human-readable test
 * results.
 *
 * Example:
 *
 ...
 * EXPECT_THAT(result, HasError(WithCode(AnyOf(EBADF, EACCES))) << "Unexpected
 * code from library";
 */
...
template <> inline void PrintTo(const Result<void> &result, std::ostream *os)
{
    if (result.ok())
    {
        *os << "OK";
...
MATCHER_P(HasValue, value_matcher, "")
{
    if (arg.ok())
    {
        return ::testing::ExplainMatchResult(value_matcher, arg.value(),
                                             result_listener);
    }
    *result_listener << "error is " << arg.error();
    return false;
...
MATCHER_P(WithCode, code_matcher, "")
{
    *result_listener << "actual error is " << arg;
    return ::testing::ExplainMatchResult(code_matcher, arg.code(),
                                         result_listener);
}

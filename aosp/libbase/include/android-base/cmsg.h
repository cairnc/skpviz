...
// Helpers for sending and receiving file descriptors across Unix domain
// sockets.
//
...
//
// These functions wrap the hard-to-use cmsg API with an easier to use
// abstraction.
...
// truncated data. For this reason, using SOCK_SEQPACKET or SOCK_DGRAM is
// recommended.
...
// -1 is returned with errno set to EMSGSIZE, and all received FDs are thrown
// away.
ssize_t ReceiveFileDescriptorVector(borrowed_fd sock, void *data, size_t len,
                                    size_t max_fds,
                                    std::vector<android::base::unique_fd> *fds);

// Helper for SendFileDescriptorVector that constructs a std::vector for you,
// e.g.:
//   SendFileDescriptors(sock, "foo", 3, std::move(fd1), std::move(fd2))
template <typename... Args>
ssize_t SendFileDescriptors(borrowed_fd sock, const void *data, size_t len,
                            Args &&...sent_fds)
{
    // Do not allow implicit conversion to int: people might try to do something
    // along the lines of:
...
// Helper for ReceiveFileDescriptorVector that receives an exact number of file
// descriptors. If more file descriptors are received than requested, -1 is
// returned with errno set to EMSGSIZE. If fewer file descriptors are received
// than requested, -1 is returned with errno set to ENOMSG. In both cases, all
// arguments are cleared and any received FDs are thrown away.
template <typename... Args>
ssize_t ReceiveFileDescriptors(borrowed_fd sock, void *data, size_t len,
                               Args &&...received_fds)
{
    ...
    ssize_t rc =
        ReceiveFileDescriptorVector(sock, data, len, fds.size(), &result);

...
// error, and returning it to the caller. If it is ok then the inner value is
// unwrapped (if applicable) and returned.
//
// Usage with Result<T>:
//
// Result<Foo> getFoo() {...}
...
#define OR_RETURN(expr)                                                        \
    UNWRAP_OR_DO(__or_return_expr, expr,                                       \
...
// Variant for use in gtests, which aborts the test function with an assertion
// failure on error. This is akin to ASSERT_OK_AND_ASSIGN for absl::Status,
// except the assignment is external. It assumes the user depends on libgmock
// and includes gtest/gtest.h.
...
// Generic macro to execute any statement(s) on error. Execution should never
// reach the end of them. result_var is assigned expr and is only visible to
// on_error_stmts.
#define UNWRAP_OR_DO(result_var, expr, on_error_stmts)                         \
    ({                                                                         \
        decltype(expr) &&result_var = (expr);                                  \
        typedef android::base::OkOrFail<                                       \
            std::remove_reference_t<decltype(result_var)>>                     \
            ok_or_fail;                                                        \
...
template <typename T, typename = void> struct OkOrFail
{
    // Checks if T is ok or fail.
    static bool IsOk(const T &);

    // Turns T into the success value.
    template <typename U> static U Unwrap(T &&);

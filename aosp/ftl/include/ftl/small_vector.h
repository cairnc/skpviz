...
template <typename> struct is_small_vector;

// ftl::StaticVector that promotes to std::vector when full. SmallVector is a
// drop-in replacement for std::vector with statically allocated storage for N
// elements, whose goal is to improve run time by avoiding heap allocation and
// increasing probability of cache hits. The standard API is augmented by an
// unstable_erase operation that does not preserve order, and a replace
// operation that destructively emplaces.
//
...
//   assert(!vector.dynamic());
//
//   ftl::SmallVector strings = ftl::init::list<std::string>("abc")("123456",
//   3u)(3u, '?'); assert(strings.size() == 3u); assert(!strings.dynamic());
//
...
class SmallVector final : details::ArrayTraits<T>,
                          details::ArrayComparators<SmallVector>
{
  ...
  public:
    ...
    // Constructs at most N elements. See StaticVector for underlying
    // constructors.
    template <typename Arg, typename... Args,
              typename = std::enable_if_t<
                  !is_small_vector<details::remove_cvref_t<Arg>>{}>>
    ...
    bool dynamic() const
    {
        return std::holds_alternative<Dynamic>(vector_);
    }
...
#define DISPATCH(T, F, ...)                                                    \
    T F() __VA_ARGS__                                                          \
    {                                                                          \
        return dynamic() ? std::get<Dynamic>(vector_).F()                      \
                         : std::get<Static>(vector_).F();                      \
    }
    ...
    {
        return dynamic() ? std::get<Dynamic>(vector_)[i]
                         : std::get<Static>(vector_)[i];
    }

    const_reference operator[](size_type i) const
    {
        return const_cast<SmallVector &>(*this)[i];
    }
    ...
    reference replace(const_iterator it, Args &&...args)
    {
        if (dynamic())
        {
            return std::get<Dynamic>(vector_).replace(
                it, std::forward<Args>(args)...);
        }
        else
        {
            return std::get<Static>(vector_).replace(
                it, std::forward<Args>(args)...);
        }
    }
    ...
    //
    // If the vector reaches its static or dynamic capacity, then all iterators
    // are invalidated. Otherwise, only the end() iterator is invalidated.
    //
    template <typename... Args> reference emplace_back(Args &&...args)
    {
        constexpr auto kInsertStatic = &Static::template emplace_back<Args...>;
        constexpr auto kInsertDynamic =
            &Dynamic::template emplace_back<Args...>;
        return *insert<kInsertStatic, kInsertDynamic>(
            std::forward<Args>(args)...);
    }
    ...
    void push_back(const value_type &v)
    {
        constexpr auto kInsertStatic =
            static_cast<bool (Static::*)(const value_type &)>(
                &Static::push_back);
        constexpr auto kInsertDynamic =
            static_cast<bool (Dynamic::*)(const value_type &)>(
                &Dynamic::push_back);
...
#undef DISPATCH
  ...
  private:
    template <typename, std::size_t> friend class SmallVector;
    ...
    Dynamic &promote(Static &static_vector)
    {
        ...
        std::move(static_vector.begin(), static_vector.end(),
  ...
  private:
    ...
    static Impl convert(SmallVector<U, M> &&other)
    {
        if constexpr (std::is_constructible_v<
                          Impl, std::vector<std::remove_const_t<U>> &&>)
        {
...
template <typename> struct is_small_vector : std::false_type
{
};

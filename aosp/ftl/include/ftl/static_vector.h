...
// Fixed-capacity, statically allocated counterpart of std::vector. Like
// std::array, StaticVector allocates contiguous storage for N elements of type
// T at compile time, but stores at most (rather than exactly) N elements.
// Unlike std::array, its default constructor does not require T to have a
// default constructor, since elements are constructed in place as the vector
// grows. Operations that insert an element (emplace_back, push_back, etc.) fail
// when the vector is full. The API otherwise adheres to standard containers,
// except the unstable_erase operation that does not preserve order, and the
// replace operation that destructively emplaces.
//
// Unlike std::vector, T does not require copy/move assignment, so may be an
// object with const data members, or be const itself.
//
...
//
//   ftl::StaticVector strings = ftl::init::list<std::string>("abc")("123456",
//   3u)(3u, '?'); assert(strings.size() == 3u); assert(strings[0] == "abc");
                           ...
                           details::ArrayComparators<StaticVector>
{
    ...
    template <typename, std::size_t> friend class StaticVector;
  ...
  public:
    ...
    //
    // IteratorRangeTag disambiguates with initialization from two iterator-like
    // elements.
    //
    template <typename Iterator,
              typename = std::enable_if_t<is_input_iterator<Iterator>{}>>
    StaticVector(Iterator first, Iterator last)
        : StaticVector(kIteratorRange, first, last)
    {
        using V = typename std::iterator_traits<Iterator>::value_type;
        static_assert(std::is_constructible_v<value_type, V>,
                      "Incompatible iterator range");
    }

    template <typename Iterator>
    StaticVector(IteratorRangeTag, Iterator first, Iterator last)
        : size_(std::min(max_size(),
    ...
    //
    //   static_assert(std::is_same_v<decltype(vector),
    //   ftl::StaticVector<std::string, 2>>);
    //
    template <
        typename E, typename... Es,
    ...
    // Constructs at most N elements in place by forwarding per-element
    // constructor arguments. The template arguments T and N are inferred using
    // the deduction guide defined below. The syntax for listing arguments is as
    // follows:
    //
    ...
    //
    //   static_assert(std::is_same_v<decltype(vector),
    //   ftl::StaticVector<std::string, 3>>); assert(vector.full());
    //   assert(vector[0] == "abc");
    //   assert(vector[1].empty());
    //   assert(vector[2] == "???");
    //
    template <typename U, std::size_t Size, std::size_t... Sizes,
              typename... Types>
    StaticVector(InitializerList<U, std::index_sequence<Size, Sizes...>,
                                 Types...> &&list)
        : StaticVector(std::index_sequence<0, 0, Size>{},
    ...
    // Replaces an element, and returns a reference to it. The iterator must be
    // dereferenceable, so replacing at end() is erroneous.
    //
    // The element is emplaced via move constructor, so type T does not need to
    // define copy/move assignment, e.g. its data members may be const.
    //
    // The arguments may directly or indirectly refer to the element being
    // replaced.
    //
    // Iterators to the replaced element point to its replacement, and others
    // remain valid.
    //
    ...
    // Appends an element, and returns an iterator to it. If the vector is full,
    // the element is not inserted, and the end() iterator is returned.
    //
    ...
    // Appends an element unless the vector is full, and returns whether the
    ...
    // Removes the last element. The vector must not be empty, or the call is
    // erroneous.
    //
  ...
  private:
    ...
    StaticVector(std::index_sequence<I>, E &&element, Es &&...elements)
        : StaticVector(std::index_sequence<I + 1>{},
                       std::forward<Es>(elements)...)
    {
    ...
    template <std::size_t I, std::size_t ArgIndex, std::size_t ArgCount,
              ...
              typename... Args>
    StaticVector(std::index_sequence<I, ArgIndex, ArgCount>,
                 std::index_sequence<Indices...>,
                 std::index_sequence<Size, Sizes...>,
                 std::tuple<Args...> &tuple)
        : StaticVector(std::index_sequence<I + 1, ArgIndex + ArgCount, Size>{},
                       std::make_index_sequence<Size>{},
                       std::index_sequence<Sizes...>{}, tuple)
    {
        construct_at(begin() + I,
                     std::move(std::get<ArgIndex + Indices>(tuple))...);
    }
...
void StaticVector<T, N>::swap(StaticVector &other)
{
    ...
    // Assume this vector has fewer elements, so the excess of the other vector
    // will be moved to it.
    ...
    // Move elements [min, max) and destroy their source for destructor side
    // effects.
    const auto [first, last] =
        std::make_pair(from->begin() + min, from->begin() + max);

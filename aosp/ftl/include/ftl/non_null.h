...
// Enforces and documents non-null pre/post-condition for (raw or smart)
// pointers.
//
//   void get_length(const ftl::NonNull<std::shared_ptr<std::string>>&
//   string_ptr,
...
//   ftl::as_non_null(std::make_shared<std::string>("android")); std::size_t
//   size; get_length(string_ptr, ftl::as_non_null(&size)); assert(size == 7u);
//
// For compatibility with std::unique_ptr<T> and performance with
// std::shared_ptr<T>, move operations are allowed despite breaking the
// invariant:
//
//   using Pair = std::pair<ftl::NonNull<std::shared_ptr<int>>,
//   std::shared_ptr<int>>;
//
//   Pair dupe_if(ftl::NonNull<std::unique_ptr<int>> non_null_ptr, bool
//   condition) {
//     // Move the underlying pointer out, so `non_null_ptr` must not be
//     accessed after this point. auto unique_ptr =
//     std::move(non_null_ptr).take();
//
//     auto non_null_shared_ptr =
//     ftl::as_non_null(std::shared_ptr<int>(std::move(unique_ptr))); auto
//     nullable_shared_ptr = condition ? non_null_shared_ptr.get() : nullptr;
//
...
//
template <typename Pointer> class NonNull final
{
    struct Passkey
    {
    };
  ...
  public:
    ...
    template <typename U,
              typename = std::enable_if_t<std::is_convertible_v<U, Pointer>>>
    ...
    [[nodiscard]] constexpr explicit operator const Pointer &() const
    {
        return get();
    }
    ...
    [[nodiscard]] constexpr Pointer take() &&
    {
        return std::move(pointer_);
    }
    [[nodiscard]] constexpr explicit operator Pointer() &&
    {
        return take();
    }
    ...
    // Private constructor for ftl::as_non_null. Excluded from candidate
    // constructors for conversions through the passkey idiom, for clear
    // compilation errors.
    template <typename P>
...
template <typename P>
[[nodiscard]] constexpr auto as_non_null(P &&pointer)
    -> NonNull<std::decay_t<P>>
{
...
template <typename P> struct std::hash<android::ftl::NonNull<P>>
{

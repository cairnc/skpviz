...
// Concise alternative to std::visit that compiles to branches rather than a
// dispatch table. For std::variant<T0, ..., TN> where N is small, this is
// slightly faster since the branches can be inlined unlike the function
// pointers.
//
...
//                     },
//                     [](const hours& h) { return std::to_string(h.count() /
//                     24) + " days"s; }));
//
...
decltype(auto) match(const std::variant<Ts...> &variant, Ms &&...matchers)
{
    const auto matcher = details::Matcher{std::forward<Ms>(matchers)...};
    static_assert(
        details::is_exhaustive_match_v<decltype(matcher), const Ts &...>,
        "Non-exhaustive match");

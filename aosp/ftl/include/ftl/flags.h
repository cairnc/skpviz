...
/* A class for handling flags defined by an enum or enum class in a type-safe
 * way. */
template <typename F> class Flags
{
    // F must be an enum or its underlying type is undefined. Theoretically we
    // could specialize this further to avoid this restriction but in general we
    // want to encourage the use of enums anyways.
  ...
  public:
    ...
    explicit constexpr Flags(
        T t, std::enable_if_t<is_scoped_enum_v<F>, T> * = nullptr)
        : mFlags(t)
    {
    }
    ...
    class Iterator
    {
      ...
      public:
        ...
        Iterator(Flags<F> flags) : mRemainingFlags(flags.mFlags)
        {
            (*this)++;
        }
        ...
        Iterator &operator++()
        {
            ...
            else
            {
                ...
                const Bits bit = static_cast<Bits>(
                    __builtin_ctzll(mRemainingFlags.to_ullong()));
        ...
        {
            return mCurrFlag == other.mCurrFlag &&
                   mRemainingFlags == other.mRemainingFlags;
        }
    ...
    bool any(Flags<F> f = ~Flags<F>()) const
    {
        return (mFlags & f.mFlags) != 0;
    }

    /* Tests whether all of the given flags are set */
    bool all(Flags<F> f) const
    {
        return (mFlags & f.mFlags) == f.mFlags;
    }
    ...
    Flags<F> operator&(Flags<F> rhs) const
    {
        return static_cast<F>(mFlags & rhs.mFlags);
    }
    Flags<F> &operator&=(Flags<F> rhs)
    ...
    Flags<F> operator~()
    {
        return static_cast<F>(~mFlags);
    }

    bool operator==(Flags<F> rhs) const
    {
  ...
  private:
    ...
    static void appendFlag(std::string &str, const std::string_view &flag,
                           bool &first)
    {
        if (first)
        {
...
// This namespace provides operator overloads for enum classes to make it easier
// to work with them as flags. In order to use these, add them via a `using

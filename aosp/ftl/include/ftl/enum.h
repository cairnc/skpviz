...
// Returns the name of enumerator E::V and optionally the class (i.e. "E::V" or
// "V") as std::optional<std::string_view> by parsing the compiler-generated
// string literal for the signature of this function. The function is defined in
// the global namespace with a short name and inferred return type to reduce
// bloat in the read-only data segment.
template <bool S, typename E, E V> constexpr auto ftl_enum_builder()
{
    ...
    //
    //   auto ftl_enum() [E = android::test::Enum, V =
    //   android::test::Enum::kValue]
    //
    std::string_view view = __PRETTY_FUNCTION__;
    const auto template_begin = view.rfind('[');
    const auto template_end = view.rfind(']');
    if (template_begin == view.npos || template_end == view.npos)
        return R{};

    // Extract the template parameters without the enclosing brackets. Example
    // (cont'd):
    //
...
// Returns the name of enumerator E::V (i.e. "V") as
// std::optional<std::string_view>
template <typename E, E V> constexpr auto ftl_enum()
{
    return ftl_enum_builder<false, E, V>();
}

// Returns the name of enumerator and class E::V (i.e. "E::V") as
// std::optional<std::string_view>
template <typename E, E V> constexpr auto ftl_enum_full()
{
...
// Trait for determining whether a type is specifically a scoped enum or not. By
// definition, a scoped enum is one that is not implicitly convertible to its
// underlying type.
//
...
// Traits for retrieving an enum's range. An enum specifies its range by
// defining enumerators named ftl_first and ftl_last. If omitted, ftl_first
// defaults to 0, whereas ftl_last defaults to N - 1 where N is the bit width of
// the underlying type, but only if that type is unsigned, assuming the
// enumerators are flags. Also, note that unscoped enums must define both
// bounds, as casting out-of- range values results in undefined behavior if the
...
//
template <typename E, typename = void> struct enum_begin
{
    static_assert(is_scoped_enum_v<E>, "Missing ftl_first enumerator");
    static constexpr E value{0};
};

template <typename E> struct enum_begin<E, std::void_t<decltype(E::ftl_first)>>
{
    static constexpr E value = E::ftl_first;
};

template <typename E> inline constexpr E enum_begin_v = enum_begin<E>::value;

template <typename E, typename = void> struct enum_end
{
    using U = std::underlying_type_t<E>;
    static_assert(is_scoped_enum_v<E> && std::is_unsigned_v<U>,
                  "Missing ftl_last enumerator");

    static constexpr E value{std::numeric_limits<U>::digits};
};

template <typename E> struct enum_end<E, std::void_t<decltype(E::ftl_last)>>
{
    static constexpr E value = E{to_underlying(E::ftl_last) + 1};
};

template <typename E> inline constexpr E enum_end_v = enum_end<E>::value;

template <typename E>
inline constexpr E enum_last_v = E{to_underlying(enum_end_v<E>) - 1};

template <typename E> struct enum_size
{
...
template <auto V> struct Identity
{
...
using make_enum_sequence =
    std::make_integer_sequence<std::underlying_type_t<E>, enum_size_v<E>>;

template <typename E, template <E> class = Identity,
          typename = make_enum_sequence<E>>
struct EnumRange;
...
//   assert(string == "ABC??F");
//
template <typename E> constexpr auto enum_range()
{
...
//
template <auto V> constexpr std::string_view enum_name_full()
{
...
//
template <typename E> constexpr std::optional<std::string_view> enum_name(E v)
{
...
//
template <typename E> constexpr std::optional<std::string_view> flag_name(E v)
{
    const auto value = to_underlying(v);
...
//
template <typename E> inline std::string enum_string(E v)
{
...
// Returns a stringified enumerator with class, or its integral value if not
// named.
//
//   enum class E { A, B, C, F = 5, ftl_last = F };
//
...
//   assert(ftl::enum_string(E{3}) == "3");
//
template <typename E> inline std::string enum_string_full(E v)
{
...
//
template <typename E> inline std::string flag_string(E v)
{

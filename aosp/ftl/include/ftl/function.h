...
// ftl::Function<F, N> is a container for function object, and can mostly be
// used in place of std::function<F>.
//
// Unlike std::function<F>, a ftl::Function<F, N>:
//
//  * Uses a static amount of memory (controlled by N), and never any dynamic
//  allocation.
//  * Satisfies the std::is_trivially_copyable<> trait.
//  * Satisfies the std::is_trivially_destructible<> trait.
//
// However those same limits are also required from the contained function
// object in turn.
//
...
//
// A ftl::Function<F, N> can always be implicitly converted to a larger size
// ftl::Function<F, M>. Trying to convert the other way leads to a compilation
// error.
//
// A default-constructed ftl::Function is in an empty state. The operator bool()
// overload returns false in this state. It is undefined behavior to attempt to
// invoke the function in this state.
//
// The ftl::Function<F, N> can also be constructed or assigned from ftl::no_op.
// This sets up the ftl::Function to be non-empty, with a function that when
// called does nothing except default-constructs a return value.
//
// The ftl::make_function() helpers construct a ftl::Function<F, N>, including
// deducing the values of F and N from the arguments it is given.
//
// The static ftl::Function<F, N>::make() helpers construct a ftl::Function<F,
// N> without that deduction, and also allow for implicit argument conversion if
// the target being called needs them.
//
// The construction helpers allow any of the following types of functions to be
// stored:
//
//  * Any SMALL function object (as defined by the C++ Standard), such as a
//  lambda with a small
//    capture, or other "functor". The requirements are:
//
//      1) The function object must be trivial to destroy (in fact, the
//      destructor will never
//         actually be called once copied to the internal storage).
//      2) The function object must be trivial to copy (the raw bytes will be
//      copied as the
//         ftl::Function<F, N> is copied/moved).
//      3) The size of the function object cannot be larger than
//      sizeof(std::intptr_t) * (N + 1),
//         and it cannot require stricter alignment than alignof(std::intptr_t).
//
//    With the default of N=0, a lambda can only capture a single pointer-sized
//    argument. This is enough to capture `this`, which is why N=0 is the
//    default.
//
//  * A member function, with the address passed as the template value argument
//  to the construction
//    helper function, along with the instance pointer needed to invoke it
//    passed as an ordinary argument.
//
//        ftl::make_function<&Class::member_function>(this);
//
//    Note that the indicated member function will be invoked non-virtually. If
//    you need it to be invoked virtually, you should invoke it yourself with a
//    small lambda like so:
//
//        ftl::function([this] { virtual_member_function(); });
//
//  * An ordinary function ("free function"), with the address of the function
//  passed as a template
//    value argument.
//
//        ftl::make_function<&std::atoi>();
//
//   As with the member function helper, as the function is known at compile
//   time, it will be called directly.
//
// Example usage:
//
//   class MyClass {
...
template <typename F, std::size_t N = 0> class Function;
...
// Detects that a type is a `ftl::Function<F, N>` regardless of what `F` and `N`
// are.
...
{
    // Enforce a valid size, with an arbitrary maximum allowed size for the
    // container of sizeof(std::intptr_t) * 16, though that maximum can be
    // relaxed.
  ...
  public:
    // Defining result_type allows ftl::Function to be substituted for
    // std::function.
    ...
    // Constructing or assigning from nullptr_t also creates an empty
    // ftl::Function.
    ...
    Function &operator=(NoOpTag)
    {
        return *this = Function(no_op);
    }

    // Constructing/assigning from a function object stores a copy of that
    ...
    Function &operator=(const F &f) noexcept
    {
        return *this = Function{
                   OpaqueStorageTraits::opaque_copy(f),
                   details::bind_opaque_function_object<F, Ret, Args...>(f)};
    }

    // Constructing/assigning from a smaller ftl::Function is allowed, but not
    // anything else.
    ...
    {
        return std::invoke(function_, opaque_.data(),
                           std::forward<Args>(args)...);
    }

    // Creation helper for function objects, such as lambdas.
    template <typename F> static auto make(const F &f) -> decltype(Function{f})
    {
        return Function{f};
    }

    // Creation helper for a class pointer and a compile-time chosen member
    // function to call.
    template <auto MemberFunction, typename Class>
    static auto make(Class *instance)
    ...
    static auto make()
        -> decltype(Function{details::bind_free_function<FreeFunction>(
            static_cast<Ret (*)(Args...)>(nullptr))})
    {
        return Function{details::bind_free_function<FreeFunction>(
            static_cast<Ret (*)(Args...)>(nullptr))};
    }

  private:
    // Needed so a Function<F, M> can be converted to a Function<F, N>.
    template <typename, std::size_t> friend class Function;

    // The function pointer type of function stored in `function_`. The first
    // argument is always
    // `&opaque_`.
...
template <typename F> auto make_function(const F &f) -> decltype(Function{f})
{
    return Function{f};
}

// Makes a ftl::Function given a `MemberFunction` and a instance pointer to the
// associated `Class`.
template <auto MemberFunction, typename Class>
auto make_function(Class *instance)
    -> decltype(Function{details::bind_member_function<MemberFunction>(
        instance,
        static_cast<details::remove_member_function_pointer_t<MemberFunction>
                        *>(nullptr))})
{
    return Function{details::bind_member_function<MemberFunction>(
        instance,

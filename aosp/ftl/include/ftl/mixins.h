...
// CRTP mixins for defining type-safe wrappers that are distinct from their
// underlying type. Common uses are IDs, opaque handles, and physical
// quantities. The constructor is provided by (and must be inherited from) the
// `Constructible` mixin, whereas operators (equality, ordering, arithmetic,
...
//
// Unlike `Constructible`, `DefaultConstructible` allows default construction.
// The default value is zero-initialized unless specified:
//
...
//
//   struct Timeout : ftl::DefaultConstructible<Timeout, std::chrono::seconds,
//   10>,
...
//
template <typename Self, typename T> struct Constructible
{
  ...
  private:
    template <typename, template <typename> class> friend class details::Mixin;
...
template <typename Self> struct Equatable : details::Mixin<Self, Equatable>
{
...
template <typename Self> struct Orderable : details::Mixin<Self, Orderable>
{
    constexpr bool operator<(const Self &other) const
    {
        return to_underlying(this->self()) < to_underlying(other);

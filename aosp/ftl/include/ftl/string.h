...
template <typename T> struct to_chars_length
{
...
// Lightweight (not allocating nor sprintf-based) alternative to std::to_string
// for integers, with optional radix. See also ftl::to_string below.
//
...
std::string_view to_chars(char (&buffer)[N], T v, Radix radix = Radix::kDec)
{
    ...
    auto begin = buffer + 2;
    const auto [end, err] =
        std::to_chars(begin, buffer + N, v, static_cast<int>(radix));
    assert(err == std::errc());
...
// Lightweight (not sprintf-based) alternative to std::to_string for integers,
// with optional radix.
//

...
// Lightweight (not allocating nor sprintf-based) concatenation. The variadic
// arguments can be values of integral type (including bool and char), string
// literals, or strings whose length is constrained:
//
...
//
template <std::size_t, typename... Ts> struct Concat;
...
template <std::size_t N> struct Concat<N>
{
    ...
    {
        return static_cast<std::size_t>(end_ - buffer_);
    }
...
// Deduction guide.
template <typename... Ts> Concat(Ts &&...) -> Concat<0, Ts...>;

template <std::size_t N> constexpr auto truncated(std::string_view v)
{

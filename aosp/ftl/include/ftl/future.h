...
// Thin wrapper around FutureImpl<T> (concretely std::future<T> or
// std::shared_future<T>) with extensions for pure values (created via
// ftl::yield) and continuations.
//
// See also SharedFuture<T> shorthand below.
//
template <typename T, template <typename> class FutureImpl = std::future>
class Future final
    : public details::BaseFuture<Future<T, FutureImpl>, T, FutureImpl>
{
    using Base = details::BaseFuture<Future, T, FutureImpl>;

    friend Base; // For BaseFuture<...>::self.
    friend details::BaseFuture<Future<T>, T,
                               std::future>; // For BaseFuture<...>::share.
  ...
  public:
    ...
    bool valid() const
    {
        return std::holds_alternative<T>(future_) ||
               std::get<FutureImpl<T>>(future_).valid();
    }

    // Forwarding functions. Base::share is only defined when FutureImpl is
    // std::future, whereas the following are defined for either FutureImpl:
    using Base::get;
    using Base::wait_for;
  ...
  private:
    template <typename V> friend Future<V> yield(V &&);

    template <typename V, typename... Args> friend Future<V> yield(Args &&...);
...
template <typename T> using SharedFuture = Future<T, std::shared_future>;
...
//   assert(*future.get() == '!');
//
template <typename V> inline Future<V> yield(V &&value)
{
    return {details::ValueTag{}, std::move(value)};
}

template <typename V, typename... Args> inline Future<V> yield(Args &&...args)
{
...
//
template <typename F, typename... Args> inline auto defer(F &&f, Args &&...args)
{
    return Future(std::async(std::launch::deferred, std::forward<F>(f),

...
// Associative container with unique, unordered keys. Unlike std::unordered_map,
// key-value pairs are stored in contiguous storage for cache efficiency. The
// map is allocated statically until its size exceeds N, at which point mappings
// are relocated to dynamic memory. The try_emplace operation has a non-standard
// analogue try_replace that destructively emplaces. The API also defines an
// in-place counterpart to insert_or_assign: emplace_or_replace. Lookup is done
// not via a subscript operator, but immutable getters that can optionally
// transform the value.
//
...
//   assert(map.get(42).transform([](const std::string& s) { return s.size(); })
//   == 3u);
//
...
//   assert(map.dynamic());
//
//   assert(map == SmallMap(ftl::init::map(-1, "xyz"sv)(0, "nil"sv)(42,
//   "???"sv)(123, "abc"sv)));
//
...
class SmallMap final
{
    ...
    template <typename, typename, std::size_t, typename> friend class SmallMap;
  ...
  public:
    ...
    //   static_assert(std::is_same_v<decltype(map), ftl::SmallMap<int, char,
    //   3>>);
    //
    ...
    auto get(const key_type &key) const
        -> Optional<std::reference_wrapper<const mapped_type>>
    {
    ...
    auto get(const key_type &key)
        -> Optional<std::reference_wrapper<mapped_type>>
    {
        for (auto &[k, v] : *this)
        {
            if (KeyEqual{}(k, key))
    ...
    std::pair<iterator, bool> try_emplace(const key_type &key, Args &&...args)
    {
        ...
        decltype(auto) ref_or_it = map_.emplace_back(
    ...
    //
    // The arguments may directly or indirectly refer to the mapping being
    // replaced.
    //
    // Iterators to the replaced mapping point to its replacement, and others
    // remain valid.
    //
    template <typename... Args>
    ...
    // On emplace, if the map reaches its static or dynamic capacity, then all
    // iterators are invalidated. Otherwise, only the end() iterator is
    // invalidated. On replace, iterators to the replaced mapping point to its
    // replacement, and others remain valid.
    //
    template <typename... Args>
    std::pair<iterator, bool> emplace_or_replace(const key_type &key,
                                                 Args &&...args)
    {
  ...
  private:
    ...
    {
        return std::find_if(first, end(), [&key](const auto &pair)
                            { return KeyEqual{}(pair.first, key); });
    }
...
template <typename K, typename V, typename E, std::size_t... Sizes,
          typename... Types>
SmallMap(InitializerList<KeyValue<K, V, E>, std::index_sequence<Sizes...>,
                         Types...> &&) -> SmallMap<K, V, sizeof...(Sizes), E>;

// Returns whether the key-value pairs of two maps are equal.
template <typename K, typename V, std::size_t N, typename Q, typename W,
          std::size_t M, typename E>
bool operator==(const SmallMap<K, V, N, E> &lhs,
                const SmallMap<Q, W, M, E> &rhs)
{
    ...
    for (const auto &[k, v] : lhs)
    {
        const auto &lv = v;
        if (!rhs.get(k)
                 .transform([&lv](const W &rv) { return lv == rv; })
                 .value_or(false))
        {
            return false;
        }
...
template <typename K, typename V, std::size_t N, typename Q, typename W,
          std::size_t M, typename E>
inline bool operator!=(const SmallMap<K, V, N, E> &lhs,
                       const SmallMap<Q, W, M, E> &rhs)
{
    return !(lhs == rhs);
}

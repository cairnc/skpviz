...
// Returns whether static_cast<R>(v) is safe, or would result in underflow or
// overflow.
//
//   static_assert(ftl::cast_safety<uint8_t>(-1) ==
//   ftl::CastSafety::kUnderflow); static_assert(ftl::cast_safety<int8_t>(128u)
//   == ftl::CastSafety::kOverflow);
//
//   static_assert(ftl::cast_safety<uint32_t>(-.1f) ==
//   ftl::CastSafety::kUnderflow);
//   static_assert(ftl::cast_safety<int32_t>(static_cast<float>(INT32_MAX)) ==
//                                           ftl::CastSafety::kOverflow);
//
//   static_assert(ftl::cast_safety<float>(-DBL_MAX) ==
//   ftl::CastSafety::kUnderflow);
//
template <typename R, typename T> constexpr CastSafety cast_safety(T v)
{
    ...
    if constexpr ((kFromSigned == kToSigned &&
                   max_exponent<R> >= max_exponent<T>) ||
                  (!kFromSigned && kToSigned &&
                   max_exponent<R> > max_exponent<T>))
    {
    ...
    if constexpr (kFromSigned)
    {
        ...
        else
        {
            ...
            if (v < 0)
                return CastSafety::kUnderflow;
            return static_cast<C>(v) <= static_cast<C>(L::max())
                       ? CastSafety::kSafe

...
// Superset of std::optional<T> with monadic operations, as proposed in
...
template <typename T> struct Optional final : std::optional<T>
{
    ...
    template <typename F> constexpr auto transform(F &&f) const &
    {
        ...
        if (has_value())
            return R(std::invoke(std::forward<F>(f), value()));
        return R();
    }
    ...
    template <typename F> constexpr auto transform(F &&f) const &&
    {
        ...
        if (has_value())
            return R(std::invoke(std::forward<F>(f), std::move(value())));
        return R();
    }
    ...
    // Returns this Optional<T> if not nullopt, or else the Optional<T> returned
    // by the function F.
    template <typename F>
    ...
    template <typename E> constexpr auto ok_or(E &&e) && -> base::expected<T, E>
    {
        if (has_value())
...
{
    return static_cast<std::optional<T>>(lhs) ==
           static_cast<std::optional<U>>(rhs);
}
...
template <typename T> Optional(std::optional<T>) -> Optional<T>;

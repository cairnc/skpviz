...
public:
  ...
  // Describes the states of the release fence. Checking the states allows
  // checks to ensure that set_value() is not called on the same promise
  // multiple times, and can indicate if the promise has been fulfilled.
  ...
  // Returns the LayerSettings to pass to RenderEngine::drawLayers. The state
  // may contain shadows casted by the layer or the content of the layer itself.
  // If the layer does not render then an empty optional will be returned.
  virtual std::optional<LayerSettings>
  prepareClientComposition(ClientCompositionTargetSettings &) const = 0;

  // Initializes a promise for a buffer release fence and provides the future
  // for that fence. This should only be called when a promise has not yet been
  // created, or after the previous promise has already been fulfilled.
  // Attempting to call this when an existing promise is INITIALIZED will fail
...
struct LayerFESpHash {
  size_t operator()(const sp<LayerFE> &p) const {
    return std::hash<LayerFE *>()(p.get());
  }
};
...
static inline bool
operator==(const LayerFE::ClientCompositionTargetSettings &lhs,
...
static inline void
PrintTo(const LayerFE::ClientCompositionTargetSettings &settings,
        ::std::ostream *os) {
...
static inline void PrintTo(const LayerFE::LayerSettings &settings,
                           ::std::ostream *os) {

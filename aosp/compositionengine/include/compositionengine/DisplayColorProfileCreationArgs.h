...
struct DisplayColorProfileCreationArgs {
  using HwcColorModes =
      std::unordered_map<ui::ColorMode, std::vector<ui::RenderIntent>>;
...
public:
  ...
  setHasWideColorGamut(bool hasWideColorGamut) {
    mArgs.hasWideColorGamut = hasWideColorGamut;
    return *this;
  }
  DisplayColorProfileCreationArgsBuilder &
  setHdrCapabilities(HdrCapabilities &&hdrCapabilities) {
    mArgs.hdrCapabilities = std::move(hdrCapabilities);
    return *this;
  }
  DisplayColorProfileCreationArgsBuilder &
  setSupportedPerFrameMetadata(int32_t supportedPerFrameMetadata) {

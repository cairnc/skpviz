...
public:
  ...
  using UniqueFELayerStateMap =
      std::unordered_map<LayerFE *, LayerFECompositionState *>;

  // A helper class for enumerating the output layers using a C++11 ranged-based
  // for loop
  template <typename T> class OutputLayersEnumerator {
  public:
    // TODO(lpique): Consider turning this into a C++20 view when possible.
    template <bool IsConstIter> class IteratorImpl {
    public:
      // Required definitions to be considered an iterator
      using iterator_category = std::forward_iterator_tag;
      using value_type =
          decltype(std::declval<T>().getOutputLayerOrderedByZByIndex(0));
      ...
      IteratorImpl(const T *output, size_t index)
          : mOutput(output), mIndex(index) {}
    ...
    auto end() const {
      return iterator(&mOutput, mOutput.getOutputLayerCount());
    }
    auto cbegin() const { return const_iterator(&mOutput, 0); }
    auto cend() const {
      return const_iterator(&mOutput, mOutput.getOutputLayerCount());
    }
  ...
  struct CoverageState {
    explicit CoverageState(LayerFESet &latchedLayers)
        : latchedLayers(latchedLayers) {}
    ...
    Region dirtyRegion;
    // The region of the output which is covered by layers, excluding display
    // overlays. This only has a value if there's something needing it, like
    // when a TrustedPresentationListener is set
  ...
  virtual void setProjection(ui::Rotation orientation,
  ...
  // Gets the transform hint used in layers that belong to this output. Used to
  // guide composition orientation so that HW overlay can be used when display
  // isn't in its natural orientation on some devices. Therefore usually we only
  // use transform hint from display output.
  ...
  virtual void setDisplayBrightness(float sdrWhitePointNits,
                                    float displayBrightnessNits) = 0;

  // Outputs a string with a state dump
  virtual void dump(std::string &) const = 0;

  // Outputs planner information
  virtual void dumpPlannerInfo(const Vector<String16> &args,
                               std::string &) const = 0;
  ...
  // Returns whether the output includes a layer, based on their respective
  // filters. See Output::setLayerFilter.
  ...
  auto getOutputLayersOrderedByZ() const {
    return OutputLayersEnumerator(*this);
  }
  ...
  present(const CompositionRefreshArgs &) = 0;
...
protected:
  ...
  virtual void rebuildLayerStacks(const CompositionRefreshArgs &,
                                  LayerFESet &) = 0;
  virtual void collectVisibleLayers(const CompositionRefreshArgs &,
                                    CoverageState &) = 0;
  ...
  virtual std::optional<base::unique_fd>
  ...
  virtual void applyCompositionStrategy(
      const std::optional<android::HWComposer::DeviceRequestedChanges>
          &changes) = 0;
  virtual bool getSkipColorTransform() const = 0;
  virtual FrameFences presentFrame() = 0;
  virtual void executeCommands() = 0;
  virtual std::vector<LayerFE::LayerSettings>
  generateClientCompositionRequests(bool supportsProtectedContent,
  ...
  virtual void
  setHintSessionGpuFence(std::unique_ptr<FenceTime> &&gpuFence) = 0;
  virtual void
  setHintSessionRequiresRenderEngine(bool requiresRenderEngine) = 0;
  virtual bool isPowerHintSessionEnabled() = 0;
  virtual bool isPowerHintSessionGpuReportingEnabled() = 0;
  virtual void cacheClientCompositionRequests(uint32_t cacheSize) = 0;
  virtual bool
  canPredictCompositionStrategy(const CompositionRefreshArgs &) = 0;
  virtual const aidl::android::hardware::graphics::composer3::
      OverlayProperties *
      getOverlaySupport() = 0;

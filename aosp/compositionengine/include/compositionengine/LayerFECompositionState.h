...
inline bool operator==(const GenericLayerMetadataEntry &lhs,
                       const GenericLayerMetadataEntry &rhs) {
...
using GenericLayerMetadataMap =
    std::unordered_map<std::string, GenericLayerMetadataEntry>;
...
struct LayerFECompositionState {
  ...
  // A picture profile handle refers to a PictureProfile configured on the
  // display, which is a set of parameters that configures the picture
  // processing hardware that is used to enhance the quality of buffer contents.
  PictureProfileHandle pictureProfileHandle{PictureProfileHandle::NONE};

  // A layer's priority in terms of limited picture processing pipeline
  // utilization.

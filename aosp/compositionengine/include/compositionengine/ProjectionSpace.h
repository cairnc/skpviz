...
public:
  ...
  ProjectionSpace(ui::Size size, Rect content)
      : mBounds(size), mContent(std::move(content)) {}
  ...
  ui::Transform getTransform(const ProjectionSpace &destination) const {
    ...
    // Compute a transformation which rotates the destination in a way it has
    // the same orientation as us.
    const uint32_t inverseRotationFlags =
        ui::Transform::toRotationFlags(-rotation);
    ui::Transform inverseRotatingTransform;
    inverseRotatingTransform.set(inverseRotationFlags,
                                 destination.getBounds().width,
                                 destination.getBounds().height);
    // The destination content rotated so it has the same orientation as us.
    Rect orientedDestContent =
        inverseRotatingTransform.transform(destination.getContent());
    ...
    // Compute scaling transform which maps source content to destination
    // content, assuming they are both at (0, 0).
    ui::Transform scale;
    const float scaleX =
        static_cast<float>(orientedDestContent.width()) / mContent.width();
    const float scaleY =
        static_cast<float>(orientedDestContent.height()) / mContent.height();
    ...
    rotationTransform.set(orientationFlags, orientedDestWidth,
                          orientedDestHeight);

    // The layerStackSpaceRect and orientedDisplaySpaceRect are both in the
    // logical orientation. Apply the logical translation, scale to physical
    // size, apply the physical translation and finally rotate to the physical
    // orientation.
  ...
  void setOrientation(ui::Rotation newOrientation) {
    mOrientation = newOrientation;
  }

  Rect getBoundsAsRect() const {
    return Rect(mBounds.getWidth(), mBounds.getHeight());
  }
...
private:
  ...
  // The orientation of this space. This value is meaningful only in relation to
  // the rotation of another projection space and it's used to determine the
  // rotating transformation when mapping between the two. As a convention when
  // using this struct orientation = 0 for the "oriented*" projection spaces.
  // For example when the display is rotated 90 degress counterclockwise, the
  // orientation of the display space will become 90, while  the orientation of
  // the layer stack space will remain the same.
...
inline std::string to_string(const compositionengine::ProjectionSpace &space) {
  return base::StringPrintf(
      "ProjectionSpace{bounds=%s, content=%s, orientation=%s}",
      to_string(space.getBoundsAsRect()).c_str(),
      to_string(space.getContent()).c_str(), toCString(space.getOrientation()));
}

// Defining PrintTo helps with Google Tests.
inline void PrintTo(const compositionengine::ProjectionSpace &space,
                    std::ostream *os) {
  *os << to_string(space);
}

...
// The buffer cache returns both a slot and the buffer that should be sent to
// HWC. In cases where the buffer is already cached, the buffer is a nullptr and
// will not be sent to HWC as an optimization.
...
private:
  static const constexpr size_t kMaxLayerBufferCount =
      BufferQueue::NUM_BUFFER_SLOTS;

public:
  // public for testing
  // Override buffers don't use the normal cache slots because we don't want
  // them to evict client buffers from the cache. We add an extra slot at the
  // end for the override buffers.
  ...
  //
  // If the buffer is already in the cache, the buffer is null to optimize away
  // sending HWC the buffer handle.
  //
  ...
  //
  // When a client process discards a buffer, it needs to be purged from the HWC
  // cache.
  //
  // Returns the slot number of the buffer, or UINT32_MAX if it wasn't found in
  // the cache.
  //

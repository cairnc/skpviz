...
private:
  ...
  static ColorModeKey getColorModeKey(ui::Dataspace dataspace,
                                      ui::RenderIntent intent) {
    return (static_cast<uint64_t>(dataspace) << 32) |
           static_cast<uint32_t>(intent);
  }
...
createDisplayColorProfile(const DisplayColorProfileCreationArgs &);

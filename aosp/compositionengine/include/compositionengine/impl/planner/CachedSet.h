...
public:
  ...
  public:
    ...
    int32_t getBackgroundBlurRadius() const {
      return mState->getBackgroundBlurRadius();
    }
    Rect getDisplayFrame() const { return mState->getDisplayFrame(); }
    const Region &getVisibleRegion() const {
      return mState->getVisibleRegion();
    }
    const sp<GraphicBuffer> &getBuffer() const {
      return mState->getOutputLayer()
          ->getLayerFE()
          .getCompositionState()
          ->buffer;
    }
    int64_t getFramesSinceBufferUpdate() const {
      return mState->getFramesSinceBufferUpdate();
    }
  ...
  CachedSet(const LayerState *,
            std::chrono::steady_clock::time_point lastUpdate);
  CachedSet(Layer layer);

  void addLayer(const LayerState *,
                std::chrono::steady_clock::time_point lastUpdate);
  ...
  Rect getTextureBounds() const {
    return mTexture ? mTexture->get()->getBuffer()->getBounds()
                    : Rect::INVALID_RECT;
  }
  ...
  // Decomposes this CachedSet into a vector of its layers as individual
  // CachedSets
...
private:
  ...
  // TODO(b/190411067): This is a shared pointer only because CachedSets are
  // copied into different containers in the Flattener. Logically this should
  // have unique ownership otherwise.

...
// While it is possible to define a texture pool supporting variable-sized
// textures to save on memory, it is a simpler implementation to only manage
// screen-sized textures. The texture pool is unbounded - there are a minimum
// number of textures preallocated. Under heavy system load, new textures may be
// allocated, but only a maximum number of retained once those textures are no
// longer necessary.
class TexturePool {
public:
  // RAII class helping with managing textures from the texture pool
  // Textures once they're no longer used should be returned to the pool instead
  // of outright deleted.
  class AutoTexture {
  public:
    ...
    const std::shared_ptr<renderengine::ExternalTexture> &get() const {
      return mTexture;
    }
  ...
  // If the pool is currently starved of textures, then a new texture is
  // generated. When the AutoTexture object is destroyed, the scratch texture is

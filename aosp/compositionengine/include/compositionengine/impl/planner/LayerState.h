...
namespace std {
template <typename T> struct hash<android::sp<T>> {
  size_t operator()(const android::sp<T> &p) {
    return std::hash<void *>()(p.get());
  }
};

template <typename T> struct hash<android::wp<T>> {
...
// An abstract interface allows us to iterate over all of the OutputLayerState
// fields without having to worry about their templated types. See
// `LayerState::getNonUniqueFields` below.
class StateInterface {
public:
  virtual ~StateInterface() = default;

  virtual ftl::Flags<LayerStateField>
  update(const compositionengine::OutputLayer *layer) = 0;
  ...
  virtual ftl::Flags<LayerStateField>
  getFieldIfDifferent(const StateInterface *other) const = 0;
...
public:
  using ReadFromLayerState =
      std::function<T(const compositionengine::OutputLayer *layer)>;
  ...
  static ToStrings getHalToStrings() {
    return [](const T &value) {
      return std::vector<std::string>{toString(value)};
    };
  }
  ...
  ftl::Flags<LayerStateField>
  update(const compositionengine::OutputLayer *layer) override {
  ...
  ftl::Flags<LayerStateField>
  getFieldIfDifferent(const StateInterface *other) const override {
    if (other->getField() != FIELD) {
      return {};
    }

    // The early return ensures that this downcast is sound
    const OutputLayerState *otherState =
        static_cast<const OutputLayerState *>(other);
  ...
  std::vector<std::string> toStrings() const override {
    return mToStrings(mValue);
  }

  bool operator==(const OutputLayerState &other) const {
    return mEquals(mValue, other.mValue);
  }
  bool operator!=(const OutputLayerState &other) const {
    return !(*this == other);
  }
...
public:
  ...
  // The hash is only computed from NonUniqueFields, and excludes GraphicBuffers
  // since they are not guaranteed to live longer than the LayerState object.
  size_t getHash() const;

  // Returns the bit-set of differing fields between this LayerState and another
  // LayerState. This bit-set is based on NonUniqueFields only, and excludes
  // GraphicBuffers.
  ...
  compositionengine::OutputLayer *getOutputLayer() const {
    return mOutputLayer;
  }
  ...
  int32_t getBackgroundBlurRadius() const {
    return mBackgroundBlurRadius.get();
  }
  ...
  int64_t getFramesSinceBufferUpdate() const {
    return mFramesSinceBufferUpdate;
  }
  ...
  float getFps() const {
    return getOutputLayer()->getLayerFE().getCompositionState()->fps;
  }
...
private:
  ...
  OutputLayerState<std::string, LayerStateField::Name> mName{
      [](auto layer) { return layer->getLayerFE().getDebugName(); },
      [](const std::string &name) { return std::vector<std::string>{name}; }};

  // Output-dependent geometry state

  OutputLayerState<Rect, LayerStateField::DisplayFrame> mDisplayFrame{
      [](auto layer) { return layer->getState().displayFrame; },
      [](const Rect &rect) {
        return std::vector<std::string>{base::StringPrintf(
            "[%d, %d, %d, %d]", rect.left, rect.top, rect.right, rect.bottom)};
      }};

  OutputLayerState<FloatRect, LayerStateField::SourceCrop> mSourceCrop{
      [](auto layer) { return layer->getState().sourceCrop; },
      [](const FloatRect &rect) {
        return std::vector<std::string>{
            base::StringPrintf("[%.2f, %.2f, %.2f, %.2f]", rect.left, rect.top,
                               rect.right, rect.bottom)};
      }};

  using BufferTransformState =
      OutputLayerState<hardware::graphics::composer::hal::Transform,
                       LayerStateField::BufferTransform>;
  BufferTransformState mBufferTransform{
      [](auto layer) { return layer->getState().bufferTransform; },
      BufferTransformState::getHalToStrings()};

  // Output-independent geometry state

  using BlendModeState =
      OutputLayerState<hardware::graphics::composer::hal::BlendMode,
                       LayerStateField::BlendMode>;
  BlendModeState mBlendMode{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->blendMode;
      },
      BlendModeState::getHalToStrings()};

  OutputLayerState<float, LayerStateField::Alpha> mAlpha{[](auto layer) {
    return layer->getLayerFE().getCompositionState()->alpha;
  }};

  using LayerMetadataState =
      OutputLayerState<GenericLayerMetadataMap, LayerStateField::LayerMetadata>;
  LayerMetadataState mLayerMetadata{
      [](auto layer) {
      ...
      [](const GenericLayerMetadataMap &metadata) {
        ...
        for (const auto &[key, value] : metadata) {
          size_t entryHash = 0;
          hashCombineSingleHashed(entryHash, std::hash<std::string>{}(key));
          hashCombineSingleHashed(entryHash,
                                  GenericLayerMetadataEntry::Hasher{}(value));
          hash ^= entryHash;
        }
        return hash;
      }};
  ...
  using VisibleRegionState =
      OutputLayerState<Region, LayerStateField::VisibleRegion>;
  VisibleRegionState mVisibleRegion{
      [](auto layer) { return layer->getState().visibleRegion; },
      VisibleRegionState::getRegionToStrings(),
      VisibleRegionState::getRegionEquals()};

  using DataspaceState =
      OutputLayerState<ui::Dataspace, LayerStateField::Dataspace>;
  DataspaceState mOutputDataspace{
      [](auto layer) { return layer->getState().dataspace; },
      DataspaceState::getHalToStrings()};

  // Output-independent per-frame state

  using PixelFormatState =
      OutputLayerState<hardware::graphics::composer::hal::PixelFormat,
                       LayerStateField::PixelFormat>;
  PixelFormatState mPixelFormat{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->buffer
                   ? static_cast<
                         hardware::graphics::composer::hal::PixelFormat>(
                         layer->getLayerFE()
                             .getCompositionState()
  ...
  using CompositionTypeState = OutputLayerState<
      aidl::android::hardware::graphics::composer3::Composition,
      LayerStateField::CompositionType>;
  CompositionTypeState mCompositionType{
      [](auto layer) {
  ...
  OutputLayerState<void *, LayerStateField::SidebandStream> mSidebandStream{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->sidebandStream.get();
      },
      [](void *p) {
        return std::vector<std::string>{base::StringPrintf("%p", p)};
      }};

  static auto constexpr BufferEquals =
      [](const wp<GraphicBuffer> &lhs, const wp<GraphicBuffer> &rhs) -> bool {
    // Avoid a promotion if the wp<>'s aren't equal
    if (lhs != rhs)
      return false;

    // Even if the buffer didn't change, check to see if we need to act as if
    // the buffer changed anyway. Specifically, look to see if the buffer is
    // FRONT_BUFFER & if so act as if it's always different
    using ::aidl::android::hardware::graphics::common::BufferUsage;
    sp<GraphicBuffer> promotedBuffer = lhs.promote();
    return !(promotedBuffer &&
             ((promotedBuffer->getUsage() &
               static_cast<int64_t>(BufferUsage::FRONT_BUFFER)) != 0));
  };

  OutputLayerState<wp<GraphicBuffer>, LayerStateField::Buffer> mBuffer{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->buffer;
      },
      [](const wp<GraphicBuffer> &buffer) {
        ...
        return std::vector<std::string>{base::StringPrintf(
            "%p", promotedBuffer ? promotedBuffer.get() : nullptr)};
      },
      BufferEquals};

  // Even if the same buffer is passed to BLAST's setBuffer(), we still
  // increment the frame number and need to treat it as if the buffer changed.
  // Otherwise we break existing front-buffer rendering paths (such as egl's
  // EGL_SINGLE_BUFFER).
  OutputLayerState<uint64_t, LayerStateField::Buffer> mFrameNumber{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->frameNumber;
      }};

  int64_t mFramesSinceBufferUpdate = 0;

  OutputLayerState<half4, LayerStateField::SolidColor> mSolidColor{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->color;
      },
      [](const half4 &vec) {
  ...
  OutputLayerState<int32_t, LayerStateField::BackgroundBlurRadius>
      mBackgroundBlurRadius{[](auto layer) {
      ...
      [](const std::vector<BlurRegion> &regions) {
        ...
        for (const auto region : regions) {
          ...
          base::StringAppendF(&str,
                              ...
                              "%f, %f], alpha=%f, rect=[%d, "
                              "%d, %d, %d]",
                              region.blurRadius, region.cornerRadiusTL,
                              region.cornerRadiusTR, region.cornerRadiusBL,
                              region.cornerRadiusBR, region.alpha, region.left,
  ...
  OutputLayerState<bool, LayerStateField::HasProtectedContent> mIsProtected{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->hasProtectedContent;
      }};

  OutputLayerState<gui::CachingHint, LayerStateField::CachingHint> mCachingHint{
      [](auto layer) {
  ...
  OutputLayerState<bool, LayerStateField::DimmingEnabled> mIsDimmingEnabled{
      [](auto layer) {
        return layer->getLayerFE().getCompositionState()->dimmingEnabled;
      }};
  ...
  std::array<const StateInterface *, kNumNonUniqueFields>
  getNonUniqueFields() const {
    return {&mDisplayFrame,   &mSourceCrop,      &mBufferTransform,
            &mBlendMode,      &mAlpha,           &mLayerMetadata,
            &mVisibleRegion,  &mOutputDataspace, &mPixelFormat,
            &mColorTransform, &mCompositionType, &mSidebandStream,
            &mBuffer,         &mSolidColor,      &mBackgroundBlurRadius,
            &mBlurRegions,    &mFrameNumber,     &mIsProtected,
            &mCachingHint,    &mIsDimmingEnabled};
  }
};

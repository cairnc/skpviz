...
public:
  ...
  struct Tunables {
    // Tunables that are specific to scheduling when a cached set should be
    // rendered
    struct RenderScheduling {
      // This default assumes that rendering a cached set takes about 3ms. That
      // time is then cut in half - the next frame using the cached set would
      // have the same workload, meaning that composition cost is the same. This
      // is best illustrated with the following example:
      //
      // Suppose we're at a 120hz cadence so SurfaceFlinger is budgeted 8.3ms
      // per-frame. If renderCachedSets costs 3ms, then two consecutive frames
      // have timings:
      //
      ...
      //
      // Now the second frame won't render a cached set afterwards, but the
      // first frame didn't really steal time from the second frame.
      static const constexpr std::chrono::nanoseconds
          kDefaultCachedSetRenderDuration = 1500us;

      static const constexpr size_t kDefaultMaxDeferRenderAttempts = 240;

      // Duration allocated for rendering a cached set. If we don't have enough
      // time for rendering a cached set, then rendering is deferred to another
      // frame.
      const std::chrono::nanoseconds cachedSetRenderDuration;
      // Maximum of times that we defer rendering a cached set. If we defer
      // rendering a cached set too many times, then render it anyways so that
      // future frames would benefit from the flattened cached set.
      const size_t maxDeferRenderAttempts;
    };

    static const constexpr std::chrono::milliseconds
        kDefaultActiveLayerTimeout = 150ms;

    static const constexpr bool kDefaultEnableHolePunch = true;

    // Threshold for determing whether a layer is active. A layer whose
    // properties, including the buffer, have not changed in at least this time
    // is considered inactive and is therefore a candidate for flattening.
  ...
  // CachedSets that contain no more than this many layers may be considered
  // inactive on the basis of FPS.
  static constexpr int kNumLayersFpsConsideration = 1;
  // Frames/Second threshold below which these CachedSets may be considered
  // inactive.
  ...
  NonBufferHash flattenLayers(const std::vector<const LayerState *> &layers,
                              NonBufferHash,
  ...
  const std::optional<CachedSet> &getNewCachedSetForTesting() const {
    return mNewCachedSet;
  }

private:
  size_t
  calculateDisplayCost(const std::vector<const LayerState *> &layers) const;

  void resetActivities(NonBufferHash,
                       std::chrono::steady_clock::time_point now);
  ...
  // A Run is a sequence of CachedSets, which is a candidate for flattening into
  // a single CachedSet. Because it is wasteful to flatten 1 CachedSet, a run
  // must contain more than 1 CachedSet or be used for a hole punch.
  class Run {
  public:
    ...
    public:
      ...
      // The provided length must be the size of the next sequential CachedSet
      // in layers
      ...
      std::optional<Run> validateAndBuild() {
        ...
        if (!requiresHolePunch) {
          // If we don't require a hole punch, then treat solid color layers at
          // the front to be "cheap", so remove them from the candidate cached
          // set.
          ...
          // Only allow for single cached sets if a hole punch is required. If
          // we're here, then we don't require a hole punch, so don't build a
          // run.
          if (mNumSets <= 1) {
            return std::nullopt;
          }
    ...
    const std::vector<CachedSet>::const_iterator &getStart() const {
      return mStart;
    }
    ...
    const CachedSet *getHolePunchCandidate() const {
      return mHolePunchCandidate;
    }
  ...
  std::vector<Run>
  findCandidateRuns(std::chrono::steady_clock::time_point now) const;

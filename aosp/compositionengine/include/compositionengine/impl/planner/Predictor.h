...
class LayerStack {
public:
  LayerStack(const std::vector<const LayerState *> &layers)
      : mLayers(copyLayers(layers)) {}
  ...
  struct ApproximateMatch {
    ...
    // This implies that only one layer is allowed to differ in an approximate
    // match.
    size_t differingIndex;
    // Set of fields that differ for the differing layer in the approximate
    // match.
    ftl::Flags<LayerStateField> differingFields;
  };

  // Returns an approximate match when comparing this layer stack with the
  // provided list of layers, for the purposes of scoring how closely the two
  // layer stacks will match composition strategies.
  //
  // If the two layer stacks are identical, then an approximate match is still
  // returned, but the differing fields will be empty to represent an exact
  // match.
  //
  // If the two layer stacks differ by too much, then an empty optional is
  // returned.
  std::optional<ApproximateMatch>
  getApproximateMatch(const std::vector<const LayerState *> &other) const;

  void compare(const LayerStack &other, std::string &result) const {
    if (mLayers.size() != other.mLayers.size()) {
      base::StringAppendF(
          &result, "Cannot compare stacks of different sizes (%zd vs. %zd)\n",
    ...
    for (size_t l = 0; l < mLayers.size(); ++l) {
      ...
      base::StringAppendF(&result, "\n+ - - - - - - - - - Layer %d [%s]\n",
                          thisLayer.getId(), thisLayer.getName().c_str());
  ...
  void dump(std::string &result) const {
    for (const LayerState &layer : mLayers) {
      base::StringAppendF(&result, "+ - - - - - - - - - Layer %d [%s]\n",
                          layer.getId(), layer.getName().c_str());
      layer.dump(result);
    }
  }

  void dumpLayerNames(std::string &result,
...
public:
  ...
  void
  addLayerType(aidl::android::hardware::graphics::composer3::Composition type) {
...
namespace std {
template <> struct hash<android::compositionengine::impl::planner::Plan> {
  size_t operator()(
      const android::compositionengine::impl::planner::Plan &plan) const {
...
private:
  struct Stats {
    void dump(std::string &result) const {
      ...
      base::StringAppendF(&result, "%.2f%% (%zd/%zd)",
                          100.0f * hitCount / totalAttempts, hitCount,
                          totalAttempts);
    }
  ...
  Stats &getStatsForType(Type type) {
    return const_cast<Stats &>(
        const_cast<const Prediction *>(this)->getStatsForType(type));
  }
...
public:
  struct PredictedPlan {
    ...
    friend bool operator==(const PredictedPlan &lhs, const PredictedPlan &rhs) {
      return lhs.hash == rhs.hash && lhs.plan == rhs.plan &&
             lhs.type == rhs.type;
    }
  };
  ...
  //
  // If the exact layer stack has previously been seen by the predictor, then
  // report the plan used for that layer stack.
  //
  // Otherwise, try to match to the best approximate stack to retireve the most
  // likely plan.
  std::optional<PredictedPlan>
  getPredictedPlan(const std::vector<const LayerState *> &layers,
                   NonBufferHash hash) const;

  // Records a comparison between the predicted plan and the resulting plan,
  // alongside the layer stack we used.
  //
  // This method is intended to help with scoring how effective the prediction
  // engine is.
  ...
  void compareLayerStacks(NonBufferHash leftHash, NonBufferHash rightHash,
                          std::string &) const;
  void describeLayerStack(NonBufferHash, std::string &) const;
  void listSimilarStacks(Plan, std::string &) const;

private:
  // Retrieves a prediction from either the main prediction list or from the
  // candidate list
  ...
  std::optional<NonBufferHash>
  getApproximateMatch(const std::vector<const LayerState *> &layers) const;

  void promoteIfCandidate(NonBufferHash);
  void recordPredictedResult(PredictedPlan,
                             const std::vector<const LayerState *> &layers,
                             Plan result);
  bool findSimilarPrediction(const std::vector<const LayerState *> &layers,
                             Plan result);
  ...
  decltype(mCandidates)::const_iterator
  getCandidateEntryByHash(NonBufferHash hash) const {
    const auto candidateMatches = [&](const PromotionCandidate &candidate) {
      return candidate.hash == hash;
    };

    return std::find_if(mCandidates.cbegin(), mCandidates.cend(),
                        candidateMatches);
  }

...
struct OutputCompositionState {
  ...
  // If true, the current frame reused the buffer from a previous client
  // composition
  ...
  // The common space for all layers in the layer stack. layerStackSpace.content
  // is the Rect which gets projected on the display. The orientation of this
  // space is always ROTATION_0.
  ProjectionSpace layerStackSpace;

  // Oriented physical display space. It will have the same size as displaySpace
  // oriented to match the orientation of layerStackSpace. The orientation of
  // this space is always ROTATION_0.
  ProjectionSpace orientedDisplaySpace;

  // The space of the framebuffer. Its bounds match the size of the framebuffer
  // and its orientation matches the orientation of the display. Typically the
  // framebuffer space will be identical to the physical display space.
  ProjectionSpace framebufferSpace;

  // The space of the physical display. It is as big as the currently active
  // display mode. The content in this space can be rotated.
  ...
  std::optional<android::HWComposer::DeviceRequestedChanges>
      previousDeviceRequestedChanges{};
  ...
  aidl::android::hardware::graphics::composer3::DimmingStage
      clientTargetDimmingStage{
          aidl::android::hardware::graphics::composer3::DimmingStage::NONE};

  // Display brightness that will take effect this frame.
  // This is slightly distinct from nits, in that nits cannot be passed to hw
  // composer.

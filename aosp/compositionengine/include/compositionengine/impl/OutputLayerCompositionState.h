...
struct OutputLayerCompositionState {
  ...
  // The portion of the layer that is obscured by all layers on top. This
  // includes transparent and opaque.
  Region coveredRegion;

  // The portion of the layer that is obscured by all layers on top excluding
  // display overlays. This only has a value if there's something needing it,
  // like when a TrustedPresentationListener is set.
  ...
  // Overrides the buffer, acquire fence, and display frame stored in
  ...
  struct {
    ...
    // True when this layer's blur has been cached with a previous layer, so
    // that this layer does not need to request blurring.
    // TODO(b/188816867): support blur regions too, which are less likely to be
    // common if a device supports cross-window blurs. Blur region support
    // should be doable, but we would need to make sure that layer caching works
    // well with the blur region transform passed into RenderEngine

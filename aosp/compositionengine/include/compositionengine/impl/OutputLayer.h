...
public:
  ...
  void writeStateToHWC(bool includeGeometry, bool skipLayer, uint32_t z,
                       bool zIsOverridden, bool isPeekingThrough) override;
  ...
  void
  applyDeviceLayerRequest(Hwc2::IComposerClient::LayerRequest request) override;
  ...
  getOverrideCompositionSettings() const override;

  void dump(std::string &) const override;
  virtual FloatRect
  calculateOutputSourceCrop(uint32_t internalDisplayRotationFlags) const;
...
private:
  ...
  void writeOutputDependentGeometryStateToHWC(
      HWC2::Layer *, aidl::android::hardware::graphics::composer3::Composition,
      uint32_t z);
  void writeOutputIndependentGeometryStateToHWC(HWC2::Layer *,
  ...
  void writeOutputIndependentPerFrameStateToHWC(
      ...
      bool skipLayer);
  void writeSolidColorStateToHWC(HWC2::Layer *,
                                 const LayerFECompositionState &);
  void writeSidebandStateToHWC(HWC2::Layer *, const LayerFECompositionState &);
  void writeBufferStateToHWC(HWC2::Layer *, const LayerFECompositionState &,
                             bool skipLayer);
  void writeCompositionTypeToHWC(
      HWC2::Layer *, aidl::android::hardware::graphics::composer3::Composition,
...
std::unique_ptr<BaseOutputLayer>
createOutputLayerTemplated(const Output &output, sp<LayerFE> layerFE) {
...
#pragma clang diagnostic ignored "-Wunused-local-typedef"
    ...
    using OutputLayerCompositionState =
        std::remove_const_t<std::remove_reference_t<
            decltype(std::declval<BaseOutputLayer>().getState())>>;
    using Output = std::remove_const_t<std::remove_reference_t<
        decltype(std::declval<BaseOutputLayer>().getOutput())>>;
    using LayerFE = std::remove_reference_t<
        decltype(std::declval<BaseOutputLayer>().getLayerFE())>;
...
createOutputLayer(const compositionengine::Output &, const sp<LayerFE> &);

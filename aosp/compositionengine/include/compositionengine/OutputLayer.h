...
public:
  ...
  // longer cares about.
  virtual void
  uncacheBuffers(const std::vector<uint64_t> &bufferIdsToUncache) = 0;

  // Get the relative priority of the layer's picture profile with respect to
  // the importance of the visual content to the user experience. Lower is
  // higher priority.
  ...
  virtual void
  applyDeviceLayerRequest(Hwc2::IComposerClient::LayerRequest request) = 0;

  // Applies a HWC device layer lut
  virtual void applyDeviceLayerLut(
      ndk::ScopedFileDescriptor,
      std::vector<std::pair<int, aidl::android::hardware::graphics::composer3::
                                     LutProperties>>) = 0;

  // Returns true if the composition settings scale pixels
  virtual bool needsFiltering() const = 0;

  // Returns LayerSettings to be used by RenderEngine if the layer has been
  // overridden during the composition process
  virtual std::optional<LayerFE::LayerSettings>
  getOverrideCompositionSettings() const = 0;

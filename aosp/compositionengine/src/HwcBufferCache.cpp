...
HwcSlotAndBuffer
HwcBufferCache::getHwcSlotAndBuffer(const sp<GraphicBuffer> &buffer) {
  if (auto i = mCacheByBufferId.find(buffer->getId());
      i != mCacheByBufferId.end()) {
    Cache &cache = i->second;
    // mark this cache slot as more recently used so it won't get evicted
    // anytime soon
...
HwcSlotAndBuffer
HwcBufferCache::getOverrideHwcSlotAndBuffer(const sp<GraphicBuffer> &buffer) {

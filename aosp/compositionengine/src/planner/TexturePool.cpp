...
void TexturePool::returnTexture(
  ...
  // Or the texture on the floor if the pool is no longer tracking textures of
  // the same size.
  if (static_cast<int32_t>(texture->getBuffer()->getWidth()) !=
          mSize.getWidth() ||
      static_cast<int32_t>(texture->getBuffer()->getHeight()) !=
          mSize.getHeight()) {
    ALOGV("Deallocating texture from Planner's pool - display size changed "
          "(previous: (%dx%d), "
          "current: (%dx%d))",
          texture->getBuffer()->getWidth(), texture->getBuffer()->getHeight(),
  ...
  if (mPool.size() == kMaxPoolSize) {
    ALOGD("Deallocating texture from Planner's pool - max size [%" PRIu64
          "] reached",
...
std::shared_ptr<renderengine::ExternalTexture> TexturePool::genTexture() {
  LOG_ALWAYS_FATAL_IF(!mSize.isValid(),
                      "Attempted to generate texture with invalid size");
  return std::make_shared<renderengine::impl::ExternalTexture>(
      sp<GraphicBuffer>::make(
          static_cast<uint32_t>(mSize.getWidth()),
          static_cast<uint32_t>(mSize.getHeight()), HAL_PIXEL_FORMAT_RGBA_8888,
          1U,
          static_cast<uint64_t>(GraphicBuffer::USAGE_HW_RENDER |
...
void TexturePool::dump(std::string &out) const {
  base::StringAppendF(&out,
                      "TexturePool (%s) has %zu buffers of size [%" PRId32
                      ", %" PRId32 "]\n",
                      mEnabled ? "enabled" : "disabled", mPool.size(),

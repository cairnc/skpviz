...
std::optional<LayerStack::ApproximateMatch> LayerStack::getApproximateMatch(
  ...
  for (size_t i = 0; i < mLayers.size(); ++i) {
    ...
    // Skip layers where both are client-composited, since that doesn't change
    ...
    // If layers are not identical, but we already detected a prior approximate
    // match for a previous layer, the LayerStacks differ by too much, so return
    // nothing
    if (approximateMatch) {
      return std::nullopt;
    }
    ...
    ftl::Flags<LayerStateField> differingFields =
        mLayers[i].getDifferingFields(*other[i]);

    // If we don't find an approximate match on this layer, then the LayerStacks
    // differ by too much, so return nothing
  ...
  // If we make it through the layer-by-layer comparison without an approximate
  // match, it means that all layers were either identical or had
  // client-composited layers in common, which don't affect the composition
  // strategy, so return a successful result with no differences.
  return ApproximateMatch{
      .differingIndex = 0,
      .differingFields = {},
  };
...
std::optional<Plan> Plan::fromString(const std::string &string) {
  ...
  for (char c : string) {
    ...
    case 'B':
      ...
      continue;
    case 'S':
      plan.addLayerType(aidl::android::hardware::graphics::composer3::
...
std::string to_string(const Plan &plan) {
  ...
  for (auto type : plan.mLayerTypes) {
    ...
    case aidl::android::hardware::graphics::composer3::Composition::SOLID_COLOR:
      result.append("S");
      break;
    case aidl::android::hardware::graphics::composer3::Composition::
        DISPLAY_DECORATION:
...
std::optional<Predictor::PredictedPlan>
Predictor::getPredictedPlan(const std::vector<const LayerState *> &layers,
  ...
  if (std::optional<Plan> exactMatch = getExactMatch(hash); exactMatch) {
    ...
    return PredictedPlan{
        .hash = hash, .plan = *exactMatch, .type = Prediction::Type::Exact};
  }

  // If only a hash was passed in for a layer stack with a cached set, don't
  // perform approximate matches and return early
  if (layers.empty()) {
  ...
  if (std::optional<NonBufferHash> approximateMatch =
          getApproximateMatch(layers);
      approximateMatch) {
    ALOGV("[%s] Found an approximate match for %zx", __func__,
          *approximateMatch);
...
void Predictor::recordResult(std::optional<PredictedPlan> predictedPlan,
                             ...
                             bool hasSkippedLayers, Plan result) {
  if (predictedPlan) {
...
void Predictor::dump(std::string &result) const {
  ...
  base::StringAppendF(
      &result, "Global non-skipped hit rate: %.2f%% (%zd/%zd)\n",
...
void Predictor::compareLayerStacks(NonBufferHash leftHash,
                                   NonBufferHash rightHash,
                                   std::string &result) const {
  const auto &[leftPredictionEntry, rightPredictionEntry] = std::make_tuple(
      mPredictions.find(leftHash), mPredictions.find(rightHash));
  ...
  const auto &[leftPrediction, rightPrediction] = std::make_tuple(
      leftPredictionEntry->second, rightPredictionEntry->second);
  const auto &[leftStack, rightStack] =
      std::make_tuple(leftPrediction.getExampleLayerStack(),
                      rightPrediction.getExampleLayerStack());
  leftStack.compare(rightStack, result);
}

void Predictor::describeLayerStack(NonBufferHash hash,
                                   std::string &result) const {
...
Prediction &Predictor::getPrediction(NonBufferHash hash) {
  return const_cast<Prediction &>(
      const_cast<const Predictor *>(this)->getPrediction(hash));
}
...
std::optional<Plan> Predictor::getExactMatch(NonBufferHash hash) const {
  ...
  if (match->getMissCount(Prediction::Type::Exact) != 0) {
    ALOGV("[%s] Skipping exact match for %zx because of prior miss", __func__,
          hash);
    return std::nullopt;
  }
...
std::optional<NonBufferHash> Predictor::getApproximateMatch(
  ...
  const auto approximateStackMatches =
      [&](const ApproximateStack &approximateStack) {
        const auto &exampleStack =
            mPredictions.at(approximateStack.hash).getExampleLayerStack();
        if (const auto approximateMatchOpt =
                exampleStack.getApproximateMatch(layers);
            approximateMatchOpt) {
  ...
  const auto candidateMatches = [&](const PromotionCandidate &candidate) {
    ...
    return candidate.prediction.getExampleLayerStack().getApproximateMatch(
               layers) != std::nullopt;
  };
  ...
  } else if (const auto candidateEntry = std::find_if(
                 mCandidates.cbegin(), mCandidates.cend(), candidateMatches);
             candidateEntry != mCandidates.cend()) {
    match = &(candidateEntry->prediction);
  ...
  if (match->getMissCount(Prediction::Type::Approximate) != 0) {
    ALOGV("[%s] Skipping approximate match for %zx because of prior miss",
          __func__, hash);
    return std::nullopt;
  }
...
void Predictor::promoteIfCandidate(NonBufferHash predictionHash) {
  ...
  ALOGV("[%s] Promoting %zx from candidate to prediction", __func__,
        predictionHash);
  ...
  mSimilarStacks[candidateEntry->prediction.getPlan()].push_back(
      predictionHash);
  mPredictions.emplace(predictionHash, std::move(candidateEntry->prediction));
  mCandidates.erase(candidateEntry);
}

void Predictor::recordPredictedResult(
    PredictedPlan predictedPlan, const std::vector<const LayerState *> &layers,
    Plan result) {
  ...
  if (prediction.getPlan() != result) {
    ALOGV("[%s] %s prediction missed, expected %s, found %s", __func__,
          to_string(predictedPlan.type).c_str(),
          to_string(prediction.getPlan()).c_str(), to_string(result).c_str());
  ...
  ALOGV("[%s] %s prediction hit", __func__,
        to_string(predictedPlan.type).c_str());
  ALOGV("[%s] Plan: %s", __func__, to_string(result).c_str());
  prediction.recordHit(predictedPlan.type);

  const auto stackMatchesHash =
      [hash = predictedPlan.hash](const ApproximateStack &stack) {
        return stack.hash == hash;
      };

  if (predictedPlan.type == Prediction::Type::Approximate) {
    // If this approximate match is not already in the list of approximate
    // stacks, add it
...
bool Predictor::findSimilarPrediction(
  ...
  for (NonBufferHash hash : similarStacks) {
    const Prediction &prediction = mPredictions.at(hash);
    auto approximateMatch =
        prediction.getExampleLayerStack().getApproximateMatch(layers);
    if (!approximateMatch) {
      continue;
    }

    const int differingFieldCount =
        __builtin_popcount(approximateMatch->differingFields.get());
    if (!bestMatch ||
        differingFieldCount <
            __builtin_popcount(bestMatch->match.differingFields.get())) {
...
void Predictor::dumpPredictionsByFrequency(std::string &result) const {
  ...
  for (const auto &[hash, prediction] : mPredictions) {
    hashFrequencies.emplace_back(
        hash, prediction.getHitCount(Prediction::Type::Total) +

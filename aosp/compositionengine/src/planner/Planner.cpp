...
std::optional<Flattener::Tunables::RenderScheduling>
buildRenderSchedulingTunables() {
  if (!base::GetBoolProperty(
          std::string("debug.sf.enable_cached_set_render_scheduling"), true)) {
    return std::nullopt;
  }

  const auto renderDuration =
      std::chrono::nanoseconds(base::GetUintProperty<uint64_t>(
          std::string("debug.sf.cached_set_render_duration_ns"),
          Flattener::Tunables::RenderScheduling::kDefaultCachedSetRenderDuration
              .count()));

  const auto maxDeferRenderAttempts = base::GetUintProperty<size_t>(
      std::string("debug.sf.cached_set_max_defer_render_attmpts"),
...
Flattener::Tunables buildFlattenerTuneables() {
  const auto activeLayerTimeout =
      std::chrono::milliseconds(base::GetIntProperty<int32_t>(
          std::string("debug.sf.layer_caching_active_layer_timeout_ms"),
...
Planner::Planner(renderengine::RenderEngine &renderEngine)
    : mFlattener(renderEngine, buildFlattenerTuneables()) {
  mPredictorEnabled = base::GetBoolProperty(
      std::string("debug.sf.enable_planner_prediction"), false);
}

void Planner::setDisplaySize(ui::Size size) { mFlattener.setDisplaySize(size); }

void Planner::plan(
    compositionengine::Output::OutputLayersEnumerator<compositionengine::Output>
        &&layers) {
  SFTRACE_CALL();
  ...
  for (auto layer : layers) {
    ...
    if (const auto layerEntry = mPreviousLayers.find(id);
    ...
    if (const auto found = removedLayers.find(id);
        found != removedLayers.end()) {
  ...
  ALOGV("[%s] Initial hash %zx flattened hash %zx", __func__, hash,
        mFlattenedHash);

  if (mPredictorEnabled) {
    mPredictedPlan = mPredictor.getPredictedPlan(
        layersWereFlattened ? std::vector<const LayerState *>()
                            : mCurrentLayers,
        mFlattenedHash);
    if (mPredictedPlan) {
      ALOGV("[%s] Predicting plan %s", __func__,
            to_string(mPredictedPlan->plan).c_str());
    } else {
      ALOGV("[%s] No prediction found\n", __func__);
    }
  }

  // Clean up the set of previous layers now that the view of the LayerStates in
  // the flattener are up-to-date.
...
void Planner::reportFinalPlan(
  ...
  mPredictor.recordResult(mPredictedPlan, mFlattenedHash, mCurrentLayers,
                          hasSkippedLayers, finalPlan);
}
...
void Planner::dump(const Vector<String16> &args, std::string &result) {
  if (args.size() > 1) {
    ...
    if (command == "--compare" || command == "-c") {
      ...
      if (args.size() < 3) {
        base::StringAppendF(
            &result, "Expected a plan string, e.g. '--planner %s <plan>'\n",
            command.c_str());
        return;
      }
      if (args.size() > 3) {
      ...
      std::optional<Plan> plan =
          Plan::fromString(std::string(planString.c_str()));
      if (!plan) {
...
void Planner::dumpUsage(std::string &result) const {
  ...
  result.append("  Compares the predictions <left_hash> and <right_hash> by "
                "showing differences"
                " in their example layer stacks\n");

  result.append("[--describe|-d] <hash>\n");
  result.append("  Prints the example layer stack and prediction statistics "
                "for <hash>\n");
  ...
  result.append(
      "  Prints the example layer names for similar stacks matching <plan>\n");

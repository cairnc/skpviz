...
const bool CachedSet::sDebugHighlighLayers = base::GetBoolProperty(
    std::string("debug.sf.layer_caching_highlight"), false);
...
CachedSet::Layer::Layer(const LayerState *state,
                        std::chrono::steady_clock::time_point lastUpdate)
    : mState(state), mHash(state->getHash()), mLastUpdate(lastUpdate) {}

CachedSet::CachedSet(const LayerState *layer,
                     std::chrono::steady_clock::time_point lastUpdate)
...
CachedSet::CachedSet(Layer layer)
    : mFingerprint(layer.getHash()), mLastUpdate(layer.getLastUpdate()),
...
NonBufferHash CachedSet::getNonBufferHash() const {
  ...
  // TODO(b/182614524): We sometimes match this with LayerState hashes.
  // Determine if that is necessary (and therefore we need to match
  // implementations).
  size_t hash = 0;
...
void CachedSet::updateAge(std::chrono::steady_clock::time_point now) {
  LOG_ALWAYS_FATAL_IF(mLayers.size() > 1,
                      "[%s] This should only be called on single-layer sets",
                      __func__);
...
void CachedSet::render(renderengine::RenderEngine &renderEngine,
  ...
  const ui::Transform::RotationFlags orientation =
      ui::Transform::toRotationFlags(
          outputState.framebufferSpace.getOrientation());
  ...
  if (mBlurLayer) {
    ...
    blurSettings.blurSetting = LayerFE::ClientCompositionTargetSettings::
        BlurSetting::BackgroundBlurOnly;

    auto blurLayerSettings =
        mBlurLayer->getOutputLayer()->getLayerFE().prepareClientComposition(
            blurSettings);
  ...
  if (mHolePunchLayer) {
    ...
    holePunchSettings->name = android::base::StringPrintf(
        "hole punch layer for %s", layerFE.getDebugName());
    ...
    layerSettings.emplace(layerSettings.begin(),
                          std::move(holePunchBackgroundSettings));
        ...
        .geometry =
            renderengine::Geometry{
                .boundaries = FloatRect(
                    0.0f, 0.0f, static_cast<float>(mBounds.getWidth()),
  ...
  if (texture->getReadyFence()) {
    // Bail out if the buffer is not ready, because there is some pending GPU
    // work left.
  ...
  auto fenceResult = renderEngine
                         .drawLayers(displaySettings, layerSettings,
                                     texture->get(), std::move(bufferFence))
                         .get();
...
bool CachedSet::hasBlurBehind() const {
  return std::any_of(mLayers.cbegin(), mLayers.cend(), [](const Layer &layer) {
    return layer.getState()->hasBlurBehind();
  });
}
...
void CachedSet::addHolePunchLayerIfFeasible(const CachedSet &holePunchLayer,
                                            bool isFirstLayer) {
  ...
  for (const auto &layer : mLayers) {
    ...
    if (contains(outputLayer->getState().displayFrame, holePunchBounds) &&
        (isFirstLayer ||
         outputLayer->getLayerFE().getCompositionState()->isOpaque)) {
...
bool CachedSet::hasKnownColorShift() const {
  return std::any_of(mLayers.cbegin(), mLayers.cend(), [](const Layer &layer) {
    ...
    // Layers are never dimmed when rendering a cached set, meaning that we may
    // ask HWC to dim a cached set. But this means that we can never cache any
    // HDR layers so that we don't accidentally dim those layers.
    const auto hdrType =
        getHdrRenderType(dataspace, layer.getState()->getPixelFormat(),
    ...
    if ((dataspace & HAL_DATASPACE_STANDARD_MASK) ==
...
bool CachedSet::hasProtectedLayers() const {
  return std::any_of(mLayers.cbegin(), mLayers.cend(), [](const Layer &layer) {
    return layer.getState()->isProtected();
  });
}
...
bool CachedSet::cachingHintExcludesLayers() const {
  ...
  LOG_ALWAYS_FATAL_IF(
      shouldExcludeLayers && getLayerCount() > 1,
...
void CachedSet::dump(std::string &result) const {
  ...
  base::StringAppendF(&result,
                      "  + Fingerprint %016zx, last update %sago, age %zd\n",
  ...
  if (mLayers.size() == 1) {
    ...
    if (const sp<GraphicBuffer> buffer =
            mLayers[0].getState()->getBuffer().promote()) {
    ...
    base::StringAppendF(&result, "    Protected [%s]\n",
                        mLayers[0].getState()->isProtected() ? "true"
                                                             : "false");
  } else {
    ...
    for (const Layer &layer : mLayers) {
      base::StringAppendF(&result, "      Layer [%s]\n",
                          layer.getName().c_str());
      if (const sp<GraphicBuffer> buffer =
              layer.getState()->getBuffer().promote()) {
        base::StringAppendF(&result, "       Buffer %p", buffer.get());
        base::StringAppendF(
            &result, "    Format[%s]",
            decodePixelFormat(buffer->getPixelFormat()).c_str());

...
// True if the underlying layer stack is the same modulo state that would be
// expected to be different like specific buffers, false otherwise.
bool isSameStack(const std::vector<const LayerState *> &incomingLayers,
  ...
  for (size_t i = 0; i < incomingLayers.size(); i++) {
    // Checking the IDs here is very strict, but we do this as otherwise we may
    // mistakenly try to access destroyed OutputLayers later on.
    ...
    if (incomingLayers[i]->getDifferingFields(*(existingLayers[i])) !=
        LayerStateField::None) {
      return false;
    }
  }
  return true;
...
Flattener::Flattener(renderengine::RenderEngine &renderEngine,
                     const Tunables &tunables)
    : mRenderEngine(renderEngine), mTunables(tunables),
      mTexturePool(mRenderEngine) {}

NonBufferHash
Flattener::flattenLayers(const std::vector<const LayerState *> &layers,
  ...
  // 3. A stricter equality check demonstrates that the layer stack really did
  // change, since the hashed geometry does not guarantee uniqueness.
  if (mCurrentGeometry != hash ||
      (!mLayers.empty() && !isSameStack(layers, mLayers))) {
...
void Flattener::renderCachedSets(
  ...
  // If we have a render deadline, and the flattener is configured to skip
  // rendering if we don't have enough time, then we skip rendering the cached
  // set if we think that we'll steal too much time from the next frame.
...
void Flattener::dump(std::string &result) const {
  ...
  const size_t displayArea =
      static_cast<size_t>(mDisplaySize.width * mDisplaySize.height);
  base::StringAppendF(&result, "      Unflattened: %.2f\n",
                      static_cast<float>(mUnflattenedDisplayCost) /
                          displayArea);
  ...
  const size_t maxLayerCount =
      mInitialLayerCounts.empty()
          ? 0u
          : std::max_element(mInitialLayerCounts.cbegin(),
                             mInitialLayerCounts.cend(), compareLayerCounts)
                ->first;

  result.append("\n    Initial counts:\n");
  for (size_t count = 1; count < maxLayerCount; ++count) {
    size_t initial = mInitialLayerCounts.count(count) > 0
                         ? mInitialLayerCounts.at(count)
                         : 0;
  ...
  for (size_t count = 1; count < maxLayerCount; ++count) {
    size_t final =
        mFinalLayerCounts.count(count) > 0 ? mFinalLayerCounts.at(count) : 0;
    base::StringAppendF(&result, "      % 2zd: %zd\n", count, final);
  }
  ...
  const auto lastUpdate = std::chrono::duration_cast<std::chrono::milliseconds>(
      now - mLastGeometryUpdate);
  base::StringAppendF(&result, "\n  Current hash %016zx, last update %sago\n\n",
                      mCurrentGeometry, durationString(lastUpdate).c_str());
...
size_t Flattener::calculateDisplayCost(
  ...
  if (hasClientComposition) {
    // If there is client composition, the client target buffer has to be both
    // written by the GPU and read by the DPU, so we pay its cost twice
    displayCost += 2 * static_cast<size_t>(coveredRegion.bounds().width() *
...
bool Flattener::mergeWithCachedSets(
    const std::vector<const LayerState *> &layers, time_point now) {
  SFTRACE_CALL();
  ...
  while (incomingLayerIter != layers.end()) {
    if (mNewCachedSet && mNewCachedSet->getFirstLayer().getState()->getId() ==
                             (*incomingLayerIter)->getId()) {
...
std::vector<Flattener::Run> Flattener::findCandidateRuns(time_point now) const {
  ...
  for (auto currentSet = mLayers.cbegin(); currentSet != mLayers.cend();
       ++currentSet) {
    bool layerIsInactive =
    ...
    // Layers should also be considered inactive whenever their framerate is
    // lower than 1fps.
    if (!layerIsInactive &&
        currentSet->getLayerCount() == kNumLayersFpsConsideration) {
    ...
    if (!layerDeniedFromCaching && layerIsInactive &&
      ...
      // If we're here then this blur layer recently had an active buffer
      // updating, meaning that there is exactly one layer. Blur radius
      // currently is part of layer stack geometry, so we're also guaranteed
      // that the background blur radius hasn't changed for at least as long as
      // this new inactive cached set.
  ...
  // If we're in the middle of a run at the end, we still need to validate and
  // build it.
  if (isPartOfRun) {
    if (auto run = builder.validateAndBuild(); run) {
      runs.push_back(*run);
...
std::optional<Flattener::Run>
Flattener::findBestRun(std::vector<Flattener::Run> &runs) const {
  if (runs.empty()) {
    return std::nullopt;
  }
...
void Flattener::buildCachedSets(time_point now) {
  ...
  // Don't try to build a new cached set if we already have a new one in
  // progress
  if (mNewCachedSet) {
    return;
  }
  ...
  for (const CachedSet &layer : mLayers) {
    // TODO (b/191997217): make it less aggressive, and sync with
    // findCandidateRuns
  ...
  if (mTunables.mEnableHolePunch && bestRun->getHolePunchCandidate() &&
    ...
    mNewCachedSet->addHolePunchLayerIfFeasible(
        *bestRun->getHolePunchCandidate(),
        bestRun->getStart() == mLayers.cbegin());
  }

  // TODO(b/181192467): Actually compute new LayerState vector and corresponding
  // hash for each run and feedback into the predictor

  ++mCachedSetCreationCount;
  mCachedSetCreationCost += mNewCachedSet->getCreationCost();

  // note the compiler should strip the follow no-op statements when ALOGV is
  // off
  const auto dumper = [&] {
    std::string setDump;

...
RenderSurface::RenderSurface(const CompositionEngine &compositionEngine,
                             Display &display,
                             const RenderSurfaceCreationArgs &args)
    : mCompositionEngine(compositionEngine), mDisplay(display),
      mNativeWindow(args.nativeWindow), mDisplaySurface(args.displaySurface),
...
bool RenderSurface::isValid() const { return mSize.isValid(); }
...
void RenderSurface::initialize() {
  ...
  ALOGE_IF(status != NO_ERROR, "Unable to set BQ format to RGBA888: %d",
           status);
  status = native_window_set_usage(window, DEFAULT_USAGE);
  ALOGE_IF(status != NO_ERROR,
           "Unable to set BQ usage bits for GPU rendering: %d", status);
}

const ui::Size &RenderSurface::getSize() const { return mSize; }
...
void RenderSurface::setBufferDataspace(ui::Dataspace dataspace) {
  native_window_set_buffers_data_space(
      mNativeWindow.get(), static_cast<android_dataspace>(dataspace));
}
...
void RenderSurface::setProtected(bool useProtected) {
  ...
  ALOGE_IF(status != NO_ERROR,
           "Unable to set BQ usage bits for protected content: %d", status);
  if (status == NO_ERROR) {
...
void RenderSurface::prepareFrame(bool usesClientComposition,
                                 bool usesDeviceComposition) {
  ...
  if (status_t result = mDisplaySurface->prepareFrame(compositionType);
      result != NO_ERROR) {
    ALOGE("updateCompositionType failed for %s: %d (%s)",
          mDisplay.getName().c_str(), result, strerror(-result));
  }
}
...
RenderSurface::dequeueBuffer(base::unique_fd *bufferFence) {
  ...
  ALOGW_IF(mTexture != nullptr,
           "Clobbering a non-null pointer to a buffer [%p].",
  ...
  if (texture) {
    ...
    mTexture = std::make_shared<renderengine::impl::ExternalTexture>(
        GraphicBuffer::from(buffer), mCompositionEngine.getRenderEngine(),
...
void RenderSurface::queueBuffer(base::unique_fd readyFence, float hdrSdrRatio) {
  ...
  if (state.usesClientComposition || state.flipClientTarget) {
    ...
    if (mTexture == nullptr) {
      ...
      status_t result = mNativeWindow->queueBuffer(
          mNativeWindow.get(), mTexture->getBuffer()->getNativeBuffer(),
          dup(readyFence));
      if (result != NO_ERROR) {
        ALOGE("Error when queueing buffer for display [%s]: %d",
              mDisplay.getName().c_str(), result);
        // We risk blocking on dequeueBuffer if the primary display failed
        // to queue up its buffer, so crash here.
        if (!mDisplay.isVirtual()) {
          LOG_ALWAYS_FATAL("ANativeWindow::queueBuffer failed with error: %d",
                           result);
        } else {
...
void RenderSurface::setSizeForTest(const ui::Size &size) { mSize = size; }

std::shared_ptr<renderengine::ExternalTexture> &
RenderSurface::mutableTextureForTest() {
  return mTexture;
}

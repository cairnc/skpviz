...
bool isHdrColorMode(const ColorMode colorMode) {
  return std::any_of(std::begin(sHdrColorModes), std::end(sHdrColorModes),
                     [colorMode](ColorMode hdrColorMode) {
                       return hdrColorMode == colorMode;
                     });
}
...
ColorMode
getHwcColorMode(const std::unordered_map<ColorMode, std::vector<RenderIntent>>
                    &hwcColorModes,
                ColorMode mode) {
...
RenderIntent getHwcRenderIntent(const std::vector<RenderIntent> &hwcIntents,
                                RenderIntent intent) {
...
createDisplayColorProfile(const DisplayColorProfileCreationArgs &args) {
  return std::make_unique<DisplayColorProfile>(args);
}

DisplayColorProfile::DisplayColorProfile(
  ...
  float maxAverageLuminance =
      args.hdrCapabilities.getDesiredMaxAverageLuminance();

  minLuminance = minLuminance <= 0.0 ? sDefaultMinLumiance : minLuminance;
  maxLuminance = maxLuminance <= 0.0 ? sDefaultMaxLumiance : maxLuminance;
  maxAverageLuminance =
      maxAverageLuminance <= 0.0 ? sDefaultMaxLumiance : maxAverageLuminance;

  mHdrCapabilities =
      HdrCapabilities(types, maxLuminance, maxAverageLuminance, minLuminance);
}

DisplayColorProfile::~DisplayColorProfile() = default;

bool DisplayColorProfile::isValid() const { return true; }
...
bool DisplayColorProfile::hasHDR10PlusSupport() const { return mHasHdr10Plus; }

bool DisplayColorProfile::hasHDR10Support() const { return mHasHdr10; }

bool DisplayColorProfile::hasHLGSupport() const { return mHasHLG; }
...
void DisplayColorProfile::addColorMode(
  ...
  ALOGV(
      "DisplayColorProfile: map (%s, %s) to (%s, %s, %s)",
      dataspaceDetails(static_cast<android_dataspace_t>(dataspace)).c_str(),
      decodeRenderIntent(intent).c_str(),
      dataspaceDetails(static_cast<android_dataspace_t>(hwcDataspace)).c_str(),
      decodeColorMode(hwcColorMode).c_str(),
      decodeRenderIntent(hwcIntent).c_str());

  mColorModes[getColorModeKey(dataspace, intent)] = {hwcDataspace, hwcColorMode,
                                                     hwcIntent};
}
...
bool DisplayColorProfile::hasLegacyHdrSupport(Dataspace dataspace) const {
  if ((dataspace == Dataspace::BT2020_PQ && hasHDR10Support()) ||
    ...
    auto iter = mColorModes.find(
        getColorModeKey(dataspace, RenderIntent::TONE_MAP_COLORIMETRIC));
...
void DisplayColorProfile::getBestColorMode(Dataspace dataspace,
                                           RenderIntent intent,
...
void DisplayColorProfile::dump(std::string &out) const {
  ...
  out.append("\n   Hdr Luminance Info:");
  dumpVal(out, "desiredMinLuminance",
          mHdrCapabilities.getDesiredMinLuminance());
  dumpVal(out, "desiredMaxLuminance",
          mHdrCapabilities.getDesiredMaxLuminance());
  dumpVal(out, "desiredMaxAverageLuminance",
          mHdrCapabilities.getDesiredMaxAverageLuminance());
  out.append("\n");
}

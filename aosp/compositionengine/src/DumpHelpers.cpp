...
void dumpVal(std::string &out, const char *name, const char *valueName,
             int value) {
  StringAppendF(&out, "%s=%s (%d) ", name, valueName, value);
}

void dumpVal(std::string &out, const char *name, const std::string &valueName,
             int value) {
...
void dumpVal(std::string &out, const char *name, const FloatRect &rect) {
  StringAppendF(&out, "%s=[%f %f %f %f] ", name, rect.left, rect.top,
                rect.right, rect.bottom);
}

void dumpVal(std::string &out, const char *name, const Rect &rect) {
  StringAppendF(&out, "%s=[%d %d %d %d] ", name, rect.left, rect.top,
                rect.right, rect.bottom);
}
...
void dumpVal(std::string &out, const char *name,
...
void dumpVal(std::string &out, const char *name, const StretchEffect &effect) {
  StringAppendF(&out, "%s={width=%f, height=%f, vec=(%f, %f), max=(%f, %f)} ",
                name, effect.width, effect.height, effect.vectorX,
                effect.vectorY, effect.maxAmountX, effect.maxAmountY);
}

...
std::unique_ptr<OutputLayer>
createOutputLayer(const compositionengine::Output &output,
...
Rect OutputLayer::calculateInitialCrop() const {
  ...
  const ui::Transform &inverseLayerTransform =
      layerState.geomInverseLayerTransform;
...
FloatRect OutputLayer::calculateOutputSourceCrop(
  ...
  if (layerState.geomBufferUsesDisplayInverseTransform) {
    ...
    invTransform =
        (ui::Transform(invTransformOrient) * ui::Transform(invTransform))
            .getOrientation();
  }
  ...
  const Rect winCrop = activeCrop.transform(invTransform, bufferSize.getWidth(),
                                            bufferSize.getHeight());

  // below, crop is intersected with winCrop expressed in crop's coordinate
  // space
...
Rect OutputLayer::calculateOutputDisplayFrame() const {
  ...
  if (!layerState.geomCrop.isEmpty() && layerState.geomBufferSize.isValid()) {
    ...
    activeCrop = activeCrop.intersect(
        outputState.layerStackSpace.getContent().toFloatRect());
    ...
    Rect topRegion = Rect(layerTransform.transform(FloatRect(
        0, 0, layerState.geomBufferSize.getWidth(), layerState.geomCrop.top)));
    Rect bottomRegion = Rect(layerTransform.transform(FloatRect(
        0, layerState.geomCrop.bottom, layerState.geomBufferSize.getWidth(),
        layerState.geomBufferSize.getHeight())));
    Rect leftRegion = Rect(layerTransform.transform(
        FloatRect(0, layerState.geomCrop.top, layerState.geomCrop.left,
                  layerState.geomCrop.bottom)));
    Rect rightRegion = Rect(layerTransform.transform(FloatRect(
        layerState.geomCrop.right, layerState.geomCrop.top,
  ...
  if (layerState.forceClientComposition &&
      layerState.shadowSettings.length > 0.0f) {
  ...
  frame =
      frame.intersect(outputState.layerStackSpace.getContent().toFloatRect());
...
uint32_t OutputLayer::calculateOutputRelativeBufferTransform(
  ...
  if (layerState.geomBufferUsesDisplayInverseTransform) {
     ...
     * See comment in BufferLayer::prepareClientLayer with ref to b/36727915 for
     * why.
     */
    transform =
        ui::Transform(invTransform) * displayTransform * bufferTransform;
  }
    ...
    for (const auto &hwcLut : hwcLutProperties) {
      if (static_cast<int32_t>(hwcLut.dimension) ==
          ...
          std::find(hwcLut.samplingKeys.begin(), hwcLut.samplingKeys.end(),
                    static_cast<LutProperties::SamplingKey>(
                        inputLut.samplingKey)) != hwcLut.samplingKeys.end()) {
        foundInHwcLuts = true;
        break;
      }
    }
...
void OutputLayer::updateCompositionState(
  ...
  auto pixelFormat = layerFEState->buffer
                         ? std::make_optional(static_cast<ui::PixelFormat>(
  ...
  // For now, also respect the colorspace agnostic flag if we're drawing to HDR,
  // to avoid drastic luminance shift. TODO(b/292162273): we should check if
  // that's true though.
  state.dataspace =
      layerFEState->isColorspaceAgnostic && hdrRenderType == HdrRenderType::SDR
          ? outputState.dataspace
          : layerFEState->dataspace;

  // Override the dataspace transfer from 170M to sRGB if the device
  // configuration requests this. We do this here instead of in buffer info so
  // that dumpsys can still report layers that are using the 170M transfer. Also
  // we only do this if the colorspace is not agnostic for the layer, in case
  // the color profile uses a 170M transfer function.
  if (outputState.treat170mAsSrgb && !layerFEState->isColorspaceAgnostic &&
      (state.dataspace & HAL_DATASPACE_TRANSFER_MASK) ==
          HAL_DATASPACE_TRANSFER_SMPTE_170M) {
    state.dataspace = static_cast<ui::Dataspace>(
        (state.dataspace & HAL_DATASPACE_STANDARD_MASK) |
        (state.dataspace & HAL_DATASPACE_RANGE_MASK) |
        HAL_DATASPACE_TRANSFER_SRGB);
  }

  // re-get HdrRenderType after the dataspace gets changed.
  hdrRenderType = getHdrRenderType(state.dataspace, pixelFormat,
                                   layerFEState->desiredHdrSdrRatio);

  // For hdr content, treat the white point as the display brightness - HDR
  // content should not be boosted or dimmed. If the layer explicitly requests
  // to disable dimming, then don't dim either.
  if (getOutput().getState().displayBrightnessNits ==
          getOutput().getState().sdrWhitePointNits ||
      getOutput().getState().displayBrightnessNits <= 0.f ||
      !layerFEState->dimmingEnabled) {
    ...
    if (FlagManager::getInstance().begone_bright_hlg()) {
      idealizedMaxHeadroom = std::min(idealizedMaxHeadroom,
                                      getIdealizedMaxHeadroom(state.dataspace));
    }

    state.dimmingRatio = std::min(idealizedMaxHeadroom / deviceHeadroom, 1.0f);
    state.whitePointNits =
        getOutput().getState().displayBrightnessNits * state.dimmingRatio;
  } else {
    float layerBrightnessNits = getOutput().getState().sdrWhitePointNits;
    // RANGE_EXTENDED can "self-promote" to HDR, but is still rendered for a
    // particular range that we may need to re-adjust to the current display
    // conditions
    if (hdrRenderType == HdrRenderType::DISPLAY_HDR) {
      layerBrightnessNits *= layerFEState->currentHdrSdrRatio;
    }
    state.dimmingRatio = std::clamp(
        layerBrightnessNits / getOutput().getState().displayBrightnessNits, 0.f,
        1.f);
...
void OutputLayer::writeStateToHWC(bool includeGeometry, bool skipLayer,
  ...
  if (!hwcLayer) {
    ALOGE("[%s] failed to write composition state to HWC -- no hwcLayer for "
          "output %s",
  ...
  if (requestedCompositionType == Composition::SOLID_COLOR &&
      state.overrideInfo.buffer) {
    requestedCompositionType = Composition::DEVICE;
  }

  // TODO(b/181172795): We now update geometry for all flattened layers. We
  // should update it only when the geometry actually changes
  ...
  if (isOverridden || prevOverridden || skipLayer || includeGeometry) {
    writeOutputDependentGeometryStateToHWC(hwcLayer.get(),
                                           requestedCompositionType, z);
    writeOutputIndependentGeometryStateToHWC(
        hwcLayer.get(), *outputIndependentState, skipLayer);
  }

  writeOutputDependentPerFrameStateToHWC(hwcLayer.get());
  writeOutputIndependentPerFrameStateToHWC(hwcLayer.get(),
                                           *outputIndependentState,
                                           requestedCompositionType, skipLayer);

  writeCompositionTypeToHWC(hwcLayer.get(), requestedCompositionType,
                            isPeekingThrough, skipLayer);
...
void OutputLayer::writeOutputDependentGeometryStateToHWC(
    HWC2::Layer *hwcLayer, Composition requestedCompositionType, uint32_t z) {
  ...
  if (outputDependentState.overrideInfo.buffer != nullptr) {
    ...
    sourceCrop = FloatRect(
        0.f, 0.f,
        static_cast<float>(
            outputDependentState.overrideInfo.buffer->getBuffer()->getWidth()),
        static_cast<float>(outputDependentState.overrideInfo.buffer->getBuffer()
                               ->getHeight()));
  }

  ALOGV("Writing display frame [%d, %d, %d, %d]", displayFrame.left,
        displayFrame.top, displayFrame.right, displayFrame.bottom);

  if (auto error = hwcLayer->setDisplayFrame(displayFrame);
  ...
  if (auto error = hwcLayer->setSourceCrop(sourceCrop);
      error != hal::Error::NONE) {
  ...
  // Solid-color layers and overridden buffers should always use an identity
  // transform.
  const auto bufferTransform =
      (requestedCompositionType != Composition::SOLID_COLOR &&
       getState().overrideInfo.buffer == nullptr)
          ? outputDependentState.bufferTransform
          : static_cast<hal::Transform>(0);
  if (auto error =
          hwcLayer->setTransform(static_cast<hal::Transform>(bufferTransform));
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set transform %s: %s (%d)",
...
void OutputLayer::writeOutputIndependentGeometryStateToHWC(
    HWC2::Layer *hwcLayer,
    const LayerFECompositionState &outputIndependentState, bool skipLayer) {
  // If there is a peekThroughLayer, then this layer has a hole in it. We need
  // to use PREMULTIPLIED so it will peek through.
  const auto &overrideInfo = getState().overrideInfo;
  const auto blendMode =
      overrideInfo.buffer || overrideInfo.peekThroughLayer
          ? hardware::graphics::composer::hal::BlendMode::PREMULTIPLIED
          : outputIndependentState.blendMode;
  if (auto error = hwcLayer->setBlendMode(blendMode);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set blend mode %s: %s (%d)",
          getLayerFE().getDebugName(), toString(blendMode).c_str(),
          to_string(error).c_str(), static_cast<int32_t>(error));
  }

  const float alpha = skipLayer ? 0.0f
                                : (getState().overrideInfo.buffer
                                       ? 1.0f
                                       : outputIndependentState.alpha);
  ALOGV("Writing alpha %f", alpha);

  if (auto error = hwcLayer->setPlaneAlpha(alpha); error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set plane alpha %.3f: %s (%d)",
          getLayerFE().getDebugName(), alpha, to_string(error).c_str(),
          static_cast<int32_t>(error));
  }

  for (const auto &[name, entry] : outputIndependentState.metadata) {
    if (auto error = hwcLayer->setLayerGenericMetadata(name, entry.mandatory,
                                                       entry.value);
        error != hal::Error::NONE) {
      ALOGE("[%s] Failed to set generic metadata %s %s (%d)",
            getLayerFE().getDebugName(), name.c_str(), to_string(error).c_str(),
            static_cast<int32_t>(error));
    }
  }
}

void OutputLayer::writeLutToHWC(
    HWC2::Layer *hwcLayer,
  ...
  for (size_t i = 0; i < lutOffsets.size(); i++) {
    ...
    properties.dimension =
        static_cast<LutProperties::Dimension>(lutProperties[i].dimension);
...
void OutputLayer::writeOutputDependentPerFrameStateToHWC(
    HWC2::Layer *hwcLayer) {
  const auto &outputDependentState = getState();

  // TODO(lpique): b/121291683 outputSpaceVisibleRegion is output-dependent
  // geometry state and should not change every frame.
  Region visibleRegion =
      outputDependentState.overrideInfo.buffer
          ? Region(outputDependentState.overrideInfo.visibleRegion)
          : outputDependentState.outputSpaceVisibleRegion;
  if (auto error = hwcLayer->setVisibleRegion(visibleRegion);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set visible region: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
    visibleRegion.dump(LOG_TAG);
  }

  if (auto error = hwcLayer->setBlockingRegion(
          outputDependentState.outputSpaceBlockingRegionHint);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set blocking region: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
  ...
  if (auto error = hwcLayer->setDataspace(dataspace);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set dataspace %d: %s (%d)",
          getLayerFE().getDebugName(), dataspace, to_string(error).c_str(),
          static_cast<int32_t>(error));
  }

  // Cached layers are not dimmed, which means that composer should attempt to
  // dim. Note that if the dimming ratio is large, then this may cause the
  // cached layer to kick back into GPU composition :( Also note that this
  // assumes that there are no HDR layers that are able to be cached. Otherwise,
  // this could cause HDR layers to be dimmed twice.
  ...
  if (auto error = hwcLayer->setBrightness(dimmingRatio);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set brightness %f: %s (%d)",
          getLayerFE().getDebugName(), dimmingRatio, to_string(error).c_str(),
          static_cast<int32_t>(error));
  }

  if (com_android_graphics_libgui_flags_apply_picture_profiles() &&
      outputDependentState.pictureProfileHandle) {
    if (auto error = hwcLayer->setPictureProfileHandle(
            outputDependentState.pictureProfileHandle);
        error != hal::Error::NONE) {
      ALOGE("[%s] Failed to set picture profile handle: %s (%d)",
            getLayerFE().getDebugName(),
            toString(outputDependentState.pictureProfileHandle).c_str(),
            static_cast<int32_t>(error));
    }
    // Reset the picture profile state, as it needs to be re-committed on each
    // present cycle when Output decides that the limited picture-processing
    // hardware should be used by this layer.
...
void OutputLayer::writeOutputIndependentPerFrameStateToHWC(
  ...
  switch (auto error = hwcLayer->setColorTransform(
              outputIndependentState.colorTransform)) {
  case hal::Error::NONE:
    break;
  case hal::Error::UNSUPPORTED:
    ...
    break;
  default:
    ALOGE("[%s] Failed to set color transform: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
  }

  const Region &surfaceDamage =
      getState().overrideInfo.buffer
          ? getState().overrideInfo.damageRegion
          : (getState().hwc->stateOverridden
                 ? Region::INVALID_REGION
                 : outputIndependentState.surfaceDamage);

  if (auto error = hwcLayer->setSurfaceDamage(surfaceDamage);
      error != hal::Error::NONE) {
    ALOGE("[%s] Failed to set surface damage: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
...
void OutputLayer::writeSolidColorStateToHWC(
    HWC2::Layer *hwcLayer,
    const LayerFECompositionState &outputIndependentState) {
  aidl::android::hardware::graphics::composer3::Color color = {
      outputIndependentState.color.r, outputIndependentState.color.g,
      outputIndependentState.color.b, 1.0f};
...
void OutputLayer::writeSidebandStateToHWC(
    HWC2::Layer *hwcLayer,
    const LayerFECompositionState &outputIndependentState) {
  if (auto error = hwcLayer->setSidebandStream(
          outputIndependentState.sidebandStream->handle());
      error != hal::Error::NONE) {
...
void OutputLayer::uncacheBuffers(
    const std::vector<uint64_t> &bufferIdsToUncache) {
  auto &state = editState();
  // Skip doing this if there is no HWC interface
  if (!state.hwc) {
    return;
  ...
  // Uncache the active buffer last so that it's the first buffer to be purged
  // from the cache next time a buffer is sent to this layer.
  ...
  if (uncacheActiveBuffer) {
    slotsToClear.push_back(
        state.hwc->hwcBufferCache.uncache(state.hwc->activeBufferId));
  }

  hal::Error error = state.hwc->hwcLayer->setBufferSlotsToClear(
      slotsToClear, state.hwc->activeBufferSlot);
  if (error != hal::Error::NONE) {
    ALOGE("[%s] Failed to clear buffer slots: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
  }
}
...
const PictureProfileHandle &OutputLayer::getPictureProfileHandle() const {
  const auto *layerState = getLayerFE().getCompositionState();
  return layerState ? layerState->pictureProfileHandle
                    : PictureProfileHandle::NONE;
}

void OutputLayer::writeBufferStateToHWC(
    HWC2::Layer *hwcLayer,
    const LayerFECompositionState &outputIndependentState, bool skipLayer) {
  ...
  if (auto error = hwcLayer->setPerFrameMetadata(
          supportedPerFrameMetadata, outputIndependentState.hdrMetadata);
      error != hal::Error::NONE && error != hal::Error::UNSUPPORTED) {
    ALOGE("[%s] Failed to set hdrMetadata: %s (%d)",
          getLayerFE().getDebugName(), to_string(error).c_str(),
          static_cast<int32_t>(error));
  }
  ...
  sp<Fence> hwcFence;
  {
    // Editing the state only because we update the HWC buffer cache and active
    // buffer.
    auto &state = editState();
    // Override buffers use a special cache slot so that they don't evict client
    // buffers.
    if (state.overrideInfo.buffer != nullptr && !skipLayer) {
      ...
      // Keep track of the active buffer ID so when it's discarded we uncache it
      // last so its slot will be used first, allowing the memory to be freed as
      // soon as possible.
      state.hwc->activeBufferId =
          state.overrideInfo.buffer->getBuffer()->getId();
    } else {
    ...
    // Keep track of the active buffer slot, so we can restore it after clearing
    // other buffer slots.
    state.hwc->activeBufferSlot = hwcSlotAndBuffer.slot;
  }

  if (auto error = hwcLayer->setBuffer(hwcSlotAndBuffer.slot,
                                       hwcSlotAndBuffer.buffer, hwcFence);
      error != hal::Error::NONE) {
...
void OutputLayer::writeCompositionTypeToHWC(
    HWC2::Layer *hwcLayer, Composition requestedCompositionType,
  ...
  // We also resend the composition type when this layer was previously skipped,
  // to ensure that the composition type is up-to-date.
  if (outputDependentState.hwc->hwcCompositionType !=
          requestedCompositionType ||
      (outputDependentState.hwc->layerSkipped && !skipLayer)) {
    outputDependentState.hwc->hwcCompositionType = requestedCompositionType;

    if (auto error = hwcLayer->setCompositionType(requestedCompositionType);
        error != hal::Error::NONE) {
      ALOGE("[%s] Failed to set composition type %s: %s (%d)",
            getLayerFE().getDebugName(),
            to_string(requestedCompositionType).c_str(),
            to_string(error).c_str(), static_cast<int32_t>(error));
    }
  }
}
...
void OutputLayer::writeCursorPositionToHWC() const {
  ...
  if (auto error = hwcLayer->setCursorPosition(position.left, position.top);
    ...
    ALOGE("[%s] Failed to set cursor position to (%d, %d): %s (%d)",
          getLayerFE().getDebugName(), position.left, position.top,
          to_string(error).c_str(), static_cast<int32_t>(error));
  }
}
...
void OutputLayer::detectDisallowedCompositionTypeChange(Composition from,
                                                        Composition to) const {
  bool result = false;
  switch (from) {
  case Composition::INVALID:
  case Composition::CLIENT:
  ...
  if (!result) {
    ALOGE("[%s] Invalid device requested composition type change: %s (%d) --> "
          "%s (%d)",
          getLayerFE().getDebugName(), to_string(from).c_str(),
          static_cast<int>(from), to_string(to).c_str(), static_cast<int>(to));
  }
}
...
void OutputLayer::applyDeviceCompositionTypeChange(
  ...
  // validated composition type may be arbitrary (usually DEVICE, to reflect
  // that there were fewer GPU layers)
  if (!hwcState.layerSkipped) {
    detectDisallowedCompositionTypeChange(hwcState.hwcCompositionType,
                                          compositionType);
  }
...
void OutputLayer::applyDeviceLayerRequest(hal::LayerRequest request) {
  ...
  default:
    ALOGE("[%s] Unknown device layer request %s (%d)",
          getLayerFE().getDebugName(), toString(request).c_str(),
...
void OutputLayer::applyDeviceLayerLut(
  ...
  for (const auto &[offset, properties] : lutOffsetsAndProperties) {
    ...
    if (properties.samplingKeys.size() == 1) {
      ...
      samplingKeys.emplace_back(
          static_cast<int32_t>(properties.samplingKeys[0]));
    }
  }
  hwcState.luts = std::make_shared<gui::DisplayLuts>(
      base::unique_fd(lutFileDescriptor.release()), std::move(offsets),
      std::move(dimensions), std::move(sizes), std::move(samplingKeys));
}
...
OutputLayer::getOverrideCompositionSettings() const {
  if (getState().overrideInfo.buffer == nullptr) {
    return {};
  }

  // Compute the geometry boundaries in layer stack space: we need to transform
  // from the framebuffer space of the override buffer to layer space.
  const ProjectionSpace &layerSpace = getOutput().getState().layerStackSpace;
  const ui::Transform transform =
      getState().overrideInfo.displaySpace.getTransform(layerSpace);
  const Rect boundaries =
      transform.transform(getState().overrideInfo.displayFrame);
  ...
  settings.source = renderengine::PixelSource{
      .buffer = renderengine::Buffer{
          ...
          // If the transform from layer space to display space contains a
          // rotation, we need to undo the rotation in the texture transform
          .textureTransform =
              ui::Transform(transform.inverse().getOrientation(), 1, 1)
                  .asMatrix4(),
      }};

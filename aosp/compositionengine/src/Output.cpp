...
template <typename T> Reversed<T> reversed(const T &c) {
  return Reversed<T>(c);
}
...
createOutput(const compositionengine::CompositionEngine &compositionEngine) {
...
bool Output::isValid() const {
  return mDisplayColorProfile && mDisplayColorProfile->isValid() &&
         mRenderSurface && mRenderSurface->isValid();
}

ftl::Optional<DisplayId> Output::getDisplayId() const { return {}; }

const std::string &Output::getName() const { return mName; }

void Output::setName(const std::string &name) {
  mName = name;
  auto displayIdOpt = getDisplayId();
  mNamePlusId = displayIdOpt
                    ? base::StringPrintf("%s (%s)", mName.c_str(),
...
void Output::setLayerCachingEnabled(bool enabled) {
  ...
  if (enabled) {
    mPlanner = std::make_unique<planner::Planner>(
        getCompositionEngine().getRenderEngine());
    if (mRenderSurface) {
...
void Output::setProjection(ui::Rotation orientation,
                           const Rect &layerStackSpaceRect,
  ...
  const uint32_t transformOrientationFlags =
      ui::Transform::toRotationFlags(orientation);
  ui::Transform rotation;
  if (transformOrientationFlags != ui::Transform::ROT_INVALID) {
    const auto displaySize = outputState.displaySpace.getBoundsAsRect();
    rotation.set(transformOrientationFlags, displaySize.width(),
                 displaySize.height());
  }
  outputState.displaySpace.setContent(
      rotation.transform(orientedDisplaySpaceRect));

  // Compute framebufferSpace
  outputState.framebufferSpace.setOrientation(orientation);
  LOG_FATAL_IF(outputState.framebufferSpace.getBoundsAsRect() ==
                   Rect::INVALID_RECT,
  ...
  outputState.layerStackSpace.setBounds(ui::Size(
      layerStackSpaceRect.getWidth(), layerStackSpaceRect.getHeight()));

  outputState.transform =
      outputState.layerStackSpace.getTransform(outputState.displaySpace);
...
ui::Transform::RotationFlags Output::getTransformHint() const {
  return static_cast<ui::Transform::RotationFlags>(
      getState().transform.getOrientation());
}
...
void Output::setColorProfile(const ColorProfile &colorProfile) {
  ...
  ALOGV("Set active color mode: %s (%d), active render intent: %s (%d)",
        ...
        decodeRenderIntent(colorProfile.renderIntent).c_str(),
        colorProfile.renderIntent);

  dirtyEntireOutput();
}

void Output::setDisplayBrightness(float sdrWhitePointNits,
...
void Output::dumpPlannerInfo(const Vector<String16> &args,
...
void Output::setDisplayColorProfile(
...
void Output::setRenderSurface(
    std::unique_ptr<compositionengine::RenderSurface> surface) {
...
void Output::cacheClientCompositionRequests(uint32_t cacheSize) {
  if (cacheSize == 0) {
    ...
    mClientCompositionRequestCache =
        std::make_unique<ClientCompositionRequestCache>(cacheSize);
  }
};

void Output::setRenderSurfaceForTest(
    std::unique_ptr<compositionengine::RenderSurface> surface) {
  mRenderSurface = std::move(surface);
}
...
Region Output::getDirtyRegion() const {
  ...
  return outputState.dirtyRegion.intersect(
      outputState.layerStackSpace.getContent());
}
...
Output::createOutputLayer(const sp<LayerFE> &layerFE) const {
  return impl::createOutputLayer(*this, layerFE);
}

compositionengine::OutputLayer *
Output::getOutputLayerForLayer(const sp<LayerFE> &layerFE) const {
...
ftl::Future<std::monostate>
Output::present(const compositionengine::CompositionRefreshArgs &refreshArgs) {
  const auto stringifyExpectedPresentTime = [this,
                                             &refreshArgs]() -> std::string {
    return getDisplayId()
  ...
  const bool predictCompositionStrategy =
      canPredictCompositionStrategy(refreshArgs);
...
void Output::rebuildLayerStacks(
    const compositionengine::CompositionRefreshArgs &refreshArgs,
    LayerFESet &layerFESet) {
  auto &outputState = editState();

  // Do nothing if this output is not enabled or there is no need to perform
  // this update
  if (!outputState.isEnabled ||
      CC_LIKELY(!refreshArgs.updatingOutputGeometryThisFrame)) {
    return;
  }
  SFTRACE_CALL();
  ALOGV(__FUNCTION__);
  ...
  coverage.aboveCoveredLayersExcludingOverlays =
      refreshArgs.hasTrustedPresentationListener ? std::make_optional<Region>()
                                                 : std::nullopt;
...
void Output::collectVisibleLayers(
    const compositionengine::CompositionRefreshArgs &refreshArgs,
...
void Output::ensureOutputLayerIfVisible(
  ...
  if (layerFEState->shadowSettings.length > 0.0f) {
    ...
    const auto inset = static_cast<int32_t>(
        ceilf(layerFEState->shadowSettings.length) * -1.0f);
  ...
  if (layerFEState->isOpaque &&
      ((layerOrientation & ui::Transform::ROT_INVALID) == 0)) {
  ...
  auto prevOutputLayer =
      prevOutputLayerIndex
          ? getOutputLayerOrderedByZByIndex(*prevOutputLayerIndex)
          : nullptr;
  ...
  const Region &oldVisibleRegion =
      prevOutputLayer ? prevOutputLayer->getState().visibleRegion
                      : kEmptyRegion;
  const Region &oldCoveredRegion =
      prevOutputLayer ? prevOutputLayer->getState().coveredRegion
                      : kEmptyRegion;
  ...
  // TODO(b/121291683): Why does this not use visibleRegion? (see
  // outputSpaceVisibleRegion below)
  const auto &outputState = getState();
  Region drawRegion(
      outputState.transform.transform(visibleNonTransparentRegion));
  ...
  outputLayerState.outputSpaceVisibleRegion =
      outputState.transform.transform(visibleNonShadowRegion.intersect(
          outputState.layerStackSpace.getContent()));
  outputLayerState.shadowRegion = shadowRegion;
  outputLayerState.outputSpaceBlockingRegionHint =
      layerFEState->compositionType == Composition::DISPLAY_DECORATION
          ? outputState.transform.transform(transparentRegion.intersect(
                outputState.layerStackSpace.getContent()))
          : Region();
  if (CC_UNLIKELY(computeAboveCoveredExcludingOverlays)) {
...
void Output::commitPictureProfilesToCompositionState() {
  ...
  std::priority_queue<::android::compositionengine::OutputLayer *,
                      std::vector<::android::compositionengine::OutputLayer *>,
                      decltype(compare)>
      layersWithProfiles;
  for (auto outputLayer : getOutputLayersOrderedByZ()) {
  ...
  // TODO(b/337330263): Use the default display picture profile from
  // SurfaceFlinger
  editState().pictureProfileHandle = PictureProfileHandle::NONE;

  // When layer-specific picture processing is supported, apply as many high
  // priority profiles as possible to the layers, and ignore the low priority
  // layers.
  if (getMaxLayerPictureProfiles() > 0) {
    for (int i = 0;
         i < getMaxLayerPictureProfiles() && !layersWithProfiles.empty();
    ...
    // No layer-specific picture processing, so apply the highest priority
    // picture profile to the entire display.
  } else if (!layersWithProfiles.empty()) {
    editState().pictureProfileHandle =
        layersWithProfiles.top()->getPictureProfileHandle();
    layersWithProfiles.top()->getLayerFE().onPictureProfileCommitted();
  }
}

void Output::setReleasedLayers(
...
void Output::updateCompositionState(
  ...
  for (auto *layer : getOutputLayersOrderedByZ()) {
    layer->updateCompositionState(
        ...
        refreshArgs.devOptForceClientComposition || forceClientComposition,
...
void Output::writeCompositionState(
    const compositionengine::CompositionRefreshArgs &refreshArgs) {
  SFTRACE_CALL();
  ALOGV(__FUNCTION__);
  ...
  if (auto frameTargetPtrOpt =
          getDisplayId()
              .and_then(PhysicalDisplayId::tryCast)
              .and_then([&refreshArgs](PhysicalDisplayId id) {
                return refreshArgs.frameTargets.get(id);
              })) {
    editState().earliestPresentTime =
        frameTargetPtrOpt->get()->earliestPresentTime();
    editState().expectedPresentTime =
        frameTargetPtrOpt->get()->expectedPresentTime().ns();
    const auto debugPresentDelay =
        frameTargetPtrOpt->get()->debugPresentDelay();
  ...
  for (auto *layer : getOutputLayersOrderedByZ()) {
    ...
    if (overrideInfo.buffer != nullptr) {
      if (previousOverride &&
          overrideInfo.buffer->getBuffer() == previousOverride) {
        ...
        if (overrideInfo.peekThroughLayer) {
          ...
          peekThroughLayer->writeStateToHWC(includeGeometry, false, z++,
                                            overrideZ, isPeekingThrough);
          outputLayerHash ^= android::hashCombine(
              reinterpret_cast<uint64_t>(&peekThroughLayer->getLayerFE()), z,
              includeGeometry, overrideZ, isPeekingThrough,
    ...
    layer->writeStateToHWC(includeGeometry, skipLayer, z++, overrideZ,
                           isPeekingThrough);
    if (!skipLayer) {
      outputLayerHash ^= android::hashCombine(
          reinterpret_cast<uint64_t>(&layer->getLayerFE()), z, includeGeometry,
          overrideZ, isPeekingThrough, layer->requiresClientComposition());
    }
  }
...
compositionengine::OutputLayer *
Output::findLayerRequestingBackgroundComposition() const {
  compositionengine::OutputLayer *layerRequestingBgComposition = nullptr;
  for (auto *layer : getOutputLayersOrderedByZ()) {
    const auto *compState = layer->getLayerFE().getCompositionState();

    // If any layer has a sideband stream, we will disable blurs. In that case,
    // we don't want to force client composition because of the blur.
    ...
    if (compState->backgroundBlurRadius > 0 ||
        compState->blurRegions.size() > 0) {
...
void Output::updateColorProfile(
    const compositionengine::CompositionRefreshArgs &refreshArgs) {
...
// can only be one of
//  - Dataspace::SRGB (use legacy dataspace and let HWC saturate when colors are
//  enhanced)
...
ui::Dataspace Output::getBestDataspace(ui::Dataspace *outHdrDataSpace,
  ...
  *outHdrDataSpace = ui::Dataspace::UNKNOWN;
  ...
  // An Output's layers may be stale when it is disabled. As a consequence, the
  // layers returned by getOutputLayersOrderedByZ may not be in a valid state
  // and it is not safe to access their properties. Return a default dataspace
  // value in this case.
  if (!getState().isEnabled) {
...
compositionengine::Output::ColorProfile Output::pickColorProfile(
  ...
  ui::Dataspace bestDataSpace =
      getBestDataspace(&hdrDataSpace, &isHdrClientComposition);
  ...
  const bool isHdr = hdrDataSpace != ui::Dataspace::UNKNOWN &&
                     !mDisplayColorProfile->hasLegacyHdrSupport(hdrDataSpace) &&
  ...
  case OutputColorSetting::kEnhanced:
    intent =
        isHdr ? ui::RenderIntent::TONE_MAP_ENHANCE : ui::RenderIntent::ENHANCE;
    break;
  ...
  mDisplayColorProfile->getBestColorMode(bestDataSpace, intent, &outDataSpace,
                                         &outMode, &outRenderIntent);
...
void Output::beginFrame() {
  ...
  ALOGV("%s: %s composition for %s (%cdirty %cempty %cwasEmpty)", __func__,
        mMustRecompose ? "doing" : "skipping", getName().c_str(),
...
ftl::Future<std::monostate>
Output::presentFrameAndReleaseLayersAsync(bool flushEvenWhenDisabled) {
  return ftl::Future<bool>(
             mHwComposerAsyncWorker->send([this, flushEvenWhenDisabled]() {
               presentFrameAndReleaseLayers(flushEvenWhenDisabled);
               return true;
             }))
...
GpuCompositionResult Output::prepareFrameAsync() {
  ...
  const bool predictionSucceeded =
      dequeueSucceeded && changes == previousChanges;
  state.strategyPrediction = predictionSucceeded
                                 ? CompositionStrategyPredictionState::SUCCESS
  ...
  if (!predictionSucceeded) {
    ...
    finishPrepareFrame();
    // Track the dequeued buffer to reuse so we don't need to dequeue another
    // one.
...
void Output::devOptRepaintFlash(
    const compositionengine::CompositionRefreshArgs &refreshArgs) {
...
void Output::finishFrame(GpuCompositionResult &&result) {
  ...
  if (isPowerHintSessionEnabled() && !isPowerHintSessionGpuReportingEnabled()) {
    ...
    setHintSessionGpuFence(std::make_unique<FenceTime>(
        sp<Fence>::make(dup(optReadyFence->get()))));
  }
  // swap buffers (presentation)
  mRenderSurface->queueBuffer(std::move(*optReadyFence),
                              getHdrSdrRatio(buffer));
}
...
void Output::updateProtectedContentState() {
  ...
  // We need to set the render surface as protected (DRM) if all the following
  // conditions are met:
  ...
  if (isProtected && supportsProtectedContent) {
    ...
    bool needsProtected =
        std::any_of(layers.begin(), layers.end(), [](auto *layer) {
...
bool Output::dequeueRenderBuffer(
    base::unique_fd *bufferFence,
...
Output::composeSurfaces(const Region &debugRegion,
                        std::shared_ptr<renderengine::ExternalTexture> tex,
                        base::unique_fd &fd) {
  SFTRACE_CALL();
  ALOGV(__FUNCTION__);
  ...
  std::vector<LayerFE::LayerSettings> clientCompositionLayers =
      generateClientCompositionRequests(
          supportsProtectedContent, clientCompositionDisplay.outputDataspace,
          clientCompositionLayersFE);
  appendRegionFlashRequests(debugRegion, clientCompositionLayers);

  OutputCompositionState &outputCompositionState = editState();
  // Check if the client composition requests were rendered into the provided
  // graphic buffer. If so, we can reuse the buffer and avoid client
  // composition.
  if (mClientCompositionRequestCache) {
    ...
    mClientCompositionRequestCache->add(tex->getBuffer()->getId(),
                                        clientCompositionDisplay,
                                        clientCompositionLayers);
  }

  // We boost GPU frequency here because there will be color spaces conversion
  // or complex GPU shaders and it's expensive. We boost the GPU frequency so
  // that GPU composition can finish in time. We must reset GPU frequency
  // afterwards, because high frequency consumes extra battery.
  const bool expensiveRenderingExpected = std::any_of(
      clientCompositionLayers.begin(), clientCompositionLayers.end(),
      [outputDataspace =
  ...
  if (isPowerHintSessionEnabled()) {
    if (fence != Fence::NO_FENCE && fence->isValid() &&
      ...
      if (isPowerHintSessionGpuReportingEnabled()) {
        // the order of the two calls here matters as we should check if the
        // previously tracked fence has signaled first and archive the previous
        // start time
...
renderengine::DisplaySettings Output::generateClientCompositionDisplaySettings(
  ...
  clientCompositionDisplay.physicalDisplay =
      outputState.framebufferSpace.getContent();
  ...
  clientCompositionDisplay.outputDataspace =
      mDisplayColorProfile->hasWideColorGamut() ? outputState.dataspace
                                                : ui::Dataspace::UNKNOWN;

  // If we have a valid current display brightness use that, otherwise fall back
  // to the display's max desired
  clientCompositionDisplay.currentLuminanceNits =
      outputState.displayBrightnessNits > 0.f
...
std::vector<LayerFE::LayerSettings>
Output::generateClientCompositionRequests(bool supportsProtectedContent,
                                          ui::Dataspace outputDataspace,
                                          std::vector<LayerFE *> &outLayerFEs) {
  std::vector<LayerFE::LayerSettings> clientCompositionLayers;
  ...
  for (auto *layer : getOutputLayersOrderedByZ()) {
    ...
    ALOGV("  Composition type: client %d clear %d", clientComposition,
          clearClientComposition);

    // If the layer casts a shadow but the content casting the shadow is
    // occluded, skip composing the non-shadow content and only draw the
    // shadows.
    ...
    if (clientComposition || clearClientComposition) {
      if (auto overrideSettings = layer->getOverrideCompositionSettings()) {
        if (overrideSettings->bufferId != previousOverrideBufferId) {
          ...
          ALOGV("Replacing [%s] with override in RE",
        ...
        if (auto clientCompositionSettings =
          ...
          clientCompositionLayers.push_back(
              std::move(*clientCompositionSettings));
...
bool Output::isPowerHintSessionEnabled() { return false; }

bool Output::isPowerHintSessionGpuReportingEnabled() { return false; }
...
void Output::presentFrameAndReleaseLayers(bool flushEvenWhenDisabled) {
  ...
  if (!getState().isEnabled) {
    if (flushEvenWhenDisabled &&
        FlagManager::getInstance().flush_buffer_slots_to_uncache()) {
  ...
  for (auto *layer : getOutputLayersOrderedByZ()) {
    ...
    if (auto hwcLayer = layer->getHwcLayer()) {
      if (auto f = frame.layerFences.find(hwcLayer);
          f != frame.layerFences.end()) {
...
bool Output::getSkipColorTransform() const { return true; }
...
compositionengine::Output::FrameFences Output::presentFrame() {
  ...
  if (getState().usesClientComposition) {
    result.clientTargetAcquireFence =
        mRenderSurface->getClientTargetAcquireFence();
  }
  return result;
}
...
Output::getOverlaySupport() {
  return nullptr;
}

bool Output::canPredictCompositionStrategy(
...
void Output::finishPrepareFrame() {
  ...
  mRenderSurface->prepareFrame(state.usesClientComposition,
                               state.usesDeviceComposition);
}

bool Output::mustRecompose() const { return mMustRecompose; }

float Output::getHdrSdrRatio(
    const std::shared_ptr<renderengine::ExternalTexture> &buffer) const {
  if (buffer == nullptr) {
    return 1.0f;
  }
  ...
  if (getState().displayBrightnessNits < 0.0f ||
      getState().sdrWhitePointNits <= 0.0f ||
...
bool Output::hasPictureProcessing() const { return false; }

int32_t Output::getMaxLayerPictureProfiles() const { return 0; }
...
void Output::applyPictureProfile() {
  ...
  // TODO(b/337330263): Move this into the Display class and add a Display unit
  // test.
  ...
  if (auto displayId = PhysicalDisplayId::tryCast(*getDisplayId())) {
    ...
    const status_t error = hwc.setDisplayPictureProfileHandle(
        *displayId, getState().pictureProfileHandle);
    ALOGE_IF(error, "setDisplayPictureProfileHandle failed for %s: %d, (%s)",
             getName().c_str(), error, strerror(-error));
  }
}

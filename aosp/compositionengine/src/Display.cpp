...
bool Display::isValid() const { return Output::isValid() && mPowerAdvisor; }

DisplayId Display::getId() const { return mId; }

bool Display::isSecure() const { return getState().isSecure; }

void Display::setSecure(bool secure) { editState().isSecure = secure; }

bool Display::isVirtual() const { return mId.isVirtual(); }

ftl::Optional<DisplayId> Display::getDisplayId() const { return mId; }
...
void Display::setColorTransform(
    const compositionengine::CompositionRefreshArgs &args) {
  Output::setColorTransform(args);
  const auto halDisplayId = HalDisplayId::tryCast(mId);
  if (mIsDisconnected || !halDisplayId ||
      CC_LIKELY(!args.colorTransformMatrix)) {
    return;
  }

  auto &hwc = getCompositionEngine().getHwComposer();
  status_t result =
      hwc.setColorTransform(*halDisplayId, *args.colorTransformMatrix);
  ALOGE_IF(result != NO_ERROR,
           "Failed to set color transform on display \"%s\": %d",
...
void Display::setColorProfile(const ColorProfile &colorProfile) {
  ...
  getCompositionEngine().getHwComposer().setActiveColorMode(
      *physicalId, colorProfile.mode, colorProfile.renderIntent);
}

void Display::dump(std::string &out) const {
  const char *const type = isVirtual() ? "virtual" : "physical";
  base::StringAppendF(&out, "Display %s (%s, \"%s\")", to_string(mId).c_str(),
                      type, getName().c_str());
...
void Display::createDisplayColorProfile(
    const DisplayColorProfileCreationArgs &args) {
  setDisplayColorProfile(
      compositionengine::impl::createDisplayColorProfile(args));
}

void Display::createRenderSurface(const RenderSurfaceCreationArgs &args) {
  setRenderSurface(compositionengine::impl::createRenderSurface(
      getCompositionEngine(), *this, args));
}
...
std::unique_ptr<compositionengine::OutputLayer> Display::createOutputLayer(
  ...
  if (const auto halDisplayId = HalDisplayId::tryCast(mId);
    ...
    ALOGE_IF(!hwcLayer,
             "Failed to create a HWC layer for a HWC supported display %s",
             getName().c_str());
...
void Display::setReleasedLayers(
  ...
  for (auto *outputLayer : getOutputLayersOrderedByZ()) {
    ...
    const bool hasQueuedFrames = std::any_of(
        refreshArgs.layersWithQueuedFrames.cbegin(),
...
void Display::applyDisplayBrightness(bool applyImmediately) {
  ...
  // Clear out the display brightness now that it's been communicated to
  // composer.
...
bool Display::chooseCompositionStrategy(
  ...
  if (status_t result = hwc.getDeviceCompositionChanges(
      ...
      result != NO_ERROR) {
    ALOGE("chooseCompositionStrategy failed for %s: %d (%s)", getName().c_str(),
          result, strerror(-result));
    return false;
  }

  if (isPowerHintSessionEnabled()) {
    mPowerAdvisor->setHwcValidateTiming(mId, hwcValidateStartTime,
                                        TimePoint::now());
    if (auto halDisplayId = HalDisplayId::tryCast(mId)) {
      mPowerAdvisor->setSkippedValidate(mId,
                                        hwc.getValidateSkipped(*halDisplayId));
    }
  }
...
void Display::applyCompositionStrategy(
    const std::optional<DeviceRequestedChanges> &changes) {
  if (changes) {
...
bool Display::getSkipColorTransform() const {
  auto &hwc = getCompositionEngine().getHwComposer();
  if (auto halDisplayId = HalDisplayId::tryCast(mId)) {
    return hwc.hasDisplayCapability(
        *halDisplayId, DisplayCapability::SKIP_CLIENT_COLOR_TRANSFORM);
  }
...
void Display::applyDisplayRequests(const DisplayRequests &displayRequests) {
  ...
  state.flipClientTarget =
      (static_cast<uint32_t>(displayRequests) &
...
void Display::applyClientTargetRequests(
    const ClientTargetProperty &clientTargetProperty) {
  if (static_cast<ui::Dataspace>(
          clientTargetProperty.clientTargetProperty.dataspace) ==
      ui::Dataspace::UNKNOWN) {
    return;
  }

  editState().dataspace = static_cast<ui::Dataspace>(
      clientTargetProperty.clientTargetProperty.dataspace);
  ...
  getRenderSurface()->setBufferPixelFormat(static_cast<ui::PixelFormat>(
      clientTargetProperty.clientTargetProperty.pixelFormat));
}

void Display::applyLayerLutsToLayers(const LayerLuts &layerLuts) {
  auto &mapper =
      getCompositionEngine().getHwComposer().getLutFileDescriptorMapper();
  for (auto *layer : getOutputLayersOrderedByZ()) {
    auto hwcLayer = layer->getHwcLayer();
    if (!hwcLayer) {
      continue;
...
compositionengine::Output::FrameFences Display::presentFrame() {
  ...
  if (isPowerHintSessionEnabled() && getState().earliestPresentTime) {
    mPowerAdvisor->setHwcPresentDelayedTime(mId,
  ...
  for (const auto *layer : getOutputLayersOrderedByZ()) {
    ...
    fences.layerFences.emplace(
        hwcLayer, hwc.getLayerReleaseFence(*halDisplayIdOpt, hwcLayer));
  }
...
// For ADPF GPU v0 this is expected to set start time to when the GPU commands
// are submitted with fence returned, i.e. when RenderEngine flushes the
// commands and returns the draw fence.
...
bool Display::hasPictureProcessing() const { return mHasPictureProcessing; }

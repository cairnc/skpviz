...
#define HAL_PRIORITY_URGENT_DISPLAY (-8)
     ...
     * the image to the first value of the next row.  It includes the width of
     * the image plus padding. ystride is the stride of the luma plane. cstride
     * is the stride of the chroma planes.
     *
     ...
     *
     * The android_flex_layout structure is used with any pixel format that can
     * be represented by it, such as:
     ...
     *    the structures
     *
     * Vertical increment (aka. row increment or stride) describes the distance
     * in bytes from the first pixel of one row to the first pixel of the next
     * row (below) for the component plane. This can be negative.
     *
     ...
     *
     * The bit-depth of each component can be arbitrary, as long as the pixels
     * are laid out on whole bytes, in native byte-order, using the most
     * significant bits of each unit.
     */
    ...
    typedef enum android_flex_format
    {
        ...
        FLEX_FORMAT_YCbCr =
            FLEX_COMPONENT_Y | FLEX_COMPONENT_Cb | FLEX_COMPONENT_Cr,
        FLEX_FORMAT_YCbCrA = FLEX_FORMAT_YCbCr | FLEX_COMPONENT_A,
        FLEX_FORMAT_RGB =
            FLEX_COMPONENT_R | FLEX_COMPONENT_G | FLEX_COMPONENT_B,
    ...
    typedef struct android_flex_layout
    {
        ...
        /* a plane for each component; ordered in increasing component value
           order. E.g. FLEX_FORMAT_RGBA maps 0 -> R, 1 -> G, etc. Can be NULL
           for FLEX_FORMAT_INVALID */
        android_flex_plane_t *planes;
    } android_flex_layout_t;

    /**
     * Structure used to define depth point clouds for format
     * HAL_PIXEL_FORMAT_BLOB with dataSpace value of HAL_DATASPACE_DEPTH. When
     * locking a native buffer of the above format and dataSpace value, the
     * vaddr pointer can be cast to this structure.
     *
     * A variable-length list of (x,y,z, confidence) 3D points, as floats.  (x,
     * y, z) represents a measured point's position, with the coordinate system
     * defined by the data source.  Confidence represents the estimated
     * likelihood that this measurement is correct. It is between 0.f and 1.f,
     * inclusive, with 1.f == 100% confidence.
     *

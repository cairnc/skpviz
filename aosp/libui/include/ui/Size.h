...
struct Size
{
    ...
    constexpr Size(T w, T h)
        : width(clamp<int32_t>(w)), height(clamp<int32_t>(h))
    {
    }
    ...
    template <typename T> void setWidth(T v)
    {
        width = clamp<int32_t>(v);
    }

    template <typename T> void setHeight(T v)
    {
    ...
    template <typename T> void set(T w, T h)
    {
        set(Size(w, h));
    }
    ...
    // Takes a value of type FromType, and ensures it can be represented as a
    // value of type ToType, clamping the input value to the output range if
    // necessary.
    ...
    static constexpr remove_cvref_t<ToType> clamp(FromType v)
    {
        ...
        using CommonLowestType =
            std::conditional_t<CommonLimits::is_integer,
                               std::make_signed_t<std::conditional_t<
                                   CommonLimits::is_integer, CommonHighestType,
                                   int /* not used */>>,
                               CommonHighestType>;
        ...
        constexpr auto commonClampHighest =
            std::min(static_cast<CommonHighestType>(fromHighest),
                     static_cast<CommonHighestType>(toHighest));
        constexpr auto commonClampLowest =
            std::max(static_cast<CommonLowestType>(fromLowest),
                     static_cast<CommonLowestType>(toLowest));

        constexpr auto fromClampHighest =
            static_cast<BareFromType>(commonClampHighest);
        constexpr auto fromClampLowest =
            static_cast<BareFromType>(commonClampLowest);

        // A clamp is needed only if the range we are clamping to is not the
        // same as the range of the input.
        constexpr bool isClampNeeded = (fromLowest != fromClampLowest) ||
                                       (fromHighest != fromClampHighest);
        ...
        return v >= fromClampHighest ? toClampHighest
                                     : static_cast<BareToType>(v);
    }
};

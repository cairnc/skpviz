...
class Rect : public ARect, public LightFlattenablePod<Rect>
{
  public:
    ...
    template <typename T> inline Rect(T w, T h)
    {
        if (w > INT32_MAX)
        {
            w = INT32_MAX;
    ...
    // rectangle's width
    __attribute__((no_sanitize("signed-integer-overflow"))) inline int32_t
    getWidth() const
    {
        return right - left;
    }
    ...
    {
        return ui::Size(getWidth(), getHeight());
    }

    __attribute__((no_sanitize("signed-integer-overflow"))) inline Rect
    getBounds() const
    {
    ...
    {
        return Rect(FloatRect(left * scaleX, top * scaleY, right * scaleX,
                              bottom * scaleY));
    }
...
namespace std
{
template <> struct hash<android::Rect>
{
    size_t operator()(const android::Rect &rect) const
    {
        return android::hashCombine(rect.left, rect.top, rect.right,
                                    rect.bottom);
    }
};
} // namespace std

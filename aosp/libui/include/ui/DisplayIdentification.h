...
struct HdmiPhysicalAddress {
  // The address describes the path from the display sink in the network of
  // connected HDMI devices. The format of the address is "a.b.c.d". For
  // example, address 2.1.0.0 means we are connected to port 1 of a device which
  // is connected to port 2 of the sink.
  uint8_t a, b, c, d;
};
...
std::optional<DisplayIdentificationInfo>
parseDisplayIdentificationData(uint8_t port, const DisplayIdentificationData &);

...
// Product-specific information about the display or the directly connected
// device on the display chain. For example, if the display is transitively
// connected, this field may contain product information about the intermediate
// device.
struct DeviceProductInfo {
  ...
  struct ManufactureWeekAndYear : ManufactureYear {
    // 1-base week number. Week numbering may not be consistent between
    // manufacturers.
    uint8_t week;
  };
  ...
  using ManufactureOrModelDate =
      std::variant<ModelYear, ManufactureYear, ManufactureWeekAndYear>;

...
class Region : public LightFlattenable<Region>
{
  public:
    ...
    const Region mergeExclusive(const Region &rhs, int dx,
                                int dy) const WARN_UNUSED;
  ...
  private:
    ...
    const Region operation(const Region &rhs, int dx, int dy,
                           uint32_t op) const;

    static void boolean_operation(uint32_t op, Region &dst, const Region &lhs,
...
namespace std
{
template <> struct hash<android::Region>
{

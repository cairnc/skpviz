...
class ColorSpace
{
  public:
     ...
     * (clamp(x, 0, 1)).
     */
    ColorSpace(const std::string &name, const mat3 &rgbToXYZ,
               transfer_function OETF = linearResponse,
               transfer_function EOTF = linearResponse,
               clamping_function clamper = saturate<float>) noexcept;
     ...
     */
    ColorSpace(const std::string &name, const mat3 &rgbToXYZ, float gamma,
               clamping_function clamper = saturate<float>) noexcept;
     ...
     * (clamp(x, 0, 1)).
     */
    ColorSpace(const std::string &name, const std::array<float2, 3> &primaries,
     ...
     */
    ColorSpace(const std::string &name, const std::array<float2, 3> &primaries,
               const float2 &whitePoint, const TransferParameters parameters,
               clamping_function clamper = saturate<float>) noexcept;
    ...
    static std::unique_ptr<float3[]>
    createLUT(uint32_t size, const ColorSpace &src, const ColorSpace &dst);

  private:
    static constexpr mat3
    computeXYZMatrix(const std::array<float2, 3> &primaries,
                     const float2 &whitePoint);
...
class ColorSpaceConnector
{
  public:
    ...
    constexpr const ColorSpace &getSource() const noexcept
    {
        return mSource;
    }
    constexpr const ColorSpace &getDestination() const noexcept
    {
        return mDestination;
    }

    constexpr const mat3 &getTransform() const noexcept
    {
        return mTransform;
    }
    ...
    constexpr float3 transform(const float3 &v) const noexcept
    {
        ...
        return apply(mDestination.fromLinear(mTransform * linear),
                     mDestination.getClamper());
    }

...
struct LayerStack {
  ...
  template <typename T> static constexpr LayerStack fromValue(T v) {
...
inline bool operator!=(LayerStack lhs, LayerStack rhs) { return !(lhs == rhs); }
...
struct LayerFilter {
  ...
  // True if the layer is only output to internal displays, i.e. excluded from
  // screenshots, screen recordings, and mirroring to virtual or external
  // displays. Used for display cutout overlays.
  ...
  bool includes(LayerFilter other) const {
    ...
    if (other.layerStack == INVALID_LAYER_STACK ||
        other.layerStack != layerStack) {
      return false;
    }

    // The output must be to an internal display if the input filter has that
    // constraint.

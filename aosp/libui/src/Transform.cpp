...
{
    return mMatrix[0][0] == other.mMatrix[0][0] &&
...
Rect Transform::transform(const Rect &bounds, bool roundOutwards) const
{
    ...
    if (roundOutwards)
    {
        r.left = static_cast<int32_t>(
            floorf(std::min({lt[0], rt[0], lb[0], rb[0]})));
        r.top = static_cast<int32_t>(
            floorf(std::min({lt[1], rt[1], lb[1], rb[1]})));
        r.right =
            static_cast<int32_t>(ceilf(std::max({lt[0], rt[0], lb[0], rb[0]})));
        r.bottom =
            static_cast<int32_t>(ceilf(std::max({lt[1], rt[1], lb[1], rb[1]})));
    }
    else
    {
        r.left = static_cast<int32_t>(
            floorf(std::min({lt[0], rt[0], lb[0], rb[0]}) + 0.5f));
        r.top = static_cast<int32_t>(
            floorf(std::min({lt[1], rt[1], lb[1], rb[1]}) + 0.5f));
        r.right = static_cast<int32_t>(
            floorf(std::max({lt[0], rt[0], lb[0], rb[0]}) + 0.5f));
        r.bottom = static_cast<int32_t>(
            floorf(std::max({lt[1], rt[1], lb[1], rb[1]}) + 0.5f));
    }
...
Transform Transform::inverse() const
{
    ...
    else
    {
        ...
        {
            // Recalculate the type if there is a 90-degree rotation component,
            // since the inverse of ROT_90 is ROT_270 and vice versa.
...
mat4 Transform::asMatrix4() const
{
    ...
    mat4 m =
        mat4{mat4::NO_INIT}; // NO_INIT since we explicitly set every element
...
void Transform::dump(std::string &out, const char *name,
                     const char *prefix) const
{
    ...
    for (size_t i = 0; i < 3; i++)
    {
        StringAppendF(&out, "%s    %.4f  %.4f  %.4f\n", prefix,
                      ...
                      static_cast<double>(mMatrix[1][i]),
                      static_cast<double>(mMatrix[2][i]));
    }
}

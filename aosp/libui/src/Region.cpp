...
// in Android.bp. Do not #define VALIDATE_REGIONS here as it requires extra
// libs.
...
/**
 * Copy rects from the src vector into the dst vector, resolving vertical
 * T-Junctions along the way
 *
 * First pass through, divideSpanRTL will be set because the 'previous span'
 * (indexing into the dst vector) will be reversed. Each rectangle in the
 * original list, starting from the bottom, will be compared with the span
 * directly below, and subdivided as needed to resolve T-junctions.
 *
 * The resulting temporary vector will be a completely reversed copy of the
 * original, without any bottom-up T-junctions.
 *
 * Second pass through, divideSpanRTL will be false since the previous span will
 * index into the final, correctly ordered region buffer. Each rectangle will be
 * compared with the span directly above it, and subdivided to resolve any
                                           ...
                                           int spanDirection)
{
    ...
    // for all other spans, split if a t-junction exists in the span directly
    // above
    while (current >= begin)
    {
        ...
        for (int prevIndex = beginLastSpan; prevIndex <= endLastSpan;
             prevIndex++)
        {
...
/**
 * Creates a new region with the same data as the argument, but divides
 * rectangles as necessary to remove T-Junctions
 *
 * Note: the output will not necessarily be a very efficient representation of
 * the region, since it may be that a triangle-based approach would generate
 * significantly simpler geometry
 */
Region Region::createTJunctionFreeRegion(const Region &r)
{
    ...
    Region outputRegion;
    reverseRectsResolvingJunctions(reversed.data(),
...
bool Region::contains(int x, int y) const
{
    ...
    while (cur != tail)
    {
        if (y >= cur->top && y < cur->bottom && x >= cur->left &&
            x < cur->right)
        {
            return true;
        }
...
bool Region::hasSameRects(const Region &other) const
{
    ...
    if (thisRectCount != otherRectCount)
        return false;

    for (size_t i = 0; i < thisRectCount; i++)
    {
        if (thisRects[i] != otherRects[i])
            return false;
    }
    return true;
}
...
Region &Region::scaleSelf(float sx, float sy)
{
    ...
    {
        rects->left =
            static_cast<int32_t>(static_cast<float>(rects->left) * sx + 0.5f);
        rects->right =
            static_cast<int32_t>(static_cast<float>(rects->right) * sx + 0.5f);
        rects->top =
            static_cast<int32_t>(static_cast<float>(rects->top) * sy + 0.5f);
        rects->bottom =
            static_cast<int32_t>(static_cast<float>(rects->bottom) * sy + 0.5f);
        rects++;
        count--;
    }
    return *this;
...
const Region Region::operation(const Region &rhs, int dx, int dy,
                               uint32_t op) const
{
    Region result;
...
class Region::rasterizer : public region_operator<Rect>::region_rasterizer
{
  ...
  public:
    explicit rasterizer(Region &reg)
        : bounds(INT_MAX, 0, INT_MIN, 0), storage(reg.mStorage), head(), tail(),
          cur()
    {
        storage.clear();
    }
  ...
  private:
    template <typename T> static inline T min(T rhs, T lhs)
    {
        return rhs < lhs ? rhs : lhs;
    }
    template <typename T> static inline T max(T rhs, T lhs)
    {
        return rhs > lhs ? rhs : lhs;
    }
...
bool Region::validate(const Region &reg, const char *name, bool silent)
{
    ...
    while (cur != tail)
    {
        ...
        if (prev != cur)
        {
            ...
            {
                ALOGE_IF(!silent,
                         "%s: spans overlap vertically prev=%p, cur=%p", name,
                         prev, cur);
                result = false;
            }
            prev = cur;
    ...
    {
        result = false;
        ALOGE_IF(!silent, "%s: invalid bounds [%d,%d,%d,%d] vs. [%d,%d,%d,%d]",
                 name, b.left, b.top, b.right, b.bottom, reg.getBounds().left,
                 reg.getBounds().top, reg.getBounds().right,
                 reg.getBounds().bottom);
    }
...
void Region::boolean_operation(uint32_t op, Region &dst, const Region &lhs,
                               const Region &rhs, int dx, int dy)
{
    ...
    for (size_t i = 0; i < lhs_count; i++)
        sk_lhs.op(lhs_rects[i].left + dx, lhs_rects[i].top + dy,
                  lhs_rects[i].right + dx, lhs_rects[i].bottom + dy,
                  SkRegion::kUnion_Op);

    for (size_t i = 0; i < rhs_count; i++)
        sk_rhs.op(rhs_rects[i].left + dx, rhs_rects[i].top + dy,
                  rhs_rects[i].right + dx, rhs_rects[i].bottom + dy,
                  SkRegion::kUnion_Op);
    ...
    {
    case op_or:
        sk_op = SkRegion::kUnion_Op;
        name = "OR";
        break;
    case op_xor:
        sk_op = SkRegion::kUnion_XOR;
        name = "XOR";
        break;
    case op_and:
        sk_op = SkRegion::kIntersect_Op;
        name = "AND";
        break;
    case op_nand:
        sk_op = SkRegion::kDifference_Op;
        name = "NAND";
        break;
    }
    sk_dst.op(sk_lhs, sk_rhs, sk_op);

    if (sk_dst.empty() && dst.empty())
        return;
    ...
    while (!it.done())
    {
        if (head != tail)
        {
            if (head->left != it.rect().fLeft || head->top != it.rect().fTop ||
    ...
    if (!same)
    {
        ...
        while (!it.done())
        {
            ALOGD("    [%3d, %3d, %3d, %3d]", it.rect().fLeft, it.rect().fTop,
                  it.rect().fRight, it.rect().fBottom);
            it.next();
        }
    }
#endif
                               ...
                               const Rect &rhs, int dx, int dy)
{
    ...
    {
        ALOGE("Region::boolean_operation(op=%d) invalid Rect={%d,%d,%d,%d}", op,
...
status_t Region::flatten(void *buffer, size_t size) const
{
    ...
    FlattenableUtils::write(buffer, size,
                            static_cast<uint32_t>(mStorage.size()));
...
status_t Region::unflatten(void const *buffer, size_t size)
{
    ...
    mStorage.clear();
    mStorage.insert(mStorage.begin(), result.mStorage.begin(),
                    result.mStorage.end());
    return NO_ERROR;
}
...
void Region::dump(std::string &out, const char *what,

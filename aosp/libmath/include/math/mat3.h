               ...
               public TMatDebug<TMat33, T>
{
  ...
  public:
     ...
     *      \f$
     */
    template <typename U> explicit CONSTEXPR TMat33(U v);
     ...
     *      \f$
     */
    template <typename U> explicit CONSTEXPR TMat33(const TVec3<U> &v);

    /**
     * construct from another matrix of the same size
     */
    template <typename U> explicit CONSTEXPR TMat33(const TMat33<U> &rhs);
    ...
    CONSTEXPR TMat33(const TVec3<A> &v0, const TVec3<B> &v1,
                     const TVec3<C> &v2);
    ...
    CONSTEXPR TMat33(A m00, B m01, C m02, D m10, E m11, F m12, G m20, H m21,
                     I m22);

    /**
     * construct from a quaternion
     */
    template <typename U> explicit CONSTEXPR TMat33(const TQuaternion<U> &q);

    /**
     * construct from a C array in column major form.
     */
    template <typename U> explicit CONSTEXPR TMat33(U const *rawArray);

    /**
     * orthogonalize only works on matrices of size 3x3
     */
    friend inline CONSTEXPR TMat33 orthogonalize(const TMat33 &m)
    {
...
template <typename T> CONSTEXPR TMat33<T>::TMat33()
{
...
template <typename T> template <typename U> CONSTEXPR TMat33<T>::TMat33(U v)
{
...
template <typename A, typename B, typename C, typename D, typename E,
          typename F, typename G, typename H, typename I>
CONSTEXPR TMat33<T>::TMat33(A m00, B m01, C m02, D m10, E m11, F m12, G m20,
                            H m21, I m22)
{
...
template <typename T>
template <typename A, typename B, typename C>
CONSTEXPR TMat33<T>::TMat33(const TVec3<A> &v0, const TVec3<B> &v1,
                            const TVec3<C> &v2)
{
    m_value[0] = v0;
    m_value[1] = v1;
...
CONSTEXPR typename TMat33<U>::col_type PURE operator*(const TMat33<T> &lhs,
                                                      const TVec3<U> &rhs)
{
...
// vector
template <typename T, typename U>
CONSTEXPR typename TMat33<U>::row_type PURE operator*(const TVec3<U> &lhs,
                                                      const TMat33<T> &rhs)
{
...
template <typename T, typename U>
constexpr
    typename std::enable_if<std::is_arithmetic<U>::value, TMat33<T>>::type PURE
...
//------------------------------------------------------------------------------
template <typename T> CONSTEXPR TMat33<T> orthogonalize(const TMat33<T> &m)
{

...
/*
 * TVec{Add|Product}Operators implements basic arithmetic and basic compound
 * assignments operators on a vector of type BASE<T>.
 *
 * BASE only needs to implement operator[] and size().
 * By simply inheriting from TVec{Add|Product}Operators<BASE, T> BASE will
 * automatically get all the functionality here.
 */

template <template <typename T> class VECTOR, typename T> class TVecAddOperators
{
  public:
    /* compound assignment from a another vector of the same size but different
     * element type.
     */
    template <typename OTHER> VECTOR<T> &operator+=(const VECTOR<OTHER> &v)
    {
...
class TVecUnaryOperators
{
  public:
    ...
    // This isn't strictly a unary operator, but it is a common place shared
    // between both matrix and vector classes
    size_t hash() const
    {
        VECTOR<T> const &rv(static_cast<VECTOR<T> const &>(*this));
        size_t hashed = 0;
 ...
 *
 * BASE only needs to implement operator[] and size().
 * By simply inheriting from TVecComparisonOperators<BASE, T> BASE will
 * automatically get all the functionality here.
 */
template <template <typename T> class VECTOR, typename T>
class TVecComparisonOperators
{
  public:
    ...
    template <typename RT>
    friend inline bool PURE operator>(const VECTOR<T> &lv, const VECTOR<RT> &rv)
    {
        for (size_t i = 0; i < lv.size(); i++)
        {
            if (lv[i] == rv[i])
    ...
    template <typename RT>
    friend inline constexpr bool PURE operator<=(const VECTOR<T> &lv,
                                                 const VECTOR<RT> &rv)
    {
        return !(lv > rv);
    }
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool> PURE equal(const VECTOR<T> &lv,
                                                    const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool> PURE notEqual(const VECTOR<T> &lv,
                                                       const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool> PURE lessThan(const VECTOR<T> &lv,
                                                       const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool>
        PURE lessThanEqual(const VECTOR<T> &lv, const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
        {
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool> PURE greaterThan(const VECTOR<T> &lv,
                                                          const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
    ...
    template <typename RT>
    friend inline CONSTEXPR VECTOR<bool>
        PURE greaterThanEqual(const VECTOR<T> &lv, const VECTOR<RT> &rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++)
        {
...
template <template <typename T> class VECTOR, typename T> class TVecFunctions
{
  public:
    ...
    template <typename RT>
    friend inline constexpr T PURE distance(const VECTOR<T> &lv,
                                            const VECTOR<RT> &rv)
    {
        return length(rv - lv);
    }
    ...
    template <typename RT>
    friend inline constexpr T PURE distance2(const VECTOR<T> &lv,
                                             const VECTOR<RT> &rv)
    {
    ...
    friend inline CONSTEXPR VECTOR<T> PURE fma(const VECTOR<T> &lv,
                                               const VECTOR<T> &rv, VECTOR<T> a)
    {
        for (size_t i = 0; i < lv.size(); i++)
        {
    ...
    friend inline CONSTEXPR VECTOR<T> PURE apply(VECTOR<T> v,
    ...
    template <typename R>
    friend inline CONSTEXPR VECTOR<R> PURE map(VECTOR<T> v,

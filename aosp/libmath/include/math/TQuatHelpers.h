...
/*
 * TQuatProductOperators implements basic arithmetic and basic compound
 * assignment operators on a quaternion of type BASE<T>.
 *
 * BASE only needs to implement operator[] and size().
 * By simply inheriting from TQuatProductOperators<BASE, T> BASE will
 * automatically get all the functionality here.
 */
...
{
  public:
    /* compound assignment from a another quaternion of the same size but
    ...
    friend inline constexpr QUATERNION<T>
        PURE operator*(const QUATERNION<T> &q, const QUATERNION<RT> &r)
    {
        // could be written as:
        //  return QUATERNION<T>(
        ...
        return QUATERNION<T>(q.w * r.w - q.x * r.x - q.y * r.y - q.z * r.z,
    ...
    template <typename RT>
    friend inline constexpr TVec3<T> PURE operator*(const QUATERNION<T> &q,
                                                    const TVec3<RT> &v)
    {
    ...
    /* For quaternions, we use explicit "by a scalar" products because it's much
    ...
    friend inline constexpr QUATERNION<T> PURE operator*(T scalar,
                                                         QUATERNION<T> q)
    {
        // don't pass q by reference because we need a copy anyways
        return q *= scalar;
...
class TQuatFunctions
{
  public:
    ...
    template <typename RT>
    friend inline constexpr T PURE dot(const QUATERNION<T> &p,
                                       const QUATERNION<RT> &q)
    {
        return p.x * q.x + p.y * q.y + p.z * q.z + p.w * q.w;
    }

    friend inline constexpr T PURE norm(const QUATERNION<T> &q)
    {
        return std::sqrt(dot(q, q));
    }

    friend inline constexpr T PURE length(const QUATERNION<T> &q)
    {
        return norm(q);
    }

    friend inline constexpr T PURE length2(const QUATERNION<T> &q)
    {
        return dot(q, q);
    }

    friend inline constexpr QUATERNION<T> PURE normalize(const QUATERNION<T> &q)
    {
        return length(q) ? q / length(q) : QUATERNION<T>(1);
    }

    friend inline constexpr QUATERNION<T> PURE conj(const QUATERNION<T> &q)
    {
        return QUATERNION<T>(q.w, -q.x, -q.y, -q.z);
    }

    friend inline constexpr QUATERNION<T> PURE inverse(const QUATERNION<T> &q)
    {
        return conj(q) * (1 / dot(q, q));
    }

    friend inline constexpr T PURE real(const QUATERNION<T> &q)
    {
        return q.w;
    }

    friend inline constexpr TVec3<T> PURE imaginary(const QUATERNION<T> &q)
    {
        return q.xyz;
    }

    friend inline constexpr QUATERNION<T> PURE unreal(const QUATERNION<T> &q)
    {
        return QUATERNION<T>(q.xyz, 0);
    }

    friend inline constexpr QUATERNION<T> PURE cross(const QUATERNION<T> &p,
                                                     const QUATERNION<T> &q)
    {
        return unreal(p * q);
    }

    friend inline QUATERNION<T> PURE exp(const QUATERNION<T> &q)
    {
        const T nq(norm(q.xyz));
        return std::exp(q.w) * QUATERNION<T>((sin(nq) / nq) * q.xyz, cos(nq));
    }

    friend inline QUATERNION<T> PURE log(const QUATERNION<T> &q)
    {
        const T nq(norm(q));
        return QUATERNION<T>((std::acos(q.w / nq) / norm(q.xyz)) * q.xyz,
                             log(nq));
    }

    friend inline QUATERNION<T> PURE pow(const QUATERNION<T> &q, T a)
    {
        // could also be computed as: exp(a*log(q));
        const T nq(norm(q));
        const T theta(a * std::acos(q.w / nq));
        return std::pow(nq, a) *
               QUATERNION<T>(normalize(q.xyz) * std::sin(theta),
                             std::cos(theta));
    }

    friend inline QUATERNION<T> PURE slerp(const QUATERNION<T> &p,
                                           const QUATERNION<T> &q, T t)
    {
    ...
    friend inline constexpr QUATERNION<T> PURE lerp(const QUATERNION<T> &p,
                                                    const QUATERNION<T> &q, T t)
    {
        return ((1 - t) * p) + (t * q);
    }
    ...
    friend inline constexpr QUATERNION<T>
        PURE nlerp(const QUATERNION<T> &p, const QUATERNION<T> &q, T t)
    {
        return normalize(lerp(p, q, t));
    }

    friend inline constexpr QUATERNION<T> PURE positive(const QUATERNION<T> &q)
    {
        return q.w < 0 ? -q : q;
    }
};

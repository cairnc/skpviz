...
inline constexpr int transpose(int v)
{
    return v;
}
inline constexpr float transpose(float v)
{
    return v;
}
inline constexpr double transpose(double v)
{
    return v;
}

inline constexpr int trace(int v)
{
    return v;
}
inline constexpr float trace(float v)
{
    return v;
}
inline constexpr double trace(double v)
{
    return v;
}
 ...
 * Matrix inversion
 */
template <typename MATRIX> MATRIX PURE gaussJordanInverse(const MATRIX &src)
{
...
template <typename MATRIX> CONSTEXPR MATRIX PURE fastInverse2(const MATRIX &x)
{
    typedef typename MATRIX::value_type T;
...
template <typename MATRIX> CONSTEXPR MATRIX PURE fastInverse3(const MATRIX &x)
{
    typedef typename MATRIX::value_type T;
...
{
    static_assert(MATRIX::NUM_ROWS == MATRIX::NUM_COLS,
                  "only square matrices can be inverted");
    return (MATRIX::NUM_ROWS == 2)
               ? fastInverse2<MATRIX>(matrix)
...
template <typename MATRIX> CONSTEXPR MATRIX PURE transpose(const MATRIX &m)
{
    // for now we only handle square matrix transpose
    static_assert(MATRIX::NUM_COLS == MATRIX::NUM_ROWS,
                  "transpose only supports square matrices");
...
{
    static_assert(MATRIX::NUM_COLS == MATRIX::NUM_ROWS,
                  "diag only defined for square matrices");
...
template <typename MATRIX> String8 asString(const MATRIX &m)
{
    String8 s;
...
/*
 * TMatProductOperators implements basic arithmetic and basic compound
 * assignments operators on a vector of type BASE<T>.
 *
 * BASE only needs to implement operator[] and size().
 * By simply inheriting from TMatProductOperators<BASE, T> BASE will
...
class TMatProductOperators
{
  public:
    ...
    //  matrix *= matrix
    template <typename U> const BASE<T> &operator*=(const BASE<U> &rhs)
    {
        BASE<T> &lhs(static_cast<BASE<T> &>(*this));
    ...
    template <typename U>
    friend CONSTEXPR BASE<T> PURE operator*(const BASE<T> &lhs,
                                            const BASE<U> &rhs)
    {
 ...
 *  - ROW_SIZE
 *
 * By simply inheriting from TMatSquareFunctions<BASE, T> BASE will
 * automatically get all the functionality here.
 */
...
template <template <typename U> class BASE, typename T> class TMatHelpers
{
  public:
    constexpr inline size_t getColumnSize() const
    {
        return BASE<T>::COL_SIZE;
    }
    constexpr inline size_t getRowSize() const
    {
        return BASE<T>::ROW_SIZE;
    }
    constexpr inline size_t getColumnCount() const
    {
        return BASE<T>::NUM_COLS;
    }
    constexpr inline size_t getRowCount() const
    {
        return BASE<T>::NUM_ROWS;
    }
    ...
    template <typename VEC> static CONSTEXPR BASE<T> translate(const VEC &t)
    {
        BASE<T> r;
    ...
    template <typename VEC> static constexpr BASE<T> scale(const VEC &s)
    {
        return BASE<T>(s);
    }
...
template <template <typename U> class BASE, typename T> class TMatTransform
{
  public:
    inline constexpr TMatTransform()
    {
        static_assert(BASE<T>::NUM_ROWS == 3 || BASE<T>::NUM_ROWS == 4,
                      "3x3 or 4x4 matrices only");
    }
...
template <template <typename T> class BASE, typename T> class TMatDebug
{
  public:

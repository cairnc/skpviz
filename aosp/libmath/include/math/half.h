...
class half
{
    struct fp16
    {
        ...
        void setS(unsigned int s) noexcept
        {
            bits = uint16_t((bits & 0x7FFF) | (s << 15));
        }
        void setE(unsigned int s) noexcept
        {
            bits = uint16_t((bits & 0xE3FF) | (s << 10));
        }
        void setM(unsigned int s) noexcept
        {
            bits = uint16_t((bits & 0xFC00) | (s << 0));
        }
        constexpr unsigned int getS() const noexcept
        {
            return bits >> 15u;
        }
        constexpr unsigned int getE() const noexcept
        {
            return (bits >> 10u) & 0x1Fu;
        }
        constexpr unsigned int getM() const noexcept
        {
            return bits & 0x3FFu;
        }
    };
    struct fp32
    {
        ...
        void setS(unsigned int s) noexcept
        {
            bits = uint32_t((bits & 0x7FFFFFFF) | (s << 31));
        }
        void setE(unsigned int s) noexcept
        {
            bits = uint32_t((bits & 0x807FFFFF) | (s << 23));
        }
        void setM(unsigned int s) noexcept
        {
            bits = uint32_t((bits & 0xFF800000) | (s << 0));
        }
        constexpr unsigned int getS() const noexcept
        {
            return bits >> 31u;
  ...
  public:
    ...
    unsigned int getExponent() const noexcept
    {
        return mBits.getE();
    }
    unsigned int getMantissa() const noexcept
    {
        return mBits.getM();
    }
    ...
    else
    {
        if (in.getE() == 0)
        {
            if (in.getM())
            {
                ...
                // (it's stupid because they can be represented as regular
                // float)
            }
        }
        else
...
{
    return android::half(android::half::binary,
                         android::half::ftoh(static_cast<float>(v)).bits);
}

} // namespace android

// C++20 disallows specializing std::is_floating_point for user types. Dropped
// since no AOSP code we port depends on half treating as floating_point.
namespace std
{

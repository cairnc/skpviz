              ...
              public TVecDebug<TVec3, T>
{
  public:
    ...
    explicit constexpr TVec3(no_init)
    {
    }
    ...
    constexpr TVec3(const TVec2<A> &v, B z)
        : x(v.x), y(v.y), z(static_cast<T>(z))
    {
    }

    template <typename A>
    explicit constexpr TVec3(const TVec3<A> &v) : x(v.x), y(v.y), z(v.z)
    {
    }
    ...
    {
        return TVec3(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z,
                     u.x * v.y - u.y * v.x);
    }
};

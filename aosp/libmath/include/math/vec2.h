              ...
              public TVecDebug<TVec2, T>
{
  public:
    ...
    explicit constexpr TVec2(no_init)
    {
    }
    ...
    template <typename A>
    explicit constexpr TVec2(const TVec2<A> &v) : x(v.x), y(v.y)
    {
    }

    // cross product works only on vectors of size 2 or 3
    template <typename RT>
    friend inline constexpr value_type cross(const TVec2 &u, const TVec2<RT> &v)
    {

                    ...
                    public TQuatDebug<TQuaternion, T>
{
  public:
    ...
    inline constexpr static size_type size()
    {
        return SIZE;
    }
    ...
    explicit constexpr TQuaternion(no_init) : xyzw(TVec4<T>::NO_INIT)
    {
    }
    ...
    template <typename A> constexpr TQuaternion(A w) : x(0), y(0), z(0), w(w)
    {
    ...
    constexpr TQuaternion(A w, B x, C y, D z)
    ...
    template <typename A>
    constexpr explicit TQuaternion(const TVec4<A> &v)
    ...
    template <typename A>
    constexpr explicit TQuaternion(const TQuaternion<A> &v)
        : x(v.x), y(v.y), z(v.z), w(v.w)
    {
    }
    ...
    template <typename A, typename B>
    constexpr static TQuaternion PURE fromAxisAngle(const TVec3<A> &axis,
                                                    B angle)
    {
        return TQuaternion(std::sin(angle * 0.5) * normalize(axis),
                           std::cos(angle * 0.5));
    }
};
...
constexpr inline quatd operator"" _id(unsigned long long v)
{ // NOLINT
    return quatd(0, static_cast<double>(v), 0, 0);
}
constexpr inline quatd operator"" _jd(unsigned long long v)
{ // NOLINT
    return quatd(0, 0, static_cast<double>(v), 0);
}
constexpr inline quatd operator"" _kd(unsigned long long v)
{ // NOLINT
    return quatd(0, 0, 0, static_cast<double>(v));
}
